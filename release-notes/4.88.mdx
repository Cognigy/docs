...
- Moved the [User Menu](/ai/agents/develop/projects-and-flows/projects) and the [Help button](/ai/agents/develop/projects-and-flows/projects) to the bottom-left corner of the Project sidebar
- Added the [Error Handling](/ai/agents/develop/node-reference/logic/think) section to the Think Node to control how Think Node errors are managed within a Flow
...
- Upgraded the `mongodb` dependency used by Mongo Nodes to v6. Now, [MongoDB Nodes](/ai/agents/develop/node-reference/service/mongo-db#restrictions) are compatible with MongoDB 3.6 or later versions
- Added the [Handover Already in Progress](/ai/agents/develop/node-reference/logic/lookup) status to the Handover Status in the Lookup Node. This status is applicable only to the Genesys Cloud Open Messaging handover provider
...
- Changed the type of the [Transcription Webhook](/ai/agents/develop/node-reference/voice/voice-gateway/transfer) field from URL to CognigyScript to support dynamic values
- Added the capability to enable the [Flow Input Timeout](/ai/agents/develop/node-reference/voice/voice-gateway/parameter-details) feature within the [AudioCodes Compatibility mode](/voice-gateway/migrate-from-ac-to-vg)
- Improved tracing log on the [Tracing](/voice-gateway/webapp/recent-calls) tab in Recent Calls within the Voice Gateway Self-Service Portal:
...
- Improved by removing the need to inject the Agent Copilot SDK into a script tag. It is now injected automatically, allowing direct use of `SDK.postback(payload)`. For more information, go to the [Copilot: IFrame Tile](/ai/agents/develop/node-reference/agent-copilot/set-iframe-tile#send-data-from-the-iframe-back-to-the-flow) and [Copilot: HTML Tile](/ai/agents/develop/node-reference/agent-copilot/set-html-tile#sending-json-data-back-to-the-flow) Nodes
...
- Implemented name validations for [inboxes](/live-agent/settings/inboxes#create-inboxes), [teams](/live-agent/settings/teams#create-teams), and [labels](/live-agent/settings/labels#create-labels) to ensure CSV safety. This improvement prevents the entry of Excel formulas that begin with the `=` character
...
to improve functionality and address known security vulnerabilities in the dependencies of Cognigy,
...
The products Cognigy, Insights, and Live Agent will be updated within the Cognigy installation, but to update Voice Gateway, you need to repeat the same steps only for the Voice Gateway installation.
