---
title: "4.51 - Happy Telecommunications Day \U0001F4E1"
---

import ReleaseNotesMongoDbMigration from '/snippets/release-notes/mongo-db-migration.mdx'
...
- Added [xApps](/xApps/overview) in Beta. Cognigy xApps are mobile first, micro web applications that empower enterprises to seamlessly connect all channels and enable personalized, multimodal self-service solutions uniting text, visuals, and voice. This feature is available to all Cognigy users
- Added [Agent Assist Workspace](/agent-copilot/overview) in Beta. Cognigy Agent Assist Workspace offers a variety of advanced features that empower agents to provide faster and more accurate customer support. The workspace is fully customizable with widgets that display the most relevant information for any chat and voice use case
- Added the handling of [Voice Gateway Call Events](/ai/agents/deploy/endpoint-reference/voice-gateway#call-events) via the [Lookup](/ai/agents/develop/node-reference/logic/lookup) Node

#### Improvements

- Added [Functions](/ai/platform-features/nlu/text-cleaner) and [Nodes](/ai/agents/develop/node-reference/ai/clean-text) to automatically parse utterances, including spelling (alpha echo lima tango) to the correct strings
- Added `uniqueIntentId` and `uniqueIntentFlowId` to the input object
- Shortened the xApps URL for improved usability.
- Moved the [Page Title](/ai/agents/develop/node-reference/xApp/init-xApp-session) field to the Style Customization section in the xApp: Init Session Node
...
- Renamed the Live Agent Escalations chart to [Handover in Time](/insights/reports/engagement#handovers-in-time) chart and updated its descriptions in the Engagement dashboard
...
- Embedded the [Agent Copilot workspace](/live-agent/assistants/agent-copilot) functionality into Live Agent. This feature is in beta
...
- Added the [Answering Machine Detection](/ai/agents/develop/node-reference/voice/voice-gateway/transfer) to the Voice Gateway Transfer Node
...
To configure the Ingress objects mentioned above, you need to create two new DNS entries and point them to your load balancer, which was provisioned during the installation of Cognigy. Here's an example of how you could name those DNS entries if your Cognigy UI is located at trial.cognigy.ai:
...
# customer running Cognigy/Cognigy Insights using this HelmChart.
cognigyApps:
  enabled: false
```
