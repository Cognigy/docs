...
- Added support for the new generative AI provider [Google Vertex AI](/ai/agents/develop/gen-ai-and-llms/model-support-by-feature) with `text-bison-001` LLM for the `LLM Prompt node`
...
- Added [Microsoft Authentication Library (MSAL) support](/ai/agents/deploy/endpoint-reference/azure-bot-services#access-scope) with legacy Azure Active Directory Authentication Library (ADAL) way of token exchange between an AI Agent and the Azure Bot Services Endpoint
