...
<Warning>
  ...
  We strongly recommend carrying out this migration since Gemini 1.0 Pro support will be removed from future Cognigy versions.
...
We have enhanced our existing [Google Gemini](/ai/agents/develop/gen-ai-and-llms/providers/google-gemini) integration by adding [support](/ai/agents/develop/gen-ai-and-llms/model-support-by-feature) for Google's latest models: `gemini-1.5-pro` and `gemini-1.5-flash`.

Additionally, we have extended our [AWS Bedrock](/ai/agents/develop/gen-ai-and-llms/providers/amazon-bedrock) integration to fully support AWS's new `Nova` model series.
...
This release introduces the ability to [pin Projects](/ai/overview/user-interface/main-page), allowing you to better organize your view when logging in to Cognigy. The list of pinned Projects will be stored in the user profile, and we plan to expand the range of Cognigy resources that can be pinned in the future.

##### Download for Knowledge Sources in CTXT

This release introduces the capability to download content stored in Knowledge Sources in the [Cognigy TXT (CTXT) format](/ai/agents/develop/knowledge-ai/knowledge-source/overview#download-knowledge-sources-as-ctxt). This feature enables customers to share content from our Knowledge AI product more easily across teams and Projects. The exporter preserves the structure of Knowledge Chunks, ensuring that re-imported files maintain the original Chunk organization.

#### Improvements
- Increased the button limit from 6 to 15 in the [Text with Buttons](/ai/agents/develop/node-reference/basic/say) output type
- Blocked inactive users from using SSO login
- Updated the links on the Get Started cards on the [Projects page](/ai/agents/develop/projects-and-flows/projects)
- Implemented the [prevention of user impersonation](/ai/administer/access/management-ui#block-user-impersonation) in the [Management UI](/ai/administer/access/management-ui)
- Added the capability to [track total token usage](/ai/agents/develop/gen-ai-and-llms/llm-session-token-counter). This capability allows users to monitor their LLM token usage per session
- Added a [Temperature](/ai/agents/develop/node-reference/ai/ai-agent) slider to the Advanced section of the AI Agent Node
- Added the [Hide References to External Resources in Transcripts](/ai/agents/deploy/endpoints/data-protection-and-analytics) setting to remove tags that can reference third-party APIs (`<a>`, `<img>`) from inputs to protect users
...
- Added the Generate Search Prompt parameter to the Grounding Knowledge section in the [AI Agent Node](/ai/agents/develop/node-reference/ai/ai-agent). This parameter is enabled by default and allows you to generate a context-aware search prompt before executing the knowledge search
...
- Added the capability to set `recognizer`, `fillerNoise`, and `synthesizer` via the [Advanced Session Parameters](/ai/agents/develop/node-reference/voice/voice-gateway/parameter-details) in the Set Session Config Node and via the Set Activity Parameters in the Say or Question Nodes
...
- Added support for images to the [Text with Quick Replies](/ai/agents/develop/node-reference/basic/say) output type. This improvement applies to Webchat v3
...
To ensure HA, Traefik pods are deployed in 3 Availability Zones (AZ) (Cognigy recommended setup) by default, one pod for each AZ.

- **For on-premises installations:**
Traefik HA default settings require that you run your cluster with 3 AZs (Cognigy recommended setup), and the Helm Release spawns HA replicas across the 3 AZs. If you deploy your cluster without AZs, override the zonal `podAntiAffinity` key in the `values.yaml` file, as follows:
