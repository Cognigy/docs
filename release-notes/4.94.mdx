...
<Warning>
  On-premises customers must add the `TMP_USE_LEGACY_NODE_DATA_VALIDATION: "true"` environment variable to the Cognigy
  `values.yaml` when updating to version 4.94.
</Warning>

<Warning>
  - [AudioCodes Nodes](/ai/agents/develop/node-reference/voice/audiocodes/overview) and [Generic Voice Nodes](/ai/agents/develop/node-reference/voice/generic/overview) will be deprecated with the release of Cognigy 4.96, on March 4, 2025. After this date, you won't be able to create these types of Nodes, but you can still edit or clone existing ones. Flows with these Nodes continue to work after the deprecation date.
...
- Renamed Contact ID to [User ID](/insights/explorers/conversations#list-of-sessions) to synchronize naming conventions across Cognigy products
