...
- Improved by adding support to remember the first accessed URL path and redirecting to it after logging in successfully
...
- Improved by adding support for trusting self-signed CA certificates and client certificates in the requests to Cognigy. Have a look at the Helm Values SSL section
...
- Fixed a bug where Cognigy multi-tenant users were not able to log in into the proper Cognigy organization using OAuth login
...
- Improved by polishing contact info section styling for displaying Cognigy contact profile fields
- Improved by changing the color of the agent messages to a slightly lighter blue to differentiate them from the Cognigy bot messages
- Improved by rendering Cognigy contact profile data in the conversation view under the contact info section.
    - A new value called `COGNIGY_AI_API_BASE_URL_WITH_PROTOCOL` must be added to the Helm values.yaml file to make it work. Refer to [Helm values URL configuration](/live-agent/installation/helm-values/helm-values#urls).
- Improved by removing functionality related to adding agents as now Cognigy is where the user management happens
...
- Improved by extending Cognigy OAuth handler logic to update user fields while doing log in when they are changed in Cognigy, such as the username
