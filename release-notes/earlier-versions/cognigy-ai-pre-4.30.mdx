...
This release of Cognigy.AI adds the ability to create entirely new Flows from child-branches of already existing Flows. This new functionality helps to reduce the overall size of big Flows into more manageable smaller fragments
...
- Improved by removing special characters from intents names, hence allowing to import packages created in previous versions of Cognigy.AI 4
...
### Features
#### New Agent templates
This release of Cognigy.AI contains new templates for our Agent creation wizard. We have introduced a variety of new templates in order to support our customers with getting started quicker
...
This release of Cognigy.AI adds a non-conversational Endpoint. The feature needs to be enabled using an additional environment variable. It allows customers to create an Endpoint that will not process inputs using NLU engines. The Endpoint can e.g. be used if Cognigy.AI is being used as a process orchestration tool
...
This release activates "Cognigy Insights" by default - Cognigy.AI 4.10.0 did not do that. An opt-out is available for customers that do not want to use Cognigy Insights right now. This opt-out will be removed with one of the next releases.
...
This release of Cognigy.AI allows you to select an external translation provider in the settings menu.
...
We have added the ability to configure CORS origins for the Cognigy.AI endpoint microservice. This allows our customers to additionally protect their endpoints by only allowing certain browser clients to connect. The default behavior of allowing all origins ('*') has not been changed.
