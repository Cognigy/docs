...
This release of Cognigy adds a new user interface for inspecting Playbook Runs. This allows our customers to see the status of Playbooks they ran server side. Take a look at our product documentation for more information.
...
- Improved by sending a helper email to those users who fail to log in due to a missing organisationId in the URL
...
This release of Cognigy adds the ability to create entirely new Flows from child-branches of already existing Flows. This new functionality helps to reduce the overall size of big Flows into more manageable smaller fragments
...
- Improved by removing special characters from intents names, hence allowing to import packages created in previous versions of Cognigy 4
...
- Fixed a bug where some keyphrases were not found when followed by a dot
...
This release of Cognigy adds the ability to execute Playbooks server side. We have added new RESTful API endpoints allowing our customers to start Playbook runs and inspect their results

#### Jump to Flow Node capabilities
This release of Cognigy adds the ability to inspect individual outputs within the Interaction Panel and understand which Flow Node has created a specific output. Our customers can jump to the respective Flow and Flow Node in order to further understand where an output is coming from
...
- Improved by updating the webchat and its widgets in service-webchat to the newest version (2.40.7)
...
### Features
#### New Agent templates
This release of Cognigy contains new templates for our Agent creation wizard. We have introduced a variety of new templates in order to support our customers with getting started quicker
...
### Features
This release of Cognigy gives customers the ability to override the Connections of a Snapshot with those of the Agent via an Endpoint setting
...
This release of Cognigy adds the ability to merge Flow from Packages with the Flows already existing in your Agent. The functionality can also be used through our RESTful API.

#### Additional Filter for Cognigy Insights Transcript Explorer
This release of Cognigy adds an additional "Flow Filter" to the Transcript Explorer in Cognigy Insights.

#### Additional Filter for Cognigy Insights Message Explorer
This release of Cognigy adds an additional "Flow Filter" to the Message Explorer in Cognigy Insights.
...
- Fixed a bug where an old Intent model could be used during Flow execution when the Locale is not explicitly selected in the Endpoint
...
This release of Cognigy adds the functionality to track the last activity of users in the Cognigy user interface. The data will be displayed to administrators of the platform in the access control tool
...
- Improved by adding a sidebar to Insights to navigate to Cognigy
...
This release of Cognigy allows users to be part of multiple Organizations. Users
...
This release of Cognigy adds Salesforce Service Cloud as a new Handover provider

#### Train on Default Replies
This release of Cognigy adds the ability to include the data of Default Replies in
...
This release of Cognigy adds two new Flow Nodes into Cognigy Flows. One lets you
...
- Improved by adding an explicit link to Cognigy Insights to the analytics widget of the Cognigy dashboard
...
This release adds a so-called Expert Mode to the Transcript Explorer in Cognigy Insights. Similar to the Expert Mode in Cognigy Interaction Panel, this mode shows additional information such as detected Slots and the classified Intent in addition to the actual transcript of conversations.
...
### Bug Fixes
- Fixed a bug related to authentication problems for OData 2.1 endpoint in Excel/PowerBI
...
- Fixed a bug related to an unusually high number of MongoDB connections
...
### Features
#### Avaya CPaaS
This release of Cognigy contains the new Avaya CPaaS endpoint which is an essential rewrite of the currently existing one. We have added many new capabilities and e.g. made TTS/STT language configurable.

#### Amazon Lex NLU Connector
This release of Cognigy adds a new NLU Connector for Amazon Lex.
...
This release of Cognigy adds a non-conversational Endpoint. The feature needs to be enabled using an additional environment variable. It allows customers to create an Endpoint that will not process inputs using NLU engines. The Endpoint can e.g. be used if Cognigy is being used as a process orchestration tool
...
- Fixed a bug where the removal of a custom Contact Profile schema field does not lead to the deletion of the actual data for the field in all Contact Profiles
...
This release activates "Cognigy Insights" by default - Cognigy 4.10.0 did not do that. An opt-out is available for customers that do not want to use Cognigy Insights right now. This opt-out will be removed with one of the next releases.
...
### Features
#### Cognigy Insights
This release of Cognigy makes Cognigy Insights the default tool for inbuilt analytics.
...
This release of Cognigy allows you to select an external translation provider in the settings menu.

#### Cognigy Marketplace
This release of Cognigy contains the ability to configure a marketplace from which additional Cognigy Extensions can be installed on the Extensions page
...
- Fixed file system permission issues for some of our NLU services when running Cognigy on top of restricted OpenShift platforms
...
#### Webchat embed code
This release of Cognigy adds an embed code for our WebchatWidget onto the Endpoint configuration page for Webchats

#### Jump to Flow functionality
This release of Cognigy adds a "Jump to Flow" button in our Endpoint editor. This allows users to quickly access Flows their Endpoints point to

#### Dynamic Fields in Flow Nodes
This release of Cognigy adds new functionality that allows our customers to use a new dynamic select field as a new field type in their Extensions. The feature can be used to dynamically fetch the content of a select field through e.g. an external API call
...
This release of Cognigy adds a set of filters to our Intent Trainer to simplify the workflow of reviewing records
...
This release of Cognigy adds the ability for on-premises customers to embed certain extensions into the baseline of Cognigy
...
### Bug Fixes
- Fixed a bug where certain messages could crash one of our analytics services which led to user transcripts no longer being stored
...
We have invested quite some time in refreshing the Cognigy Intent Trainer in order to improve the UX and simplify handling review tasks for our customers. More specifically, we have added the following new features:
...
#### Configurable CORS for Cognigy endpoint
We have added the ability to configure CORS origins for the Cognigy endpoint microservice. This allows our customers to additionally protect their endpoints by only allowing certain browser clients to connect. The default behavior of allowing all origins ('*') has not been changed.
...
- Fixed a bug where wrong data would be displayed on various Cognigy analytics dashboards
...
#### OpenID Connect
Cognigy already supports SSO (Single Sign-On) through the usage of our SAML 2.0 integration. With this release, we have now added support for OpenID Connect. Customers can now select which type of strategy they want to use in order to implement their corporate SSO strategy.
...
- Fixed a bug where Cognigy would find the wrong Intent when our cIntent feature is used in conjunction with executeFlow
...
- product alignment with Voice Gateway 2.4 features
...
- updated internal dependencies of our NLU services in order to support certain DNS schemes when e.g. using MongoDB Atlas for a Cognigy installation
...
- improved our documentation for applying a custom theme to the Cognigy user interface
...
Cognigy Live Agent (lite) is here! The Cognigy Live Agent replaces the old Cognigy Livechat in Cognigy version 4. Live Agent comes as a more de-coupled product and will be extended in the future.
...
#### Indian languages
These release of Cognigy introduces support for the following new languages:
...
- Fixed a bug where comments would not be shown on hexagonal nodes (e.g. our If-Node)
...
- Fix a bug where long sentences with line breaks would get cut-off when pasted into a CognigyScript field
...
This is our ``latest`` major release. We changed a lot, fixed a lot of issues, and added many new features that make Cognigy the most advanced conversational AI platform on the market. Feel free to explore our new documentation and sign up for your free product trial:
...
- Fixed a race-condition within the `endpoint` that sometimes would result in an `incorrect order of messages`
