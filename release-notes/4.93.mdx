...
- Fixed the issue where the Cognigy user interface didn't load in different environments after the v4.93.2 update
...
- Fixed the issue where logging in to the Voice Gateway Self-Service Portal with the Cognigy account wasn't working
...
With this release, we introduce support for the powerful [`amazon.titan-embed-text-v2:0`](/ai/agents/develop/gen-ai-and-llms/model-support-by-feature) embedding model by Amazon Bedrock. This model is now available for Cognigy Knowledge AI and hidden behind the `FEATURE_ENABLE_AWS_BEDROCK_EMBEDDING_LLM_WHITELIST` feature flag.
...
- Added the [Watermark URL](/webchat/v3/configuration#watermark) parameter to the Watermark section in the Webchat v3 Endpoint
- Moved the [Webchat Plugins](/webchat/v3/configuration#webchat-plugins) settings to the Demo Webchat Settings section in the Webchat v3 Endpoint
...
- Fixed the issue with the [File Upload](/live-agent/conversation/send-reply#request-file-uploads) plugin where files sent by the end user were uploaded but displayed as Unknown File in the conversation chat
...
- Added the [Goals Performance (in numbers)](/insights/reports/goals) chart to the Goals dashboard
- Shortened the [default date range](/insights/global-filters) in the Insights global filters from `30 days` to `7 days`
