...
<Accordion title="Cognigy SaaS Service Health Platform">
...
- Refined the Cognigy UI by switching back to the previous Sidebar menu
...
- Added the Stream Buffer Flush Tokens and Stream Buffer Flush Overrides options to the [Storage and Streaming](/ai/agents/develop/node-reference/service/llm-prompt) in the LLM Prompt Node
- Added the [Enable input sanitization](/ai/agents/deploy/endpoints/data-protection-and-analytics) option to disable HTML text input sanitization within the Endpoint Settings 
...
- Redesigned the Cognigy sidebar 
- Removed the Cognigy Journeys API
...
- Fixed the issue where the Intent description of an attached Flow's Intent was missing in the NLU inference results 
...
- Added the [Enable User No Input Timeout](/ai/agents/develop/node-reference/voice/voice-gateway/parameter-details) option in the Set Session Config Node to enable or disable the User No Input Timeout feature
...
- Added the [Collect Metadata](/webchat/v3/metadata) option to the Webchat v3 Endpoint Settings. This option lets you gather additional metadata from user browsers
...
- Added the [Enable text-to-speech option](/webchat/v3/features#text-to-speech) to the Chat Options settings section within the Webchat v3 Endpoint Settings
...
This schema migration is a one-time job that runs automatically during the upgrade to release 4.81.0. 
The migration job will be disabled in release 4.84.0.
