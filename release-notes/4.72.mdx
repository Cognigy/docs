...
<Check>
  This version solves an issue with an incorrect caching behavior of Cognigy.AI Handovers by deactivating the cache by default. If you are upgrading to this version, the following configuration, which was necessary in the older version of 4.72 is **NOT necessary**:
  ```yaml
  cognigyEnv:
...
- The [ElevenLabs Streaming TTS](/voice-gateway/webapp/streaming) feature is not available. We have switched your settings to traditional TTS
...
- Added the capability to activate the Copy button within the [Copilot: Next Action Tile](/ai/agents/develop/node-reference/agent-copilot/next-action-tile) and [Copilot: Knowledge Tile Nodes](/ai/agents/develop/node-reference/agent-copilot/next-action-tile) for all handover providers except Live Agent. For Salesforce, ensure to update your Salesforce integrations using [this guide](https://github.com/Cognigy/salesforce-integrations?tab=readme-ov-file#update)
