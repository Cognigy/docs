...
import AiDeprecatedAzureModels from '/snippets/ai/deprecated-azure-models.mdx'
import ReleaseNotesAiCopilotDnsNames from '/snippets/release-notes/agent-copilot-dns-names.mdx'
import ReleaseNotesHandoversBrokenComponent471Component472 from '/snippets/release-notes/handovers-broken-4.71-4.72.mdx'

<ReleaseNotesHandoversBrokenComponent471Component472/>

<AiDeprecatedAzureModels/>
...
<Check>
  This version solves an issue with an incorrect caching behavior of Cognigy Handovers by deactivating the cache by default. If you are upgrading to this version, the following configuration, which was necessary in the older version of 4.72 is **NOT necessary**:
  ```yaml
  cognigyEnv:
...
- Deactivated the default meta-data caching option for Cognigy Handovers (LRU_CACHE_ENABLED: false)
...
- The [ElevenLabs Streaming TTS](/voice-gateway/streaming) feature is not available. We have switched your settings to traditional TTS
...
- Added [Webchat v3](/webchat/v3/overview) as a preview feature. This is a new improved version of Webchat with updated design and various settings for widget customization. We encourage you to try it out and provide us with feedback.

#### Improvements

- Implemented the [LLM Entity Extract](/ai/agents/develop/node-reference/other-nodes/llm-entity-extract) Node and added the [LLM Entity Extraction Options](/ai/agents/develop/node-reference/basic/question) to the Question Node
- Added the capability to activate the Copy button within the [Copilot: Next Action Tile](/ai/agents/develop/node-reference/agent-copilot/next-action-tile) and [Copilot: Knowledge Tile](/ai/agents/develop/node-reference/agent-copilot/knowledge-tile) for all handover providers except Live Agent. For Salesforce, ensure you update your Salesforce integrations using [this guide](https://github.com/Cognigy/salesforce-integrations?tab=readme-ov-file#update)
- Extended the functionality of the Copilot: Next Action Tile Node to include the [Knowledge Assist](/ai/agents/develop/node-reference/agent-copilot/next-action-tile) options
...
- Enabled the [Agent Copilot Embedding URL](/ai/escalate/handover-reference/genesys-cloud-open-messaging) for the Genesys Open Message handover provider
...
- Added the capability to [select a handover provider](/ai/agents/deploy/endpoint-reference/voice-gateway#) for the Voice Gateway Endpoint. After handover initiation, a `gather` verb is sent to collect transcripts and forward them to the human agent 
...
- Added the UI element to [change the size of the conversation chat interface](/live-agent/conversation/other-operations#expand-or-collapse-the-chat-area)
