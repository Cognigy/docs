...
<Warning>
  ...
  - [Insights Beta API](https://api-trial.cognigy.ai/openapi#get-/insights/beta/messages/report/-queryHash-). Instead, use the [Cognigy OData endpoint](/ai/agents/analyze/odata).
  - The `actions.think` and `api.think` functions. Instead, use `actions.thinkV2()` and `api.thinkV2()`. The removal date is June 2025.
</Warning>

<Warning>
  - The shutdown date for the Google Gemini 1.0 Pro model is April 9, 2025. Google recommends migrating to [Gemini 1.5 Pro and Gemini 1.5 Flash](https://cloud.google.com/vertex-ai/generative-ai/docs/deprecations/gemini-1.0-pro) before April 9, 2025. We strongly recommend carrying out this migration since Gemini 1.0 Pro support will be removed from future Cognigy versions.
  - On April 29, 2025, we will discard `say` verbs with `empty text content` affecting the following Voice Gateway Nodes: [Say](/ai/agents/develop/node-reference/basic/say), [Question](/ai/agents/develop/node-reference/basic/question), [Optional Question](/ai/agents/develop/node-reference/basic/optional-question)
  - In September, 2025, OData 2.3 will be deprecated. Before this date, ensure a smooth transition by switching to the [Cognigy OData 2.4](/ai/agents/analyze/odata#supported-versions).
...
Introduced a [new handover provider for Salesforce MIAW](/ai/escalate/handover-reference/salesforce-miaw). With the Salesforce MIAW handover provider, you can integrate Cognigy with Salesforce Messaging for In-App and Web, enabling end users to connect with human agents working in a contact center that uses Salesforce.

#### Improvements

- Improved AI Agent voice capabilities in the [AI Agent creation wizard](/ai/agents/develop/manage-ai-agents) and [AI Agent Node](/ai/agents/develop/node-reference/ai/ai-agent). Now, you can add labels for TTS vendors and disable the audio cache
- Removed the beta flag from the [Yes/No Intents](/ai/platform-features/nlu/intents/yes-no-intents) feature

#### Bug Fixes

- Fixed the issue where the left-side menu in Cognigy wasn't reacting to Locale changes
- Fixed the issue where the Impersonate user function in the Management UI was missing from the frontend URL
- Fixed the issue where API requests for copying a Playbook weren't working
...
- Added the `notifyURL` parameter for additional cases, enabling proper reporting of `REFER` and `DIAL` success and failure statuses via the `call:status` [webhook event](/voice-gateway/creating-outbound-calls#basic-configuration-request)
...
- Fixed the issue where invalid empty transcripts from Microsoft Azure Speech Services caused errors when a Say Node was created
- Fixed the issue where fallback refer transfers without a configured Referred by setting led to an error
...
- Renamed Goals to Tasks in the [Contact Profile](/ai/agents/analyze/contact-profiles) to standardize naming conventions across Cognigy
- Replaced the `goals` property in the [Profile object](/ai/agents/develop/ai-agent-memory/profile) with `tasks`
