...
<Danger>
  The Cognigy Journeys feature has been removed.
</Danger>
...
- Added the [Advanced Parser](/ai/agents/develop/knowledge-ai/knowledge-source/text-extraction/advanced-parser) for Knowledge AI. This Parser is activated by default if configured for your installation 
- Added support for PPTX format in both the [Basic](/ai/agents/develop/knowledge-ai/knowledge-source/text-extraction/basic-parser) and [Advanced](/ai/agents/develop/knowledge-ai/knowledge-source/text-extraction/azure-ai) Parsers
- Added the API [`Post /v2.0/projects/{projectId}/nlu/scores`](/ai/platform-features/nlu/intents/intent-analyzer) request for matching and scoring sentences with NLU Intents
- Redesigned the Cognigy interface to create a clearer brand identity and deliver a seamless user experience throughout the product 
...
- Removed the Cognigy Journey feature from the Cognigy UI 
...
- The feature [Atmosphere Sounds](/ai/agents/develop/node-reference/voice/voice-gateway/parameter-details) adds the possibility to insert additional audio tracks into the conversation. One common usage is to add background sounds of keyboards clicking to simulate an office environment.
...
- Added the capability to generate Adaptive Cards via Generative AI in the [Copilot: Adaptive Card Tile Node](/ai/agents/develop/node-reference/agent-copilot/set-adaptive-card-tile)
