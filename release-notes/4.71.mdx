...
<Accordion title="Cognigy Handover problem solved in 4.71.4">
  This version solves an issue with an incorrect caching behavior of Cognigy Handovers by deactivating the cache by default. If you are upgrading to this version, the following configuration, which was necessary in the older version of 4.71 is **NOT necessary**:
  ```yaml
  cognigyEnv:
...
- Implemented [streaming](/voice-gateway/streaming) capability with the ElevenLabs Speech provider
...
You can do this by using the following command. Replace `<namespace>` with the namespace where Cognigy is deployed.
...
- Update `redis high availability` images:
