...
<Warning>
  ...
  - [Cognigy Serverless](/ai/for-developers/extensions#cognigy-serverless) is required starting with Cognigy 2026.21.0. The legacy Extension runtime for trusted and untrusted Extensions has been deprecated. The removal is planned for Cognigy 2026.21.0.
    ...
    - For on-premises installations, [install Cognigy Serverless](/ai/administer/installation/serverless-installation) before upgrading to Cognigy 2026.21.0. Otherwise, all Extensions will stop working after the upgrade.
  ...
  - The [`GET /v2.0/profiles`](/api-reference/contact-profiles/get-contact-profiles) API method has been deprecated. The removal is planned for Cognigy 2026.24.0. Use [`GET /v2.1/profiles`](/api-reference/contact-profiles/get-contact-profiles-performance) instead for improved performance when retrieving Contact Profiles.
</Warning>
...
- Fixed the issue where the audio connection wasn't properly updated during certain call transfers, causing audio issues after the transfer
- Fixed the issue where DTMF-only barge-in settings weren’t restored after the system finished waiting for caller input. As a result, calls incorrectly accepted speech as an interruption when only key presses were allowed
- Fixed the issue where speech transcription unexpectedly reconnected after a call transfer. This issue also occurred after a speech provider failover
- Fixed the issue where the system reported a media path change as successful while the call continued using the previous path
- Fixed the issue where audio failed to decrypt after one call side disconnected while the other remained active
...
- Standardized the Endpoints-specific API authentication key header for the [REST](/ai/agents/deploy/endpoint-reference/rest) and [Webhook](/ai/agents/deploy/endpoint-reference/webhook) Endpoints. Now, use the `x-cognigy-endpoint-key` request header for both Endpoints. The legacy `x-rest-endpoint-key` and `x-webhook-key` headers are still accepted for backward compatibility, but `x-cognigy-endpoint-key` takes precedence if both are provided
- Added the `isMasked` field to the [GoalEvents](/ai/agents/analyze/odata#goalevents) collection for the Cognigy OData endpoint 2.4
- Added the [`POST /v2.1/projects/{projectId}/logs/package`](/api-reference-logs/logs-v21/package-log-entries-into-a-downloadable-file) and [`GET /v2.1/projects/{projectId}/logs/package/{logsFileId}/download`](/api-reference-logs/logs-v21/download-a-packaged-logs-file) API methods to package log entries into a JSON file you can download. These API methods allow you to avoid paginating through logs when using the [`GET /v2.1/projects/{projectId}/logs`](/api-reference-logs/logs-v21/get-log-entries) API method. The methods are available to some customers via the `FEATURE_PACKAGE_LOGS` feature flag. Contact [Cognigy technical support](/help/get-help) to activate them for your installation
...
- Increased the default maximum upload size for UI uploads of [Snapshots](/ai/agents/deploy/snapshots#limitations) and [Packages](/ai/platform-features/packages#limitations) to 8 GB. Direct Cognigy API upload limits remain unchanged
...
- Added the Region option to the [Voice Preview settings](/ai/agents/test/voice-preview#set-up-a-speech-provider) for the Deepgram speech provider
- Added the option to opt in to the [Deepgram Model Improvement Program](/ai/agents/test/voice-preview#set-up-a-speech-provider) for Deepgram and Deepgram Flux under **Speech > Speech Service** in the Voice Gateway Self-Service Portal. Opting in allows Deepgram to use call audio and transcripts to improve its models and may reduce per-request costs. Customers can opt out to keep their call data private
