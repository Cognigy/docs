...
- Added the [Context-Aware Search](/ai/agents/develop/node-reference/other-nodes/search-extract-output) option to the Search settings in the Search Extract Output Node. This feature considers the transcript's context for the search, allowing AI Agents to address follow-up questions
...
- Renamed the Live Agent Escalations chart to [Live Agent Handovers](/insights/reports/live-agent#live-agent-handovers) chart in the Live Agent dashboard
...
- Added the option to [filter conversations based on their priority](/live-agent/conversation/assign-conversations#assign-priority)
...
- Added the opportunity to access the Voice Gateway Self-Service Portal [via Cognigy user credentials](/voice-gateway/getting-started#set-up-voice-gateway-for-your-organization). This feature is available to users with the [Account](/voice-gateway/webapp/users#account) role

#### Improvements

- Added the TTS Label and STT Label fields to the [Session Speech Parameters](/ai/agents/develop/node-reference/voice/generic/session-speech-parameters-config) Node, [Voice Gateway Set Session Config](/ai/agents/develop/node-reference/voice/voice-gateway/set-session-config) Node, [Voice Gateway Transfer](/ai/agents/develop/node-reference/voice/voice-gateway/transfer) Node, [Cognigy Voice Gateway](/ai/agents/deploy/endpoint-reference/voice-gateway) Endpoint. This allows for the definition of preferred speech credentials configured on the Voice Gateway Self-Service portal directly within the Flow
- Added the Google Model field to the [Session Speech Parameters](/ai/agents/develop/node-reference/voice/generic/session-speech-parameters-config) Node, [Voice Gateway Set Session Config](/ai/agents/develop/node-reference/voice/voice-gateway/set-session-config) Node, [Voice Gateway Transfer](/ai/agents/develop/node-reference/voice/voice-gateway/transfer) Node, [Cognigy Voice Gateway](/ai/agents/deploy/endpoint-reference/voice-gateway) Endpoint. This allows selecting a specific speech model used for transcription.
- Improved the TTS error handling by adding more logging and a warning message
