...
<Warning>
  Deprecations in Cognigy 2026.7.0:

  - The States in the Interaction Panel, States functions, and States API methods have been deprecated. The removal is planned for Cognigy 2026.12.0. Delete your States and retrain your NLU model before the removal. Use [Intent Conditions](/ai/platform-features/nlu/intents/conditions) as an alternative for controlling Intent recognition.
  - The [`GET /v2.0/projects/{projectId}/logs`](/api-reference-logs/logs/get-log-entries) API method has been deprecated. The removal of this API method is planned for Cognigy 2026.20.0. Use the `GET /v2.1/projects/{projectId}/logs` API method with `start` and `end` parameters instead.
</Warning>
...
- Fixed the issue where Extensions imported via Snapshot failed with the error `Unable to find extension module`. The stored file path in the Extension referenced the original pre-import ID instead of the new ID generated during the Snapshot import. The issue occurred after upgrading to Cognigy 2026.7.1
...
- Fixed the connection between Cognigy and Voice Gateway, ensuring Mute Speech Input prevents speech barge-in
...
Introduced the [NiCE CXone](/ai/escalate/handover-reference/nice-cxone/nice-cxone-integrated) handover provider, allowing end users to hand over their conversations from an AI Agent to a human agent in a contact center. This feature is available for Cognigy installations integrated with NiCE CXone.
...
- Fixed the Playbooks schema and description in the Cognigy OpenAPI documentation
...
- Fixed the `$filter` behavior for the Sessions collection in the Cognigy OData endpoint when values in the `goals` field were stored as `text[]`. Now, filters using string functions, such as `contains`, `startswith`, and `endswith`, work correctly
...
If you previously added the `FEATURE_USE_NEW_PAGINATION_PLUGIN` variable in `cognigyEnv` or `extraEnvVars`, remove the variable before updating to Cognigy 2026.7.0. This variable is now set by default and no longer needs to be configured manually.
