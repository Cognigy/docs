...
import ReleaseNotesFeatureRemoval from '/snippets/release-notes/feature-removal.mdx'
...
- Added the ability to [forward Genesys bot messages to the end user](/ai/escalate/handover-reference/genesys-cloud-open-messaging) to the end user while using the Genesys Cloud Open Messaging handover provider. These messages will be injected into the chat. To enable this feature for on-premises installations, use the `GENESYS_CLOUD_OM_HANDLE_BOT_MESSAGE: "true"` feature flag
