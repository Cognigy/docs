...
- Updated the Agent Copilot configuration. Adding the Agent Copilot config via API is no longer mandatory. In the Copilot section of the Endpoint Settings, select [Workspace > None](/agent-copilot/getting-started) and use a [Copilot: Set Grid Node](/ai/agents/develop/node-reference/agent-copilot/set-grid) to set up the workspace within your Flow
