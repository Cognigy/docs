...
import ReleaseNotesUpcomingFeatureRemovals from '/snippets/release-notes/upcoming-feature-removals.mdx'

<ReleaseNotesUpcomingFeatureRemovals/>
...
- Added charts for [monitoring the number of Knowledge Chunks and Queries metrics](/ai/administer/access/admin-center/dashboard) according to a [new license](/ai/agents/develop/knowledge-ai/activate#apply-for-a-license) for the Knowledge AI feature. To ingest new Knowledge Sources after the v4.78.0 update, a new license key for the Knowledge AI feature should be updated in the Cognigy installation, and the maximum Knowledge Chunks quota should be allocated for the organization.

#### Improvements

- Added the functionality to record user acceptance of the [Privacy Notice](/webchat/v3/features#privacy-notice) within the [Contact Profile](/ai/agents/analyze/contact-profiles#profile-schema)
- Added the [Minimize](/webchat/v3/features#controlling-the-chat-window) button at the top of the Chat screen within the Webchat v3 interface 
- Updated the Webchat v3 `preview` tag to `beta`
- Renamed the Organization Details page to [Admin Center](/ai/administer/access/admin-center/overview)
...
- Updated the Agent Copilot configuration. Adding the Agent Copilot config via API is no longer mandatory. In the Copilot section of the Endpoint Settings, select [Workspace > None](/agent-copilot/getting-started/overview) and use a [Copilot: Set Grid Node](/ai/agents/develop/node-reference/agent-copilot/set-grid) to set up the workspace within your Flow
- Added support for multiple languages in addition to English
- Increased the size limit of a custom field in the Cognigy OData Endpoint. Now, the maximum length of a custom field is 1024 characters
...
- Enabled bidirectional audio support for the [`listen`](/voice-gateway/references/verbs/listen) verb 
