...
- Added the option to choose whether the `AND` or `OR` operators are used in the [Search Extract Output](/ai/agents/develop/node-reference/other-nodes/search-extract-output) Node for filtering search results by [Source Tags](/ai/agents/develop/knowledge-ai/knowledge-source/overview-tags)
