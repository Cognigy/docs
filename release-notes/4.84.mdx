...
import ReleaseNotesLlmRemovals from '/snippets/release-notes/llm-removals.mdx'

<Danger>
  The Any Slots feature has been removed. Instead, use the [LLM Entity Extraction](/ai/agents/develop/node-reference/other-nodes/llm-entity-extract) Node to identify Slots in the user input via LLMs.
</Danger>

<ReleaseNotesLlmRemovals/>
...
- Added [Debug mode](/ai/agents/test/interaction-panel/chat#debug-mode) to the Interaction Panel, replacing its predecessor, Expert mode. Debug mode provides detailed information on Flow execution, including triggered Intents, matched Slots, State changes, and Flow transitions. Debug Messages can be triggered via the [Debug Message](/ai/agents/develop/node-reference/basic/debug-message) Node or [debug actions](/ai/for-developers/code/api-functions) in the Code Node

#### Improvements

- Added the option to choose whether the `AND` or `OR` operators are used in the [Search Extract Output](/ai/agents/develop/node-reference/other-nodes/search-extract-output) Node for filtering search results by [Source Tags](/ai/agents/develop/knowledge-ai/knowledge-source/knowledge-source-tags)
- Added support for the `text-embedding-3-large` and `text-embedding-3-small` models provided by [OpenAI and Azure OpenAI](/ai/agents/develop/gen-ai-and-llms/model-support-by-feature). These models are now available for use in the [Knowledge Search](/ai/agents/develop/knowledge-ai/overview#which-models-to-choose) feature
- Marked the GPT-3.5 family models as [legacy](/ai/agents/develop/gen-ai-and-llms/model-support-by-feature) and removed the deprecated `text-davinci` model
...
- Improved the [accessibility of Webchat v3](/webchat/v3/accessibility) by adding a [Video Captions URL](/ai/agents/develop/node-reference/basic/say) field for the Video type in the Say, Question, and Optional Question nodes, and by enabling the download of audio and video text transcripts
...
- Added the option to hide and [display empty fields](/live-agent/conversation/overview#view-hidden-fields-in-contact-profile-details) in the Contact Profile on the Details tab in the right panel of the conversation interface
