...
<Warning>
  With this release, the Understood Messages concept and its logic in the [NLU Performance](/insights/reports/nlu-performance#understood-messages) and [Intent Trainer](/ai/platform-features/nlu/intents/intent-trainer) will be deprecated. Use the [Goals and Tasks](/ai/agents/analyze/goals-and-tasks/overview) concepts to evaluate whether your AI Agent understood a particular phrase.
...
- Fixed the issue with invalid Project IDs in the [Inbox Settings](/live-agent/settings/inboxes). Previously, you could enter an invalid Project ID in the Inbox Settings and no errors would be raised. Now, a validation step does not allow entering invalid an invalid Project ID
