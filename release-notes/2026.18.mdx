...
- Fixed the issue where certain IP allowlist entries matched unintended IP addresses and where blocking malicious traffic caused a memory leak by updating `vg-drachtio` to `1.3.5`
...
- Fixed the issue where the Intent Level list of a Lookup Node wasn't editable. Saving either reverted the selected value to the default value or failed with a `400 Bad Request` error
