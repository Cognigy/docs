---
title: "4.67 - Happy New Year \U0001F389❄️"

---

import ReleaseNotesSingleReplicaMigration from '/snippets/release-notes/single-replica-migration.mdx'
...
<ReleaseNotesSingleReplicaMigration/>
...
- Added the [Generic Settings](/ai/agents/deploy/endpoint-reference/voice-gateway) section to the Voice Gateway Endpoint
...
- Added [ElevenLabs](/voice-gateway/references/tts-and-stt-vendors) as a Text-to-Speech (TTS) provider
- Improved the Voice Gateway Endpoint by adding a setting to trim `input.data` for better transcripts
- Added support to import the [secondary locale of the package](/ai/platform-features/packages) as the primary locale of the agent
...
- Fixed the issue where the current year was not displayed in the Billable Conversations filter on the Organization Details page
...
- Added the capability for human agents to create draft messages. If a [reply](/live-agent/conversation/send-reply#send-replies-to-users) or a [private](/live-agent/conversation/send-reply#send-replies-to-other-human-agents) message is composed but not sent immediately, the message will be saved for 24 hours

#### Improvements

- Renamed Live Agent Assist Bot to [Agent Copilot Bot](/live-agent/assistants/agent-copilot-bot)
