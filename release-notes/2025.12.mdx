...
<Warning>
  The [Insights Beta API](https://api-trial.cognigy.ai/openapi#get-/insights/beta/messages/report/-queryHash-) has been removed. Use the [Cognigy OData endpoint](/ai/agents/analyze/odata) instead.
...
- Added support for a [custom authentication header name](/ai/agents/develop/gen-ai-and-llms/providers/openai-compatible) field to the OpenAI-compatible LLM provider settings
...
- Fixed the issue in `feature-server` where DTMF inputs were gathered after the maximum number of digits had been reached
...
- Added support for the [`speak`](/webchat/v3/accessibility#adaptive-cards) parameter for adaptive cards, which allows you to include spoken text summary of the adaptive card for better accessibility and user engagement
...
Introduced the [Business Dashboard](/insights/dashboards/business). It gives you a quick overview of your key session metrics through interactive charts and tables, and includes location-based activity, active sessions by AI or human agents, and business outcomes.
...
#### Cognigy + Insights + AI Copilot, Voice Gateway

Traefik, the Kubernetes ingress controller, has been updated to enhance functionality and address known security vulnerabilities in the dependencies of Cognigy and Cognigy Insights. This update upgrades Traefik to version 3.4.0 and includes the latest Helm chart, reflecting a commitment to continuous improvement.

##### Traefik Update

Make updates for Cognigy and Voice Gateway separately, because they have different Helm charts and `values.yaml` files.
