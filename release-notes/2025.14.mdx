...
<Warning>
    If you have the Knowledge AI feature enabled and Qdrant is deployed, upgrading Cognigy to version 2025.14.0 will automatically upgrade Qdrant to v1.13.4. If you need to roll back Cognigy 2025.14.0 to an earlier version, refer to the [Infrastructure section](#infrastructure).
</Warning>
...
- The following headers added in Cognigy 2025.14.0 were removed because they broke some xApps by restricting iframe embedding and cross-origin resource loading:
...
- Added gravatar to the [Main page](/ai/overview/user-interface/main-page). Now, you can see your gravatar in the upper-right corner of the Cognigy UI, making it easier to identify your account
...
For environments where the Knowledge AI feature is enabled and Qdrant is deployed, upgrading Cognigy to version 2025.14.0 will automatically upgrade Qdrant to v1.13.4.

If you need to roll back Cognigy 2025.14.0 to an earlier version, don't roll back Qdrant to v1.8.3. The Qdrant upgrade isn't reversible.

To prevent issues, before rolling back Cognigy 2025.14.0 to an earlier version, specify the v1.13.4 tag in the Qdrant configuration as follows:
...
This configuration ensures that Qdrant will continue to run on v1.13.4 even if Cognigy 2025.14.0 and its Qdrant dependency Helm chart are rolled back.
