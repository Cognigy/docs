...
- Added the Duration Limit setting to the [Transfer Node](/ai/agents/develop/node-reference/voice/voice-gateway/transfer) and [Voice Gateway Endpoint](/ai/agents/deploy/endpoint-reference/voice-gateway) when using dial. With this setting, you set the maximum duration of call, after which the call ends even if the caller is on the line
