---
title: "4.46 - Happy National Be Heard Day \U0001F50A\U0001F91D"
---
...
- Added [Yes/No Intents](/ai/platform-features/nlu/intents/yes-no-intents) in Beta, allowing to detect Yes/No using a machine learning based Intent model. This feature is available to all Cognigy users

#### Improvements

- Added an API for tracking analytics in the Code Node
- Renamed the Complete Text Node to the [GPT Prompt](/ai/agents/develop/node-reference/service/llm-prompt) Node
...
- Added the option to [trust an Extension](/ai/for-developers/extensions#make-extensions-trusted) via UI
- Improved by trusting the extension after the update if it is already trusted
- Improved by rendering xApps submit payloads in the Live Follow session of the Interaction panel and Insights UI. To configure that, go to [Set HTML xApp State](/ai/agents/develop/node-reference/xApp/set-html-xApp-state#description) or [Set AdaptiveCard xApp state](/ai/agents/develop/node-reference/xApp/set-AdaptiveCard-xApp-state#description) Nodes
...
- Supervisor can now [override the status of any Agent](/live-agent/reports/supervisor-overview)
...
- Added the Nuance speech provider to the Voice Gateway web application and to Cognigy Voice Nodes
...
- Fixed the Session Config node Barge-In feature where it only allows users to barge in once
...
In this release, we added support to use static public IP on the SBC-RTP nodes in an EKS cluster.
...
For more information, read [AWS EKS Cluster Setup](https://github.com/Cognigy/cognigy-vg-helm-chart/blob/main/docs/003-aws-eks-cluster-setup.md) on GitHub.
