...
<Warning>
  The following models have been removed from Cognigy because OpenAI and Microsoft Azure OpenAI no longer support them: `gpt-3.5-turbo`, `gpt-3.5-turbo-instruct`, and `gpt-4`.
...
- Fixed the issue with the Cognigy OData endpoint where account-level filtering was not correctly applied to the `ReportingEvent` collection
