...
description: "Configure Agent Copilot for voice-to-chat applications with next action suggestion and conversation transcript widgets for human agents in contact centers."
sidebarTitle: "Voice-to-Chat"
keywords: ['voice-to-chat setup', 'tts handover', 'voice to chat workspace', 'copilot configuration']
---
...
- Configure a speech provider in [Cognigy](/ai/agents/test/voice-preview#set-up-a-speech-provider) and the [Voice Gateway Self-Service Portal](/voice-gateway/webapp/speech-services).
...
- Configure a contact center platform that you can integrate with a [handover provider](/ai/escalate/handover-reference/overview). For example, for [Live Agent](/ai/escalate/handover-reference/live-agent), you need to have an [Inbox](/live-agent/settings/inboxes) with the [Project ID and the Webhook URL set up](/live-agent/settings/inboxes#edit-inboxes) and, if necessary, the **Enable Agent Copilot Workspace** option activated.
...
<Steps>
  ...
  <Step title="Create an Agent Copilot Flow">
    1. In your Cognigy Project, go to **Build > Flows** and create a [Flow](/ai/agents/develop/projects-and-flows/overview) and give it a clear name, for example, `Agent Copilot`.
  ...
  <Step title="Create a Voice Gateway Application">
    ...
    2. On the **Add an application** page, configure the following:
        ...
        - **Speech synthesis vendor** and **Speech recognizer vendor** — make sure you configure the same speech provider as configured in [Cognigy](/ai/agents/test/voice-preview#set-up-a-speech-provider).
