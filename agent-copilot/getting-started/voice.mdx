...
description: "Set up Agent Copilot for voice applications with a workspace including next action widgets and call transcripts for human agents in contact centers."
sidebarTitle: "Voice"
slug: "voice"
keywords: ['copilot voice setup', 'voice workspace', 'voice ai agent', 'copilot configuration']
---
...
- Configure a speech provider in [Cognigy.AI](/ai/agents/test/voice-preview#configure-speech-provider) and the [Voice Gateway Self-Service Portal](/voice-gateway/webapp/speech-services).
...
- Configure a contact center platform that supports voice calls, for example, [Genesys Cloud Open Messaging](/ai/escalate/handover-reference/genesys-cloud-open-messaging), and can integrate with a [handover provider](/ai/escalate/handover-reference/overview).
...
<Steps>
  ...
  <Step title="Create a Voice Gateway Application">
    ...
    2. On the **Add an application** page, configure the following:
        ...
        - **Speech synthesis vendor** and **Speech recognizer vendor** — make sure you configure the same speech provider as configured in [Cognigy.AI](/ai/agents/test/voice-preview#configure-speech-provider).
