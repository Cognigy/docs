...
description: Learn how Cognigy Insights manages time zone settings with pre-aggregation for accurate data reporting.
keywords: ['time zone', 'pre-aggregation', 'insights settings', 'utc', 'data analysis']
---

import InsightsNewFramework from '/snippets/insights/new-framework.mdx'
import InsightsTimeZoneOdataAndInsights from '/snippets/insights/time-zone-odata-and-insights.mdx'

<InsightsNewFramework/>
...
After migrating to the new framework, Insights introduced [pre-aggregation](/insights/data-management/pre-aggregation) to improve data retrieval performance.
...
<Tabs>
  ...
  <Tab title="On-Premises">
        ...
        Before installation or an upgrade, we strongly advise aligning the list of supported time zones with the customer's preferences. To do this, specify `CUBEJS_SCHEDULED_REFRESH_TIMEZONES=<time-zone-1>,<time-zone-2>,<time-zone-3>` in the `values.yaml` file. For example, `CUBEJS_SCHEDULED_REFRESH_TIMEZONES=UTC,CET,ACST`.
  </Tab>
</Tabs>
...
- [Data pre-aggregation](/insights/data-management/pre-aggregation)
- [Time Zone](/insights/data-management/time-zone)
