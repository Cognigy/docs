...
description: Use global filters in Cognigy Insights to focus reports on specific time spans, Endpoints, Snapshots, and Locales for targeted data analysis.
keywords: ['global filters', 'insights filters', 'time span', 'endpoints filter', 'data analysis']
---
...
<Frame>
  <img src="/_assets/insights/global-filters.png" alt="Global filter bar with time span, endpoint, snapshot, and locale options" style={{ width: 'auto' }} />
</Frame>
...
When **Select time** is turned on, the start time is included, and the end time isn't.
For example, 09:00–10:00 includes sessions starting at or after 09:00:00 and before 10:00:00. Any session starting at 10:00:00 is counted in the next time window.
This behavior only applies to the **Custom Period** setting. Date-based presets, such as **Last 7 Days** or **This Month**, are unaffected.
...
For more information, read the [Endpoints](/ai/agents/deploy/endpoints/overview) documentation.
...
5. _(Optional)_ To view only selected options, click ![filter](/_assets/icons/black-and-white/filter.svg) in the **Type to search** field.
...
### Interaction Panel, Playbooks, and Simulations

By default, the sessions created via the Interaction Panel, Playbooks, or Simulator aren't included in the reports or explorers. To include such data, proceed as follows:

1. At the top of the Insights interface, go to the filter bar.
2. Click **Endpoint**.
3. Activate the **Interaction Panel / Playbooks / Simulations** option.
...
Snapshots save the current form of your AI Agent and include Flows, Lexicons, Connections, Functions, and more. The Snapshots serve as an immutable form of an AI Agent in Cognigy.
For more information, read the [Snapshots](/ai/agents/deploy/snapshots) documentation.
...
5. *(Optional)* To view only selected options, click ![filter](/_assets/icons/black-and-white/filter.svg) in the **Type to search** field.
...
For more information, read the [Localization](/ai/platform-features/translation-and-localization/localization) documentation.
...
For more information, refer to the [Goals](/ai/agents/analyze/goals-and-tasks/goals) documentation.

To use the **Goals** filter, follow these steps:

1. In the left-side menu of the Insights interface, select where you want to filter goals: the **Goals Report** or the **Conversations Explorer**.
...
5. _(Optional)_ To view only the selected options, click ![filter](/_assets/icons/black-and-white/filter.svg) in the **Type to search** field.
6. _(Optional)_ To reset the search results, click **x** in the **Type to search** field.
...
- When reloading an open transcript in the Conversations explorer, you will land on the same page with the transcript open.

## More Information

- [Cognigy Insights](/insights/overview)
- [Download Report](/insights/download-reports)
- [Interaction Panel](/ai/agents/test/interaction-panel/overview)
- [Snapshots](/ai/agents/deploy/snapshots)
