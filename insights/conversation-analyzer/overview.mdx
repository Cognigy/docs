...
<a href="/release-notes/2026.19"><Badge className="version-badge" color="purple">Updated in 2026.19 (beta)</Badge></a>
...
<div style={{textAlign: 'center'}}>
   <video width="650" height="400" controls autoPlay title="Conversation Analyzer Overview" aria-label="The video shows an overview of Conversation Analyzer's three dashboards: Topic Discovery, Basic Analysis, and Custom Evaluation. The video demonstrates how to configure evaluation settings, run analysis on conversations, and check results across the dashboards." style={{border: '1px solid #ccc', borderRadius: '12px', display: 'block', margin: '0 auto'}}>
...
- **Consistent evaluation**. Unlike manual reviews, the analyzer applies the same criteria to every conversation and every run, helping reduce reviewer bias and inconsistency.
...
1. In Cognigy, open your Project.
...
- Conversation Analyzer doesn't support the following models:
    - Anthropic — `claude-sonnet-4-5`, `claude-sonnet-5`, `claude-opus-4-7`, `claude-opus-4-8`, `claude-opus-5`
    - Google GenAI — `gemini-3.6-flash`, `gemini-3.7-flash`
    - AWS Bedrock — `custom - anthropic.claude-sonnet-4-6`, `amazon.nova-pro-v1:0`, `amazon.nova-premier-v1:0`, `amazon.nova-2-lite-v1:0`
