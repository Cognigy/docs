...
"seo":
  "metatags":
    "description": "The Session Management section allows you to configure session expiration settings for Endpoints, controlling how long user sessions remain active before requiring a restart."
keywords: ['session management', 'session expiration', 'user sessions', 'conversation management', 'endpoint settings']
---
...
- By default, the limit is 30 minutes. For dedicated SaaS and on-premises installations, this value can be overridden if a lower value is set in the [Management UI](/ai/administer/access/management-ui#expiration-values-ttl-for-sensitive-data) or via the `MAX_SESSION_STATE_TTL_IN_MINUTES` environment variable.