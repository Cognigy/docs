...
seo:
  metatags:
    description: >-
      Snapshots are immutable versions of an AI Agent in Cognigy.AI that
...
<a href="/release-notes/4.47"><Badge className="version-badge" color="blue">Updated in 4.47</Badge></a>
...
You can also download a Snapshot as a Cognigy Snapshot file. Snapshots files have a `.csnap` extension and can be used only in Cognigy.AI. Also, they are encrypted because they may include sensitive information, such as data in [Connections](/ai/platform-features/connections).
...
- [Handover Providers](/ai/escalate/handovers)<sup><a href="#footnote1">1</a></sup>
...
- You can store up to 256 MB of Snapshots. If you reach this limit, you must first delete other Snapshots before creating or uploading new ones. For on-premises installations, you can increase the maximum memory for Snapshots using the `SNAPSHOT_MAX_FILE_SIZE` environment variable in `values.yaml`.
...
<Accordion title="Important Warning">
...
<Tabs>
  <Tab title="GUI">
    ...
    <Accordion title="Download a Snapshot">
      ...
      1. On the **Snapshots** page, select ![vertical-ellipsis](/_assets/icons/black-and-white/vertical-ellipsis.svg) **> Prepare Download**. A new [task](/ai/agents/develop/projects-and-flows/projects) named **Prepare Download** is created.
  ...
  <Tab title="API">
    You can view, create, delete, and [restore](#restore-a-snapshot) Snapshots using the [Cognigy.AI API](https://api-trial.cognigy.ai/openapi#get-/v2.0/snapshots) as well as download and upload them.
  </Tab>
  <Tab title="CLI">
...
<span style={{ fontSize: "0.8rem" }}>
  ...
  Snapshots don't support the [NiCE CXone](/ai/escalate/handover-reference/nice-cxone/overview) handover providers because they aren't transferable between Projects. If you want to use the same handover provider in another Project or environment, create a new handover provider.
</span>