...
- The maximum Snapshot size depends on how you create or upload it:
    - **Cognigy.AI UI**: You can create and upload Snapshots of up to 8 GB. For on-premises installations, you can increase this limit by setting the `RESUMABLE_UPLOAD_MAX_FILE_SIZE` environment variable in `values.yaml`. The same limit also applies to [Packages](/ai/platform-features/packages).
    - **Cognigy.AI API**: You can create and upload Snapshots of up to 2 GB. Snapshots that exceed this limit can't be created or uploaded. For on-premises installations, you can increase this limit by setting the `SNAPSHOT_MAX_FILE_SIZE` environment variable in `values.yaml`.
