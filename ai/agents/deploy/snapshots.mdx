...
seo:
  metatags:
    description: >-
      Snapshots are immutable versions of an AI Agent in Cognigy that
      includes various resources such as Flows, Lexicons, Connections,
      Functions, and more. Snapshots can be used to create a copy of an AI Agent
      for deployment, to create backups, or to revert to an earlier state if
      needed.
keywords: ['snapshots', 'deployment', 'version control', 'backup', 'production', 'development', 'staging']
---

<a href="/release-notes/2026.16"><Badge className="version-badge" color="blue">Updated in 2026.16</Badge></a>

A _Snapshot_ is an immutable version of a Project that you can use to publish AI Agents through [Endpoints](/ai/agents/deploy/endpoints/overview). Snapshots are intended for production. They offer maximum performance and aren't affected by changes in the Project configuration.

You can also download a Snapshot as a Cognigy Snapshot file. Snapshots files have a `.csnap` extension and can be used only in Cognigy. Also, they are encrypted because they may include sensitive information, such as data in [Connections](/ai/platform-features/connections).

You can use Snapshots for the following purposes:
...
- [Functions](/ai/for-developers/functions)
...
- [Natural Language Understanding (NLU) Connectors](/ai/platform-features/nlu/external/nlu-connectors/overview)
- [Playbooks](/ai/agents/test/playbooks)
- [Settings](/ai/agents/develop/projects-and-flows/projects#working-with-projects)
- [Tokens](/ai/platform-features/tokens)
- [Trained NLU Intent models](/ai/platform-features/nlu/intents/overview)
...
- The `text-embedding-3-model` [model](/ai/agents/develop/gen-ai-and-llms/model-support-by-feature) increases the size of Snapshots, leading to longer download and upload times.
- Earlier versions of Cognigy might not support Snapshots created in newer versions and can cause unexpected errors.
- Snapshot creation fails with an error if any JSON key in the Project's data contains a period (`.`). Periods are reserved for dot notation used to access nested properties in Cognigy, for example, `context.user.name`. Avoid using periods in JSON keys across all Cognigy resources and other data included in the Snapshot.

## Limitations

- You can create up to 10 Snapshots per AI Agent. For on-premises installations, you can change this value using the `MAX_AMOUNT_SNAPSHOTS_IN_AGENT` environment variable in `values.yaml`.
- The maximum Snapshot size depends on how you create or upload it:
    - **Cognigy UI**: You can create and upload Snapshots of up to 8 GB. For on-premises installations, you can increase this limit by setting the `RESUMABLE_UPLOAD_MAX_FILE_SIZE` environment variable in `values.yaml`. The same limit also applies to [Packages](/ai/platform-features/packages).
    - **Cognigy API**: You can create and upload Snapshots of up to 2 GB. Snapshots that exceed this limit can't be created or uploaded. For on-premises installations, you can increase this limit by setting the `SNAPSHOT_MAX_FILE_SIZE` environment variable in `values.yaml`.
...
<Tabs>
  <Tab title="GUI">
    You can create, delete, and [restore](#restore-a-snapshot) Snapshots in **Deploy > Snapshots**. Additionally, you can download and upload Snapshots:
    
    <Accordion title="Download a Snapshot">

      To download a Snapshot, follow these steps:
      
      1. On the **Snapshots** page, select ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg) **> Prepare Download**. A new [task](/ai/agents/develop/projects-and-flows/projects) named **Prepare Download** is created.
      2. Once the task is completed, click ![snapshot-download](/_assets/icons/snapshot-download.svg) to download the Snapshot.
    
    </Accordion>
    <Accordion title="Upload a Snapshot">

      To upload a Snapshot, follow these steps:
      
      1. On the **Snapshots** page, click **Upload Snapshot**.
      2. Select a Snapshot from your computer. When the file starts uploading, you will see a dialog window with a progress bar.
      3. Once the file has been uploaded, a [task](/ai/agents/develop/projects-and-flows/projects) named **Upload Snapshot** is created. After the task is completed, the Snapshot is successfully uploaded.

    </Accordion>
  </Tab>
  <Tab title="API">
    You can view, create, delete, and [restore](#restore-a-snapshot) Snapshots using the [Cognigy API](https://api-trial.cognigy.ai/openapi#get-/v2.0/snapshots) as well as download and upload them.
  </Tab>
  <Tab title="CLI">
...
To deploy a Snapshot, select it from the **Snapshot** list in the [Endpoint settings](/ai/agents/deploy/endpoints/overview).
...
<AccordionGroup>
  <Accordion title="Develop an AI Agent with a Single Project">
  ...
  <Accordion title="Develop an AI Agent with Multiple Projects">
  ...
  <Accordion title="Develop an AI Agent with a CI/CD Environments">
