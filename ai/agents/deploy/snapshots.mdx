...
- A Snapshot can be up to 2 GB. If a Snapshot exceeds this limit, you can't create or upload it. For on-premises installations, you can increase the maximum size by setting the `SNAPSHOT_MAX_FILE_SIZE` environment variable in `values.yaml`.
