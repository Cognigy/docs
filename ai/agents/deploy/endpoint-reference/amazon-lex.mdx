---
title: Amazon Lex
keywords: ['amazon lex', 'amazon lambda', 'amazon lex endpoint', 'aws lex', 'aws lambda']
seo:
  metatags:
    description: >-
      The Amazon Lex Endpoint enables you to connect your AI Agent with Amazon
      Lex and AWS Lambda.
---

<Frame>
  <img src="/_assets/ai/deploy/endpoint-reference/amazon-lex.png" alt="Amazon Lex Endpoint logo"/>
</Frame>
...
<Accordion title="Amazon Lex Settings">
  | Parameter           | Description                                                                                                                                                                                                                               |
  |---------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
  | Reparse Alexa Slots | Allows Slots detected by Amazon Lex to be passed to Cognigy and be processed by the Cognigy NLU model. To make this parameter available, select **Amazon Lex (Built-in)** from the **NLU Connector** list in the Endpoint settings. |
</Accordion>
## How to Set Up

## Setup on the Cognigy Side

<Accordion title="Create an Amazon Lex Endpoint">
  ...
  5. _(Optional)_ In the **Amazon Lex Settings** section, activate the **Reparse Alexa Slots** option only if you want the Cognigy NLU model to process Amazon Lex slots.
</Accordion>
...
<AccordionGroup>
  <Accordion title="Use Amazon Lex with Amazon NLU">
    Use your existing Amazon Lex bots along with their native NLU capabilities to process user input within Cognigy.
    ...
    3. Select **Create function**. The console creates a Lambda function with a single source file named `index.mjs`. Edit this file in the built-in code editor. Copy the following code and paste it into your `index.mjs` file:
        ...
        exports.handler = async (event, context, callback) => {
            ...
            if (resultMap["data"] && resultMap["data"]["connect_action"]) {
                defaultSessionAttributes["action"] = resultMap["data"]["connect_action"];
            }
            if (resultMap["data"] && resultMap["data"]["connect_action_data"]) {
                defaultSessionAttributes["action_data"] = JSON.stringify(resultMap["data"]["connect_action_data"]);
            }
            const response = {
                sessionState: {
                    sessionAttributes: {
                        action: defaultSessionAttributes.action,
                        action_data: JSON.stringify(defaultSessionAttributes.action_data),
                    },
                    dialogAction: {
                        type: "Close",
        ...
        For example, if your Cognigy trial Endpoint URL is `https://endpoint-trial.cognigy.ai/175525264283054b52edb3b9ece270755e46e651ac00015fad9f1175bb47c110`, specify the following values:
    ...
    10. Select the previously created Lambda function with its latest version and save changes.
  </Accordion>

  <Accordion title="Use Amazon Lex with Cognigy NLU">
    Use the Amazon Lex Endpoint to transcribe user input, while Cognigy's NLU model handles intent recognition and slot parsing.
            ...
            try {
                ...
                const response = {
                    sessionState: {
                        sessionAttributes: {
                            action: defaultSessionAttributes.action,
                            action_data: defaultSessionAttributes.action_data,
                        },
                        dialogAction: {
                            type: "Close",
                            fulfillmentState: "Fulfilled",
    ...
    11. Select the previously created Lambda function with its latest version and save changes.
  </Accordion>
</AccordionGroup>
## More Information
...
- [Cognigy Help Center: Amazon Connect: Control Amazon Connect from Cognigy](https://support.cognigy.com/hc/en-us/articles/6229683335196)
