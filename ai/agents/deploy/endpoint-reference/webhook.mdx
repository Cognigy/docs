...
The Endpoint receives `POST` requests at the Cognigy Endpoint URL, processes them with the assigned Flow, and sends results asynchronously to your webhook URL.

```mermaid
flowchart LR
A[External System] -->|POST Request| B[Cognigy Webhook Endpoint]
...
- To receive `POST` requests from Cognigy at your webhook URL, run a web server on your side.
...
<Accordion title="Basic Auth Credentials">
  ...
  Use this section to provide the webhook URL and, optionally, credentials Cognigy uses to authenticate outgoing requests to the webhook URL.
...
<Accordion title="1. Create a Webhook Endpoint">
  ...
  3. _(Optional)_ To set up Endpoint-specific API authentication, select **API Key** from the **Authentication Method** list in the **API Key Authentication** section. The **Endpoint API Keys** section appears. Follow these steps:
      ...
      3. Click the **Key** field to copy the generated API key for later use in the header of requests to the Endpoint URL. Save the API key because you can't retrieve it after closing the dialog. Alternatively, you can create and manage API keys for this Endpoint using the `/v2.0/endpoints/{endpointId}/apikeys` routes via [Cognigy API](https://api-trial.cognigy.ai/openapi#tag--Endpoints).
  4. In the **Basic Auth Credentials** section, enter the external webhook URL in the **Webhook** field. This URL is where Cognigy sends output data.
  5. _(Optional)_ If your webhook uses basic authentication, fill in the **User** and **Password** fields.
  6. Save changes and go to the **Configuration Information** section. For sending `POST` requests to the Cognigy Webhook Endpoint, copy the URL from the **Endpoint URL** field.
</Accordion>
...
<AccordionGroup>
  <Accordion title="1. Send a Request">
    ...
    Send a `POST` request to the Cognigy Webhook Endpoint. Your web server must accept `POST` requests and process the JSON payload sent by Cognigy. For testing purposes, you can use [webhook.site](https://webhook.site) as a temporary web server.
    ...
    <AccordionGroup>
      ...
      <Accordion title="With API Key Authentication">
        ...
        The API key authentication header name depends on your Cognigy version.

        <AccordionGroup>
          <Accordion title="For Cognigy 2026.16.0 and later version">

            Use the `x-cognigy-endpoint-key` header. The Endpoint also accepts the legacy `X-Webhook-Key` header for backward compatibility, but `x-cognigy-endpoint-key` takes precedence if both are provided.
            ...
            <Tabs>
              <Tab title="cURL">
                ...
                curl -X POST https://<your-endpoint-url> \
                  -H "Content-Type: application/json" \
                  -H "x-cognigy-endpoint-key: <your-api-key>" \
                  -d '{
                    "userId": "user123",
                    "sessionId": "session123",
                    "text": "Hello, I need help with my order",
              ...
              <Tab title="Postman">
                ...
                3. Go to the **Headers** tab and configure:
                    ...
                    - `x-cognigy-endpoint-key` — the API key you generated in the [API Key Authentication](#api-key-authentication) section.
                4. Go to the **Body** tab, select **raw**, then select **JSON** as the format.
                5. Paste the request body:
          ...
          <Accordion title="For Cognigy 2026.13 to 2026.15">
            <Tabs>
              <Tab title="cURL">
                Replace `https://<your-endpoint-url>` with the Endpoint URL and `<your-api-key>` with an active API key from the Endpoint settings.
