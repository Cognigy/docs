---
title: Voice Copilot
keywords: ['voice copilot', 'siprec', 'agent copilot', 'voice endpoint', 'real-time transcription']
---

<a href="/release-notes/4.66"><Badge className="version-badge" color="blue">Updated in 4.66</Badge></a>

<Frame>
  <img src="/_assets/ai/deploy/endpoint-reference/voice-copilot.png" alt="Voice Copilot Endpoint logo" />
</Frame>

The Voice Copilot Endpoint handles transcripts from voice calls via webhooks, processes them in real time and sends them to [Agent Copilot](/agent-copilot/overview) Flows. After receiving a transcript, the Endpoint passes the `userId` and `sessionId` variables to Cognigy to identify the Agent Copilot workspace to be updated.

The Voice Copilot Endpoint works with any [handover provider](/ai/escalate/handover-reference/overview) supported by Cognigy. This Endpoint also supports integration with SIP recording (SIPREC). SIPREC is an open SIP-based protocol for call recording in contact centers. Session Border Controllers (SBC) integrate SIPREC with the contact center.

You can [configure](#configure-the-voice-copilot-endpoint) the Voice Copilot Endpoint in the following setups:

- Cognigy in front of a contact center
...
flowchart TD
    ...
    B --> C["Transforms data for<br/>processing in Cognigy"]
...
3. In the **Configuration Information** section, copy the Endpoint URL for later use in the Transfer Node if you use [Cognigy in front](#configure-the-voice-copilot-endpoint) of your contact center.

## Configure the Voice Copilot Endpoint

<Tabs>
  <Tab title="Cognigy in Front">
