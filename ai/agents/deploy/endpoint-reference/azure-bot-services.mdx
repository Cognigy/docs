...
<Accordion title="Azure Bot Services Settings">
  ...
  | SSO Connection Name           | The name of the OAuth connection you create in Azure Bot Services.<br/>The [Get Token](/ai/agents/develop/node-reference/service/get-token), [Token Status](/ai/agents/develop/node-reference/service/token-status), and [Invalidate Token](/ai/agents/develop/node-reference/service/invalidate-token) Nodes use this connection unless you turn on the **Overwrite Connection** toggle.<br/>Depending on your Cognigy.AI version, this connection's token and approval status are accessible from the following fields:<ul><li>Cognigy.AI 2026.16.0 and earlier — `input.data.microsoftSsoToken` and `input.data.microsoftSsoPermissionRequest`.</li><li>Cognigy.AI 2026.17.0 and later — from the `ssoToken` and `permissionRequest` properties in `input.data.microsoftSsoConnections['<connection-name>']`, which allow [multiple SSO connections](#multiple-sso-connections), and from legacy `input.data.microsoftSsoToken` and `input.data.microsoftSsoPermissionRequest`, which allow Flows configured previous to Cognigy.AI 2026.16.0 to keep working.</li></ul> |
</Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="4. Add the Teams Channel to your Azure Bot">
    ...
    <Note>The Teams channel might display errors. You can ignore these errors because each message is properly delivered to and processed by Cognigy.AI.</Note>
  </Accordion>
