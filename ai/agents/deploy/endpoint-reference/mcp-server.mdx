...
<a href="/release-notes/2026.17"><Badge className="version-badge" color="purple">Updated in 2026.17 (experimental)</Badge></a>

<Frame>
  <img src="/_assets/ai/deploy/endpoint-reference/mcp.svg" alt="MCP Server Endpoint logo" />
</Frame>
...
The MCP Server Endpoint allows external AI applications to use tools from an AI Agent in Cognigy via the Model Context Protocol (MCP). Compatible clients, such as desktop AI assistants or custom MCP applications, can discover and execute these tools.

Unlike standard Endpoints, the MCP Server Endpoint isn't designed for conversational use. It doesn't use NLU, process user messages, or perform real-time translation. Instead, it works as a tool server: clients discover the available tools and invoke them using structured requests. Cognigy then executes the corresponding Flow logic and returns the results through the MCP protocol.
...
- **Direct Flow Execution**. When a tool is called, Cognigy executes the related Flow and returns the result through MCP.
...
<AccordionGroup>
  ...
  <Accordion title="MCP Explorer">
    ...
    |---------------|-----------------|
    | MCP Explorer | A link to test your tools and view connection details. It displays all exposed tools, including their names, descriptions, and parameters, and allows validation and testing of the configuration. The MCP Explorer also provides ready-to-use configuration snippets for Claude Desktop and Cursor, with a server key based on the Endpoint name. |
  </Accordion>
...
sequenceDiagram
    participant User
    participant Cognigy as Cognigy Platform
...
<Accordion title="Configure an MCP Server Endpoint">
   ...
   7. Go to the **Authentication** section. From the **Authentication Type** list, select the authentication method for the MCP server:
        ...
        - **OAuth 2.0** — activate token-based authentication using OAuth 2.0. Use this option to verify that only authorized clients can access the MCP server. Configure the following fields:
          ...
          - **Key Refresh Interval (minutes)** — this field is optional. The frequency at which the JWKS public keys are refreshed. Increase this value to reduce the number of requests to your identity provider. Must be a value between `1` and `1440`, for example, `120`. If you leave this parameter empty, the server uses the default interval of 60 minutes.
</Accordion>
    ...
    4. Call tools using the `tools/call` method, providing the tool name and required arguments.
     ...
     <Tab title="Claude Desktop">
      ...
      2. Add the following configuration to `claude_desktop_config.json`:
        ...
        ```
         Replace `{token}` with the token from the MCP Server URL and `my-endpoint-name` with a name for your Endpoint. The MCP Explorer automatically generates a key based on the Endpoint name.
      ...
      <Tab title="Cursor">
        ...
        2. Add the following configuration to the `mcp.json` file:
            ...
            ```
            The `-y` flag lets `npx` run the package without asking for confirmation. Replace `{token}` with the token from the MCP Server URL and `my-endpoint-name` with a name for your Endpoint. The MCP Explorer automatically generates a key based on the Endpoint name.
...
<AccordionGroup>
  ...
  <Accordion title="Connect Custom Applications">
    Custom AI applications, development environments, or third-party systems can integrate with Cognigy
...
| Authentication Failed                                    | The Bearer token is missing, expired, or doesn't include the required scopes.                    | Verify that the token is valid and includes the scopes configured in the **Required Scopes** field. Re-authenticate if the token has expired.                                                                                      |
| The **Run Test** Button in the MCP Explorer isn't Active | A reverse proxy or security layer is changing the Content Security Policy (CSP) headers set by the MCP Explorer page. | The MCP Explorer sets its own CSP headers using nonce-based script allowlisting. Make sure upstream proxies or security tools don't remove or change these headers. Contact your platform administrator if CSP changes are needed. |