---
title: NiCE CXone
keywords: ['nice', 'nice cxone', 'nice cxone endpoint']
seo:
  metatags:
    description: >-
...
<Frame>
  <img src="/_assets/ai/deploy/endpoint-reference/nice.svg" alt="NiCE CXone Endpoint logo" />
  <img src="/_assets/ai/deploy/endpoint-reference/nice-dark.svg" alt="NiCE CXone Endpoint logo" />
</Frame>
...
<Accordion title="NiCE CXone Setup">
  ...
  |--------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
  | Verify Token | A verification token for connecting the NiCE CXone Endpoint to the CXone Virtual Agent Hub. This token is generated on the Cognigy side and unique for each Endpoint. |
</Accordion>
## How to Set Up
...
<Accordion title="Create an App in NiCE CXone">
  ...
  9. At **Step 5: Test + Add**, test the bot by sending a welcome message to the chat. If you receive a response from the Cognigy Agent, the connection is established correctly.
  ...
  through which end users will communicate with the Cognigy Agent.
