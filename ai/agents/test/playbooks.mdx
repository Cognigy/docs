...
keywords: ['playbooks', 'testing', 'flow testing', 'assertions', 'test automation', 'conversation scripts']
seo:
  metatags:
    description: >-
      Cognigy Playbooks are conversation scripts that you can run
      automatically to test your Flows.
---
...
- [Assertions](/ai/agents/test/assertions) — check the AI Agent's responses and different aspects of the Flow for each Step against a defined value. You can also invert the Assertion check, which means the Assertion checks if the defined value is missing.

<Accordion title="Assertion Types">
...
- Playbooks don't allow dots `(.)` in JSON keys. Referencing subkeys like `user.name` can cause runs to be hidden or [Snapshots](/ai/agents/deploy/snapshots) to fail. Instead, validate the full JSON object:

   <CodeGroup>
    ```json Valid Example title
    {
       "user": {
           "name": "John"
       }
...
<Tabs>
  <Tab title="GUI">
        ...
        In **Test > Playbooks**, you can create, delete, and run Playbooks as well as export them as a Package. To define Steps and Assertions in the Playbook editor, select a Playbook from the list. Playbook runs are [Tasks](/ai/agents/develop/projects-and-flows/projects#task-panel) and take place in the background. The results are available in the Playbook editor.

        Alternatively, you can also run a Playbook and view the results from the [Interaction Panel](/ai/agents/test/interaction-panel/overview), as well as create Playbooks from a conversation with the AI Agent.
        ...
        **Interaction Panel**
        ...
        To run a Playbook in the Interaction Panel, select and run a Playbook from the ![playbook](/_assets/icons/black-and-white/playbooks-interaction-panel.svg) tab. You can hover over the Assertion checks for more details.

        To create Playbooks from a conversation, follow these steps:

        1. Chat with the AI Agent on the ![chat](/_assets/icons/black-and-white/chat-interaction-panel.svg) tab.
        2. Click ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg) above the chat window and select **Create Playbook**.

        You are redirected to the Playbook editor. You can then add Assertions and additional Steps.
  </Tab>
  <Tab title="CLI">
        You can run Playbooks using the [Cognigy CLI](https://github.com/Cognigy/Cognigy-CLI?tab=readme-ov-file#command-run). Schedule multiple Playbook runs simultaneously using the `run` command.
  </Tab>
  <Tab title="API">
        You can create, edit, and delete Playbooks using the [Cognigy API](https://api-trial.cognigy.ai/openapi#tag--Playbooks-v2.0) as well as run Playbooks.
  </Tab>
</Tabs>
