...
In Cognigy, _Analytics Steps_ are progress markers that help you monitor the Flow paths users take during interactions with AI Agents. These steps are recorded in the Analytics database and can be analyzed in [Insights](/ai/agents/analyze/insights) or retrieved through the [OData Endpoint](/ai/agents/analyze/odata).
