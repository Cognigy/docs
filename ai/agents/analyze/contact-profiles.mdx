...
- A [simulated Contact Profile](/ai/agents/test/simulator) is removed after 5 days if not read or modified. If you have a dedicated SaaS installation, you can configure an expiration date for simulated Contact Profiles via the `SIMULATION_PROFILE_TTL_IN_MINUTES` variable. Contact [Cognigy technical support](/help/get-help) to apply this configuration.
...
<Tabs>
  <Tab title="GUI">
    ...
    You can also use the **Simulation Profiles** option to view only personas created in the [simulation scenarios](/ai/agents/test/simulator).
