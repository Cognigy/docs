...
- A Contact Profile is removed after 30 days if not read or modified. If you have a dedicated SaaS or on-premises installation, you can configure an expiration date for Contact Profiles in the [Management UI](/ai/administer/access/management-ui).
...
- A [simulated Contact Profile](/ai/agents/test/simulator/overview) is removed after 5 days if not read or modified. If you have a dedicated SaaS installation, you can configure an expiration date for simulated Contact Profiles via the `SIMULATION_PROFILE_TTL_IN_MINUTES` variable. Contact [Cognigy technical support](/help/get-help) to apply this configuration.
...
<Tabs>
  <Tab title="GUI">
    ...
    The **Contact Profile** page displays a list of conversation transcripts, with the stored Contact Profile data available in the right pane. To view more details about each session, select a session from the list. You are redirected to the [Transcript explorer](/insights/explorers/transcript) in Insights.
