---
title: Cognigy OData Analytics Endpoint
sidebarTitle: OData
description: >-
  Cognigy provides an OData analytics Endpoint for retrieving raw data via
  GET requests. It supports enterprise analytics needs, enabling detailed
  queries for spreadsheets and dashboard creation with preferred data
  visualization tools.
keywords: ['odata', 'analytics', 'metrics', 'tracking', 'data collections']
---
...
Cognigy offers an _OData Analytics endpoint_ to retrieve raw data using `GET` requests.
...
<AccordionGroup>
  ...
  <Accordion title="Request Limits">
    ...
    The following settings control how the OData endpoint limits request rate, concurrency, and bursts.
    These settings apply to both SaaS and on-premises installations. For on-premises installations, you can change the configuration values in the `values.yaml` file to adjust the request limits.

    | Limit                              | Description                                                                                                                                   |              Default | Configuration variable                          |
    | ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | -------------------: | ----------------------------------------------- |
    | **All request limits**             | Controls whether OData request limits are enforced. `true` enforces all limits; `false` bypasses the rate limiter.                            |               `true` | `ODATA_RATE_LIMIT_ENABLED`                      |
    | **Burst capacity**                 | The maximum number of requests that can be sent in a burst before the request rate is limited.                                                |        `50` requests | `ODATA_RATE_LIMIT_BUCKET_CAPACITY`              |
    | **Request rate**                   | The maximum sustained request rate after the burst capacity is exhausted.                                                                     | `10` requests/second | `ODATA_RATE_LIMIT_REFILL_RATE`                  |
    | **Concurrent requests**            | The maximum number of requests that can run at the same time.                                                                                 |         `4` requests | `ODATA_RATE_LIMIT_MAX_IN_FLIGHT`                |
    | **Request slot lease**             | The amount of time a request slot remains allocated after a server crashes during a request before the slot is released.                      |           `60000` ms | `ODATA_RATE_LIMIT_LEASE_TTL_MS`                 |
    | **Request heartbeat**              | The interval at which a long-running request confirms that it is still active.                                                                |          `~20000` ms | `ODATA_RATE_LIMIT_LEASE_HEARTBEAT_MS`           |
    | **Concurrent request retry delay** | The number of seconds returned in the `Retry-After` header when a request is rejected because the concurrent request limit has been exceeded. |          `5` seconds | `ODATA_RATE_LIMIT_INFLIGHT_RETRY_AFTER_SECONDS` |

    The OData endpoint applies request limits to each request budget. A request budget determines which requests share the same limits.

    There are two request budgets:

    - **Project request budget** — applies to requests that filter by a single Project, for example, `$filter=projectId eq '<project-id>'`, or to requests that don't filter by Project but use an API key that has access to only one Project. 
    - **Shared organization request budget** — applies to requests that are unfiltered across multiple Projects or that use an API key that has access to more than one Project.

    Two independent limits apply to each request budget: 

    - **Request rate** — limits how many requests can be sent per second. You can send a batch of requests all at once, after which the requests-per-second limit applies.
    - **Concurrent requests** — limit how many requests can run at the same time. This limit affects queries that remain open for a while, such as queries without `$top` over a wide time range.

    Requests to `https://odata-trial.cognigy.ai/v2.4/$metadata` are excluded from these limits. 

    When a limit is exceeded, the OData endpoint returns an HTTP `429 Too Many Requests` response with a `Retry-After` header, in seconds. The `message` field tells you which limit was exceeded and why:

    | Limit exceeded | Message | Why | 
    |---|---|---| 
    | Request rate, Project request budget | `Rate limit exceeded for this project. Please retry later.` | Too many requests were sent against one Project's request budget in a short time. |
    | Concurrent requests, Project request budget | `Too many concurrent requests for this project. Please retry later.` | Too many requests for one Project are in progress at the same time, often caused by long-running or streaming queries. |
    | Request rate, Shared organization request budget | `Rate limit exceeded for cross-project requests in this organisation. Please retry later. Pinning one project with $filter=projectId eq '<id>' gives that project its own budget.` | Too many unfiltered or multi-project requests were sent against the organization's shared request budget. |
    | Concurrent requests, Shared organization request budget | `Too many concurrent requests for cross-project requests in this organisation. Please retry later. Pinning one project with $filter=projectId eq '<id>' gives that project its own budget.` | Too many unfiltered or multi-project requests for the organization's shared request budget are in progress at the same time. |

     <Tip> If you receive a `429` response, wait for the number of seconds specified in the `Retry-After` header before retrying. If you send a high volume of requests to a single Project, filter by that Project's `projectId` to use the Project's request budget instead of the organization-wide request budget. </Tip>

     If the rate limiter is temporarily unavailable, requests are allowed through rather than rejected. This means that a limiter outage never blocks access to your data.
...
- The Cognigy.AI OData endpoint doesn't support duplicate query options. Each query option, for example, `$filter`, may be set only once in the query string.

### Query Complexity Limits

To protect service stability, the OData endpoint enforces the following limits on query complexity. Requests that exceed any of these limits are rejected before any database work occurs. This limitation prevents a single oversized query from disrupting the service for other users.

| Query Option | Limit |
|---|---|
| `$filter`, `$search` | <ul><li>Length: 8,192 characters maximum.</li><li>Logical operators: 100 maximum. Applies to the number of `and`, `or`, or `not` operators. Operators inside string literals aren't counted.</li><li>Nesting depth: 40 maximum.</li></ul> |
| `$select`, `$orderby` | 100 comma-separated items maximum. |

To change these limits, do the following:

-  For SaaS installations, contact [Cognigy technical support](/help/get-help).
-  For on-premises installations, adjust the limits in `values.yaml`:
    - `ODATA_MAX_FILTER_LENGTH` — maximum length in characters in `$filter` and `$search`.
    - `ODATA_MAX_FILTER_LOGICAL_OPERATORS` — maximum `and`/`or`/`not` operators in `$filter` and `$search`.
    - `ODATA_MAX_FILTER_PAREN_DEPTH` — maximum parenthesis nesting depth in `$filter` and `$search`.
    - `ODATA_MAX_QUERY_OPTION_ITEMS` — maximum items in `$select` and `$orderby`.

<Tip>
If your query exceeds the logical operator or item limit, split it into multiple smaller requests. For example, instead of a single `$filter` with 150 `or` operators, issue several requests with up to 100 operators each and merge the results client-side.
</Tip>
- The Cognigy OData endpoint supports only the [OData protocol version 4](https://www.odata.org/documentation/).
- The Cognigy OData endpoint only supports `GET` requests and doesn't support any other request types, such as `PATCH`, `DELETE`, or `POST`.
- The data retrieved through the Cognigy OData endpoint isn't updated in real time and may not reflect the current state.
- The Cognigy OData endpoint supports a limited set of system query options, operators, and query functions. For detailed information about the query syntax, refer to the [OData Query Syntax](#odata-query-syntax) section.

## Supported Versions

| Cognigy version | Cognigy OData endpoint version | Status                                   |
|--------------------|-----------------------------------|------------------------------------------|
...
You can control the data available through the OData endpoint at the [Project](/ai/administer/access/project-settings), [Endpoint](/ai/agents/deploy/endpoints/overview), and Node levels:

- Project — use [Data Redaction](/ai/platform-features/data-redaction) to detect and redact data, including personally identifiable information (PII), from the OData endpoint.
- Endpoint — go to the [Data Protection & Analytics](/ai/agents/deploy/endpoints/data-protection-and-analytics) section in the Endpoint settings:
...
- Node — use the **Mask and keep analytics data** parameter in the [Delete Profile](/ai/agents/develop/node-reference/analytics/delete-profile) Node and the **Keep analytics records, mask personal data** option in the [Deactivate Profile](/ai/agents/develop/node-reference/analytics/deactivate-profile) Node. These Nodes can mask personal data while including analytics data across the Analytics, Conversations, and Sessions collections. By default, rows with masked personal data in these collections are included in OData results. To exclude rows where personal data has been masked:
  - For SaaS installations, contact [Cognigy technical support](/help/get-help).
  - For on-premises installations, set `ODATA_FILTER_MASKED_ROWS=true` in `values.yaml`.

## OData Endpoint Access

You can access the OData endpoint through a different domain than the one from Cognigy:
...
<Tabs>
  <Tab title="Cognigy">
    ...
    - Dedicated SaaS — contact your Customer Success Manager or Cognigy Support to get the OData endpoint URL.
...
- The Cognigy OData base domain. It is the root URL where your OData service is hosted.
- The API version of the Cognigy OData endpoint. Currently, Cognigy supports OData 2.3 and 2.4.
...
- Cognigy OData base domain — `odata-trial.cognigy.ai`
- Cognigy OData endpoint version — `v2.4`
...
They apply to [all Cognigy OData endpoint Collections](#cognigy-ai-odata-collections) and must be followed when creating query expressions.
...
The Cognigy OData endpoint supports a limited set of [system query options](https://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptions):
...
<AccordionGroup>
    ...
    <Warning>
      The `not` logical operator isn't supported in the Cognigy OData endpoint. Expressions such as `not startswith(field, 'value')`, `not contains(field, 'value')`, or `startswith(field, 'value') eq false` won't work.
   </Warning>
...
Select the OData endpoint version [compatible with your Cognigy version](#supported-versions).
...
<Tabs>
  <Tab title="OData 2.4">
    ...
    | Returns all Analytics records for a specific Cognigy Project filtered by `projectId`.                                                                                                                  | `https://odata-trial.cognigy.ai/v2.4/Analytics/?$filter=projectId eq '<project-id>'&apikey=<your-api-key>`                                                         |
    | Returns Project and session IDs for a specific Cognigy project. The request filters the data based on the `projectId` field and selects only the `projectId` and `sessionId` for each matching entry. | `https://odata-trial.cognigy.ai/v2.4/Analytics/?$filter=projectId eq '<project-id>'&$select=projectId,sessionId&apikey=<your-api-key>`                             |
  ...
  <Tab title="OData 2.3">
    ...
    | Returns all Analytics records for a specific Cognigy Project filtered by `projectId`                                                                                                                      | `https://odata-trial.cognigy.ai/v2.3/Analytics/?$filter=projectId eq '<project-id>'&apikey=<your-api-key>`                                                         |
    | Returns Project IDs and session IDs for a specific Cognigy project. The request filters the data based on the `projectId` field and selects only the `projectId` and `sessionId` for each matching entry. | `https://odata-trial.cognigy.ai/v2.3/Analytics/?$filter=projectId eq '<project-id>'&$select=projectId,sessionId&apikey=<your-api-key>`                             |
...
## Cognigy OData Collections

The OData endpoint provides access to several data _collections_ containing information about your interactions and workflows. A _collection_ is similar to a database table or a set of entities containing data.
...
The Analytics collection offers a log of comprehensive details such as organizational information, Project identifiers, Flow specifics, and essential performance metrics. Each chat or voice AI Agent session in Cognigy creates an Analytics record for an interaction. Each interaction is logged as a single record. The information is saved only after the end of the session. You can change this data using the [Overwrite Analytics Node](/ai/agents/develop/node-reference/analytics/overwrite-analytics).

**Requests**

Select an API request to extract data from the collection. The API version in the request must match [your Cognigy version](#supported-versions).
...
<Accordion title="Response Fields">
  ...
  | isMasked           | This field is available only for OData 2.4. A flag indicating whether the record has personally identifiable information (PII) has been masked using mask and keep analytics options from either the [Deactivate Profile](/ai/agents/develop/node-reference/analytics/deactivate-profile) or [Delete Profile](/ai/agents/develop/node-reference/analytics/delete-profile) Node. | Boolean (nullable) | `true`                                                           |

  <Note>
    The Analytics step label can be updated from the Cognigy UI at any time.
...
Each time one of these sources sends a message to a Flow, Cognigy creates a record to log the interaction.
...
<Accordion title="Response Fields">
  ...
  | id                | The unique ID of the record. Applicable to the OData endpoint 2.4 or later.     | String | 5a91d194fde28b0011ce2423         |
  | _id               | The unique ID of the record. Applicable to the OData endpoint 2.3 or earlier.   | String | 5a91d194fde28b0011ce2423         |
  | label             | The Analytics step label defined for the entity (Node or Intent) in Cognigy. | String | Question (2)                     |
...
<Accordion title="Response Fields">
  ...
  | agentName         | The name of the human agent assigned to the conversation. This field is supported from Cognigy 4.80.                                       | String   | John Doe                                                         |
...
<Accordion title="Response Fields">
  ...
  | snapshotId        | The ID of the Snapshot.                                                                                                                                      | String     | 63ff0cc47a466cab278fd19b                                         |
  | snapshotName      | The name for the Snapshot used by the Endpoint.                                                                                                              | String     | Bot Release 2.2                                                  |
  | isMasked            | A flag indicating whether the record has personally identifiable information (PII) has been masked using mask and keep analytics options from either the [Deactivate Profile](/ai/agents/develop/node-reference/analytics/deactivate-profile) or [Delete Profile](/ai/agents/develop/node-reference/analytics/delete-profile) Node. | Boolean (nullable) |
</Accordion>

## Cognigy OData Collections: Entity Relationship Diagram (ERD)

The Entity Relationship Diagram (ERD) describes the relationships between the collection entities.

```mermaid
erDiagram
    %% ERD diagram for Cognigy OData v2.4 collections 
    %% Layout: Top-down data flow — Sessions → ExecutedSteps → Goals

    %% ========== Session Layer ==========
    Sessions ||--o{ Conversations         : "logs messages"
    Sessions ||--o{ ExecutedSteps         : "executes"
    Sessions ||--o{ Analytics             : "summarized by"
    Sessions ||--o{ LiveAgentEscalations  : "escalates via"
    Sessions ||--o{ GoalEvents            : "tracks goals via"
    ...
    Analytics {
        string id PK
        string sessionId FK
        string flowReferenceId
        string organisation
        string projectId
        string intent
        string completedGoalsList
        datetime timestamp
    ...
    %% ========== Execution Layer ==========
    Steps         ||--o{ ExecutedSteps : "executed as"
    ExecutedSteps ||--o{ ExecutedSteps : "parentStep (self)"

    ExecutedSteps {
        string id PK
        string sessionId FK
        string parentStep FK
        string entityReferenceId FK
        string flowReferenceId
        string stepLabel
        datetime timestamp
        string projectId
    ...
    %% ========== Goal Layer ==========
    Goals     ||--o{ GoalSteps       : "has"
    Goals     ||--o{ GoalStepMetrics : "measured by"
    Goals     ||--o{ GoalEvents      : "tracked by"
    GoalSteps ||--o{ GoalStepMetrics : "measured by"
    ...
    GoalStepMetrics {
        string goalStepMetricId PK
        string goalId FK
        string goalStepId FK
        string name
        string type
        int value
        string projectId
    ...
    Legend
    Legend {
        string info "One-to-Many: ||--o{ (1..*)"
        string info2 "One-to-One: O--O (1..1)"
        string info3 "Many-to-Many: }o--o{ (*..*)"
        string info4 "Optional-to-One: |o--|| (0..1)"
        string info5 "Strong One-to-One: ||--||"
        string abbr "PK=Primary Key, FK=Foreign Key, UK=Unique Key"
    }
    classDef legend fill:#fefce8,stroke:#facc15,color:#444
    class Legend legend
```

## Integrations

The Cognigy OData endpoint can integrate with all tools compatible with the OData protocol version 4.
...
| Microsoft Power BI                                                  | Microsoft Power BI is a business analytics service that provides interactive visualizations and business intelligence capabilities. For integrating with the Cognigy OData endpoint, refer to the [Microsoft Power BI](https://docs.microsoft.com/en-us/power-bi/desktop-connect-odata) documentation. |
