---
title: Data Expiration in Cognigy
sidebarTitle: Data Expiration
keywords: ['data expiration', 'data retention', 'ttl', 'time to live']
seo:
  metatags:
    description: >-
      In Cognigy, data retention for Analytics data is controlled by Time to
      Live (TTL) settings. TTL defines how long data is stored before automatic
      deletion. This article outlines the expiration logic and provides examples
      of how different TTL settings affect data retention.
---

In Cognigy, data retention for Analytics data is controlled by time-to-live (TTL) settings. TTL defines how long data is stored before automatic deletion. 
...
Cognigy uses the smallest positive TTL value from the following sources:
...
<AccordionGroup>
  <Accordion title="List of Environment Variables">
...
Conversation data in Cognigy expires according to the following data expiration logic:
...
### TTL for Simulator Results

Simulation results have a fixed retention period that isn't configurable through the Management UI or environment variables.

| Data               | Retention Period | Configurable |
|--------------------|-------------------|--------------|
| Simulation Results | 90 days           | No           |

For more information, read [Simulator Limitations](/ai/agents/test/simulator/overview#limitations).

## More Information

- [Quotas and Limits](/ai/administer/limitations)
- [Data Expiration in Insights](/insights/data-management/data-expiration)
