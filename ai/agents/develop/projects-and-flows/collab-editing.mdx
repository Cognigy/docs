...
keywords: ['collaborative editing', 'flow editor', 'nodes', 'node editor']
seo:
  metatags:
    description: >-
      Collaborative editing allows multiple users to edit a Flow simultaneously.
      With Collaborative Editing, you can see the avatars of other users editing
      the Flow and Nodes. Their changes appear in the Flow and Node editors in
      real time.
---
...
<div style={{textAlign: 'center'}}>
   <video width="650" height="400" controls autoPlay title="Collaborative editing function shown as multiple users editing a Flow simultaneously" aria-label="Collaborative editing function shown as multiple users editing a Flow simultaneously" style={{border: '1px solid #ccc', borderRadius: '12px', display: 'block', margin: '0 auto'}}>
...
- When another user makes changes, the Node editor is locked for all other users and displays ![locked](/_assets/icons/node-lock.svg).
- When the changes are saved, the Node editor displays the ![refresh](/_assets/icons/blue/refresh.svg) button. Then, you can:
  - Click the ![refresh](/_assets/icons/blue/refresh.svg) button to refresh the Node editor and view the latest changes.
  - Select ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg) **> Edit** to unlock and keep editing the Node. The changes you save after unlocking the Node overwrite the changes other users previously made.
