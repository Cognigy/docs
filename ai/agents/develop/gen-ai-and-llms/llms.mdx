...
Similar to the standard model, you must enter the API credentials from the [LLM provider](/ai/agents/develop/gen-ai-and-llms/providers/all-providers) that offers this model to get access to its functionality.
...
As an alternative to the [default retry mechanism](#retry-mechanism) that is applicable to all models, you can override it for a specific model by using an [LLM fallback](/ai/agents/develop/gen-ai-and-llms/fallback).
