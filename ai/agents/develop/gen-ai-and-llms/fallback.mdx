...
seo:
  metatags:
    description: >-
      LLM Fallback is a mechanism designed to maintain continuous service when a
      main model fails to respond or encounters an error.
---

import AiBeta from '/snippets/ai/beta.mdx'

<a href="/release-notes/4.90"><Badge color="purple">Added in 4.90 (Beta)</Badge></a>
...
minimizing disruptions for AI Agents during [design](/ai/agents/develop/gen-ai-and-llms/generative-ai#design-time-generative-ai-features) and [run-time](/ai/agents/develop/gen-ai-and-llms/generative-ai#run-time-generative-ai-features) operations.
...
Using a fallback model complements the [default retry mechanism](/ai/agents/develop/gen-ai-and-llms/llms#retry-mechanism).
If the main model fails to respond, the system first uses the default retry mechanism. If all retry attempts are exhausted, it then switches to the fallback model.
...
If your Project has only one model, you can add another by choosing one [from the same LLM provider or another provider](/ai/agents/develop/gen-ai-and-llms/providers/all-providers) with similar capabilities.
...
- [Other Operations](/ai/agents/develop/gen-ai-and-llms/other-operations)
