...
- A generative model that supports [LLM Prompt Nodes or Answer Extraction](/ai/agents/develop/gen-ai-and-llms/model-support-by-feature) to use Knowledge Search results to generate context-aware response to user questions.

Once a model is set up within a [Project](/ai/agents/develop/projects-and-flows/projects), all other [Knowledge Stores](/ai/agents/develop/knowledge-ai/knowledge-store) will use the same model. If you want to switch to a different model, you must either delete all existing Knowledge Stores in the current Project or create a new Project.
