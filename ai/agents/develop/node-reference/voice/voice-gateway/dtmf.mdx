---
title: DTMF
keywords: ['voice gateway', 'dtmf']
seo:
  metatags:
    description: >-
      A Voice Gateway DTMF Node sends RTP payloads to the caller's SIP provider
      to play DTMF digit tones to the caller. If the caller's SIP provider
      supports RTP payloads according to [RFC
      2833](https://datatracker.ietf.org/doc/html/rfc2833), the customer will
      hear the voice tones of the DTMF digits. The Node can also be used in
      scenarios where it confirms digit entries by playing back the
      corresponding DTMF tones.
---

<Frame>
  <img class="image-center" src="/_assets/ai/develop/node-reference/vg/dtmf.png" alt="Voice Gateway DTMF Node configuration panel" style={{ width: 'auto' }} />
</Frame>
...
This Node is useful when paired with a [Handover to Human Agent](/ai/agents/develop/node-reference/service/handover-to-agent) Node or a [Transfer](/ai/agents/develop/node-reference/voice/voice-gateway/transfer) Node.
...
(the **Capture DTMF Signals** parameter in the [Set Session Config](/ai/agents/develop/node-reference/voice/voice-gateway/set-session-config) Node) to provide feedback on the caller's input.
...
<Accordion title="Example DTMF Flow">
  ...
  6. Configure the logic in the branch for the second Case Node:
      ...
      4. Add a Handover to Human Agent Node to route the caller to the contact center.

      <Frame>
        <img class="image-center" src="/_assets/ai/develop/node-reference/vg/dtmf-example.png" alt="DTMF example Flow with Question, Lookup, and Handover Nodes" style={{ width: 'auto' }} />
      </Frame>

  Test your Flow via the Interaction Panel. Use the [Voice Call](/ai/agents/test/interaction-panel/voice-call) setting.
</Accordion>

## More Information

- [DTMF Parameter](/ai/agents/develop/node-reference/voice/voice-gateway/parameter-details)
- [DTMF Verb](/voice-gateway/references/verbs/dtmf)
