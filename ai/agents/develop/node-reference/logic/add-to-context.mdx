...
This Node stores information in the [Context](/ai/agents/develop/ai-agent-memory/context) object, so that it can be used throughout the session on subsequent user inputs. 
...
<AccordionGroup>
  ...
  <Accordion title="Adding a Value to an Array Only Contains Unique Values">
