---
title: Lookup
keywords: ['lookup node', 'switch case', 'input matching', 'flow branching']
---
...
<Frame>
  <img
    ...
    alt="Lookup Node" style={{ width: 'auto' }} />
</Frame>
...
<Accordion title="Text">
  ...
  <Accordion title="State">
    By changing the lookup type to State, the Flow selects a case that matches the value of `input.state`. The case values must manually be written for this type of lookup.

    States allow limitations to be placed on a conversation that restrict the valid Intents that the NLU has access to. You can use [Intent Conditions](/ai/platform-features/nlu/intents/conditions) as an alternative for controlling Intent recognition.
