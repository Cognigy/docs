...
slug: "go-to"
hidden: false
keywords: [' Cognigy.AI', ' Nodes', ' Go To']
---

<Frame>
  <img class="image-center" src="/_assets/ai/develop/node-reference/logic/go-to.png" style={{ width: 'auto' }} />
</Frame>
...
<Accordion title="Using Go To and Execute Together">
...
| Select Node | Select the target Node that is switched to whenever the Flow execution triggers the **Go To** Node. To manually enter the Node ID, click **Manual Input** and enter the Node ID. <br/><br/>A Node must be set as an Entrypoint before it can be selected. To set any Node as an Entrypoint, use the [Set Entrypoint](/ai/agents/develop/nodes/overview#set-node-as-entrypoint) parameter. |
