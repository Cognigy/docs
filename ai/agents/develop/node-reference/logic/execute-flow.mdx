...
slug: "execute-flow"
hidden: false
keywords: [" Cognigy.AI", " Nodes", " Execute Flow"]
---
...
| Select Node | A Node must be set as an Entrypoint before it can be selected. To set any Node as an Entrypoint, use the [Set Entrypoint](/ai/agents/develop/nodes/overview#set-node-as-entrypoint#set-node-as-entrypoint) setting. Select the target Node that executes whenever the Flow execution triggers the **Execute Flow** Node. |
