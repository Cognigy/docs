---
title: "AI Agent"
description: "Configure the AI Agent Node to assign an AI Agent to a job, provide job guidance, and control access to retrievable knowledge and tools."
keywords:
  [
    " ai agent node",
    " agentic ai",
...
import ToolsParameterLimitation from '/snippets/ai/tools-parameter-limitation.mdx';
...
<Frame>
  <img
    ...
    alt="AI Agent Node" style={{ width: 'auto' }} />
</Frame>

## Description

<Warning>
As of Cognigy 2026.18.0, the **Maximum Completion Tokens** and **Temperature** parameters are deprecated. The removal is planned for April 2027. For models that don't support these parameters, activate the **Ignore deprecated parameters** toggle to exclude them from the request to the LLM provider. Otherwise, the request to the LLM provider might fail. If you plan to use these parameters, migrate them to the [Custom Options section](#custom-options) and activate the **Ignore deprecated parameters** toggle. Otherwise, Custom Options might not override them for every provider and model combination.

For example, OpenAI models such as `gpt-5`, `gpt-5.1`, and `gpt-5.2` don't support these parameters. Similar models from other LLM providers might also not support these parameters. Check your LLM provider's documentation for model-specific support.
</Warning>
...
- By default, the number of transcript turns is set to 50. In the AI Agent Node, this number is fixed and can't be adjusted, unlike in the [LLM Prompt](/ai/agents/develop/node-reference/service/llm-prompt#advanced) and [Get Transcript](https://docs.cognigy.com/ai/agents/develop/node-reference/service/get-transcript#parameters) Nodes, where customization is possible.
<ToolsParameterLimitation />
...
<AccordionGroup>
  ...
  <Accordion title="Grounding Knowledge">
    ...
    | Top K                       | Slider         | Appears when you select **When Required** or **Once for Each User Input**. Specify how many knowledge chunks to return. The default value is 3, and the maximum value is 5. Providing more results gives the AI Agent additional context but may increase noise and token usage. |
    ...
    | Generate Search Prompt      | Toggle         | Appears when you select **Once for Each User Input**. Enabled by default. Generates a context-aware search prompt before executing the knowledge search. May increase cost and latency. By default, the prompt includes the last 7 conversation turns. This value can't be changed. |
  ...
  <Accordion title="Tool Settings">
      ...
      | Parameter      | Type     | Description |
      |----------------|----------|-------------|
      | Tool Choice | Selector | Determines how the AI Agent selects tools:<ul><li>**Auto** — the model decides whether to call a tool or return a plain-text response. Use this option for general-purpose AI Agents. For example, a support AI Agent that answers FAQs directly but calls a Knowledge tool when the question needs it.</li><li>**Required** — the model must call one of the provided tools on every turn and won't return a plain-text response. Use this option when the Flow must always pass through a structured tool call. For example, when the tool's output must be retrieved and included in the response before anything is returned to the user. Selecting this option can cause an infinite tool-calling loop. See [Avoiding infinite loops when Tool Choice is Required](#avoiding-infinite-loops-when-tool-choice-is-required) for mitigation options.</li><li>**None** — the model ignores all configured tools and returns only a plain-text response. Use this option to temporarily disable tool use without removing the tool configuration, for example, during testing or debugging.</li></ul> <br></br> The options in **Tool Choice** map to the following LLM function-calling modes: <ul><li>**Auto** — `mode: AUTO`</li><li>**Required** — `mode: ANY`</li><li>**None** — `mode: NONE`</li></ul>Availability of these modes depends on your LLM provider. Refer to your provider's documentation to confirm support. |
  ...
  <Accordion title="Advanced">
    ...
    | Parameter               | Type     | Description |
    |-------------------------|----------|-------------|
    | Ignore deprecated parameters | Toggle | Excludes the deprecated **Maximum Completion Tokens** and **Temperature** parameters from requests to the LLM provider. Activate this toggle if your model doesn't support these parameters. If you activate this toggle for a model that supports them, the model uses default values. |
  ...
  <Accordion title="Custom Options">
    ...
    **Forcing Model Versions**
    ...
    You can force the AI Agent Node to use a specific model version by including it in the Custom Options.
    This means that the AI Agent Node will use the specified version of the language model instead of the default or any other available versions. This allows for more control over the behavior of the AI Agent Node, ensuring it uses the desired model version for generating prompts or responses.
    ...
    You can force the AI Agent Node to use the model version `claude-opus-4-1-20250805`, despite the LLM resource defaulting to the `claude-opus-4-5-20251101` model:
    ...
    The AI Agent Node now utilizes the `claude-opus-4-1-20250805` model.
  ...
  <Accordion title="Debug Settings">
    ...
    | Show Token Count              | Toggle        | Send a debug message containing the input, output, and total token count. The message appears in the Interaction Panel when debug mode is enabled. Cognigy uses the GPT-3 tokenizer algorithm, so actual token usage may vary depending on the model used. The parameter is inactive by default.                                                                                                     |
...
When **Tool Choice** is set to **Required**, the model must call a tool on every turn. After a tool branch executes and a [Resolve Tool Action](/ai/agents/develop/node-reference/ai/resolve-tool-action) Node returns the result to the AI Agent Node, the model still can't respond with plain text. Instead, it must call another tool. If every tool branch loops back through a **Resolve Tool Action** Node, the AI Agent keeps calling tools without ever producing a response. This loop continues until the Flow times out and can consume a large number of tokens.
...
<AccordionGroup>
  <Accordion title="Option 1: Change Tool Choice to Auto">
  ...
  <Accordion title="Option 2: Add an Exit Tool">
  
  Keep **Tool Choice** set to **Required** and add a dedicated tool that the model can call to end the loop. This tool branch must not include a Resolve Tool Action Node, so execution doesn't return to the AI Agent Node. Instead, the branch must end with a Say Node or another output Node that produces the final response.
  
  Example exit tool configuration:
  
  | Field       | Value                                                                                                          |
  | ----------- | -------------------------------------------------------------------------------------------------------------- |
  | Tool ID     | `finish_response`                                                                                              |
  | Description | `Call this tool when you have gathered all the information needed and are ready to respond to the user.`       |
  
  The resulting Flow structure looks like this:
  
  ```text
  AI Agent Node (Tool Choice: Required)
  ├── search_knowledge → Resolve Tool Action → [returns to AI Agent Node]
  └── finish_response  → Say Node            → [end of Flow, no Resolve Tool Action]
  ```
...
You can retrieve all job configurations and associated tools for a specific AI Agent via the Cognigy API [`GET /v2.0/aiagents/{aiAgentId}/jobs`](https://api-trial.cognigy.ai/openapi#get-/v2.0/aiagents/-aiAgentId-/jobs) request. The response includes each job's configuration details and a list of available tools.
