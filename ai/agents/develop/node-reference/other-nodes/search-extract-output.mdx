...
<Frame>
  <img class="image-center" src="/_assets/ai/develop/node-reference/other/search-extract-output.png" alt="Search Extract Output Node configuration panel" style={{ width: 'auto' }} />
</Frame>
...
<AccordionGroup>
  ...
  <Accordion title="Custom Options">
    ...
    | Custom Request Options | JSON | Set parameters to customize the request to the LLM provider. For example, you can set parameters related to `timeout`, `retries`, or `headers`. For more information, refer to the LLM provider's API documentation. | `{ "timeout": 5000 }`, `{ "headers": { "Authorization": "Bearer <token>" } }`                                           |
