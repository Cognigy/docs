...
<Frame>
  <img src="/_assets/ai/develop/node-reference/basic/question.png" alt="Question Node configuration panel" style={{ width: 'auto' }} />
</Frame>

## Description

<Warning>
As of Cognigy 2026.18.0, the **Temperature** parameter in the **LLM Entity Extraction Options** is deprecated. The removal is planned for April 2027. For models that don't support this parameter, activate the **Ignore deprecated parameters** toggle to exclude it from the request to the LLM provider. Otherwise, the request to the LLM provider might fail.
...
<AccordionGroup>
  ...
  <Accordion title="LLM Entity Extraction Options">
    ...
    | Custom Model Options  | JSON          | Sets custom parameters for the LLM configuration, such as `temperature`, `top_k`, or `seed`. These parameters override the ones set in this Node. For the full list of parameters that your model supports, refer to the LLM provider's API documentation, for example, [OpenAI](https://platform.openai.com/docs/api-reference/chat/create) or [Azure OpenAI](https://learn.microsoft.com/en-us/azure/ai-services/openai/). Example: `{ "seed": 42, "temperature": 0.2 }` |
    | Custom Request Options | JSON         | Sets custom parameters for the request sent to the LLM provider, such as HTTP headers. These parameters override the ones set in this Node. For more information, refer to the LLM provider's API documentation. Example: `{ "headers": { "Authorization": "Bearer mytoken" } }` |
  ...
  <Accordion title="Advanced">
    ...
    | Parameter   | Type      | Description                                                                                            |
    |-------------|-----------|--------------------------------------------------------------------------------------------------------|
    | Ignore deprecated parameters | Toggle | Excludes the deprecated **Temperature** parameter from the request to the LLM provider. Activate this toggle if your model doesn't support this parameter. If you activate this toggle for a model that supports it, the model uses a default value. |
  ...
  <Accordion title="Escalation - Intents">
    ...
    | Prevent Reprompt Message Once  | Toggle | If enabled, the escalation message will only be triggered once per conversation, preventing repeated escalation messages.                                                                  |
       ...
       | Handover Provider                                    | Selector       | The provider that will handle the handover. The list contains the handover providers created in **Deploy > Handover Providers**.                                                           |
       ...
       | Get Queue Position                                   | Toggle         | Retrieves the end user's current queue position. Supported by Cognigy Live Agent and Salesforce MIAW. The parameter is deactivated by default. |
       | Get Estimated Wait Time                              | Toggle         | Retrieves the estimated wait time for the end user. End users then receive wait time updates while they wait to be connected to a human agent. Supported by Cognigy Live Agent, Salesforce MIAW, and Genesys Cloud Open Messaging. The parameter is deactivated by default. |
       | Update Interval                                      | CognigyScript  | Sets the interval, in seconds, for how often to receive updates on the queue position or estimated wait time. This parameter appears if **Get Queue Position** or **Get Estimated Wait Time** is selected. |
       | Alternative Update                                   | Toggle         | Sends an alternative message to the end user when the maximum queue position or the maximum estimated wait time is reached. The parameter is deactivated by default. |
