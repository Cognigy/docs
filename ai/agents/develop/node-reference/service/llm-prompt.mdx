---
title: "LLM Prompt"
keywords: ['new llm prompt node', 'llm prompt node']
description: "The Cognigy LLM Prompt Node allows using Generative AI for creating relevant content. This Node supports Tools, including MCP tools, image recognition, and conversation transcripts."
---
...
<Frame>
  <img
    src="/_assets/ai/develop/node-reference/services/llm-prompt.png"
    alt="LLM Prompt Node" style={{ width: 'auto' }} />
</Frame>
...
<AccordionGroup>
  ...
  <Accordion title="Advanced">
    ...
    |------------------------|--------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | Ignore deprecated parameters | Toggle | Excludes the deprecated **Sampling Method**, **Temperature**, **Top Percentage**, **Presence Penalty**, **Frequency Penalty**, **Use Stops**, **Stops**, and **Seed** parameters from the request to the LLM provider. Activate this toggle if your model doesn't support these parameters. If you activate this toggle for a model that supports them, the model uses default values. |
    ...
    | Transcript Turns       | Slider |  The number of previous conversation turns included in the LLM request. A conversation turn includes a user input and the AI Agent’s response. The default value is `50`.<br/>This parameter affects only past turns. The current turn is always included, containing the user input, LLM processing, tool calls, tool results, and multi-step tool workflows.<br/>For example, setting **Transcript Turns** to `0` excludes all previous turns, while the current turn is still fully included.|
  ...
  <Accordion title="Storage & Streaming Options">
    ...
    **Streaming Results**
    ...
    Cognigy monitors delimiter tokens (Stream Buffer Flush Tokens),
  ...
  <Accordion title="Debugging Settings">
    ...
    | Parameter                      | Type          | Description                                                                                                                                                                                                                                                                                                                                                                                            |
    |--------------------------------|---------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | Show Token Count               | Toggle        | Sends a debug message containing the input, output, and total token count. The message appears in the Interaction Panel when [debug mode](/ai/agents/test/interaction-panel/chat#debug-mode) is enabled. Cognigy uses the GPT-3 tokenizer algorithm, so actual token usage may vary depending on the model used. The parameter is inactive by default.                                              |
