...
seo:
  metatags:
    description: >-
      The Cognigy LLM Prompt Node (legacy) allows using Generative AI for
...
<Frame>
  <img
    class="image-center"
    src="/_assets/ai/develop/node-reference/services/llm-prompt-legacy.png"
    style={{ width: 'auto' }}
  />
</Frame>
...
<Warning>
  This Node is in legacy mode, which means it is deprecated. Use a [new version](/ai/agents/develop/node-reference/service/llm-prompt) of this Node to get more flexibility when building AI Agents.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Custom Options">
    ...
    These options let you use use parameters that aren't included in the LLM Prompt Node or overwrite set configurations.
  ...
  <Accordion title="Debugging Settings">
    ...
    | Parameter                  | Type   | Description                                                                                                                                                                                                                                                                                                                                               |
    |----------------------------|--------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | Show Token Count           | Toggle | Send a debug message containing the input, output, and total token count. The message appears in the Interaction Panel when [debug mode](/ai/agents/test/interaction-panel/chat#debug-mode) is enabled. Cognigy.AI uses the GPT-3 tokenizer algorithm, so actual token usage may vary depending on the model used. The parameter is inactive by default. |
    | Log Request and Completion | Toggle | Send a debug message containing the request sent to the LLM provider and the subsequent completion. The message appears in the Interaction Panel when [debug mode](/ai/agents/test/interaction-panel/chat#debug-mode) is enabled. The parameter is inactive by default.                                                                                  |
