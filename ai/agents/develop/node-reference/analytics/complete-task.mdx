---
title: Complete Task
keywords: ['complete task', 'task tracking', 'analytics', 'contact profile', 'insights']
seo:
  metatags:
    description: >-
      The Complete Task Node provides an easy way to track the completion of a
      specific event, once the Node is triggered.
---
...
<Frame>
  <img src="/_assets/ai/develop/node-reference/analytics/complete-task.png" alt="Complete Task Node configuration panel" style={{ width: 'auto' }} />
</Frame>
...
and can be used only with [Tasks](/ai/agents/analyze/goals-and-tasks/tasks).
...
the completed task is stored in the [Contact Profile](/ai/agents/analyze/contact-profiles) and [Analytics database](/ai/agents/analyze/collecting-data).
...
The [top tasks](/insights/reports/overview#top-tasks) are listed in the Overview report
and allow you to see in how many sessions each task was completed.
If you don't intend to analyze Contact Profiles and prefer a more focused analysis using the [OData Endpoint's Steps collection](/ai/agents/analyze/odata#steps), then [Analytics Steps](/ai/agents/analyze/analytics-steps) are more suitable.
...
<Accordion title="Measuring AI Agent Effectiveness">
  ...
  3. If the customer selects B2, handle this separately by triggering a Handover to Human Agent Node. In this case, you don't need to use the Complete Task Node.
...
As an alternative to this Node, you can use the [Track Goal](/ai/agents/develop/node-reference/analytics/track-goal) Node. 
In this case, you can define multiple steps within one [goal](/ai/agents/analyze/goals-and-tasks/goals) and track important metrics such as time and money.
