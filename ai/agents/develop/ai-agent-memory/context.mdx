---
title: Context
keywords: ['context object', 'session persistence', 'session data', 'default context', 'flow control', 'context', 'context properties']
seo:
  metatags:
    description: >-
      The Context Object is a JSON object that persists throughout the entire
      chat or call session.
---

<a href="/release-notes/2026.16"><Badge className="version-badge" color="blue">Updated in 2026.16</Badge></a>

The _Context Object_ is a JSON object that persists throughout the entire chat or call session. If a user disconnects from Cognigy, the Context object can be retrieved upon reconnection with the same session ID. By default, the Context object is empty. You need to set a [Default Context](#default-context) or [configure](#storing-information-in-the-context-object) what data is stored in this object.
...
- By default, you can store up to 256 kB in the Context object, and this limit can be raised up to 1.5 MB. The limit is based on JavaScript object size (UTF-16 encoding), so the actual raw text size may be smaller than expected. For example, 1 MB of text can take up about 2 MB in memory. For on-premises installations, you can change this limit using the `MAX_MEMORY_OBJECT_SIZE` variable in the `values.yaml` file. Increasing the limit raises memory usage and may slow performance.
...
<Tabs>
  ...
  <Tab title="API">
        ...
        You can inject data into and reset the Context object of a session with the [Cognigy API](https://api-trial.cognigy.ai/openapi#tag--Session-v2.0).
  </Tab>
</Tabs>
### Default Context
