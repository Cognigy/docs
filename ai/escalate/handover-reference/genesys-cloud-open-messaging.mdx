...
description: "Genesys Cloud Open Messaging lets AI Agents connect end users with human agents via live chat escalation."
sidebarTitle: "Overview"
keywords: ['genesys cloud open messaging', 'handover provider', 'genesys om']
"seo": {
  "metatags": {
    "description": "Cognigy includes Genesys Cloud Open Messaging as an additional handover provider, allowing Cognigy users to connect with human agents from Genesys Cloud."
  }
}
---
...
<Frame>
  <img src="/_assets/ai/escalate/handover-reference/genesys.svg" alt="Genesys Cloud Open Messaging handover provider logo"/>
  <img src="/_assets/ai/escalate/handover-reference/genesys-dark.svg" alt="Genesys Cloud Open Messaging handover provider logo"/>
</Frame>

The Genesys Cloud Open Messaging handover provider bridges Cognigy and [Genesys](https://login.mypurecloud.com/), enabling end users to connect with human agents working in a contact center that uses Genesys Cloud CX.
...
- Create an Endpoint [compatible with Genesys Cloud Open Messaging](/ai/escalate/handovers#endpoints-compatible-with-handover-providers).
...
- To detect handover completion in Genesys:
    - Starting from Cognigy 4.100, migrate to [Amazon EventBridge](/ai/escalate/handover-reference/genesys-cloud-open-messaging-with-amazon-eventbridge) using the Amazon EventBridge Genesys connector to send requests to Cognigy.
    - For Cognigy 4.99 and earlier versions, you have two options:
        ...
        - As an alternative, you can configure [HTTP webhooks](/ai/escalate/handover-reference/genesys-cloud-open-messaging-handover-end-detection) using Triggers and Web Services Data Actions to send requests to Cognigy.
...
<AccordionGroup>
  ...
  <Accordion title="2. Create a Queue">
    ...
    7. Save the queue ID for later use in Cognigy.
  </Accordion>
  ...
  <Accordion title="3. Create an Inbound Message Flow">
    ...
    2. Hover over the ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg) icon on the **Flows** tab and select **Inbound Message**.
  ...
  <Accordion title="5. Configure Credentials">
    ...
    Once your client is authorized, you can start configuring the Genesys Cloud Open Messaging handover provider on the Cognigy side.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="2. Configure Handover Settings">
    In the [Handover to Human Agent](/ai/agents/develop/node-reference/service/handover-to-agent) Node, select **Genesys Open Messaging** in the **Handover Provider** list, and configure the following fields:
    ...
    - **Get Estimated Wait Time** — retrieve the estimated wait time for the end user. End users then receive wait time updates while they wait to be connected to a human agent. The parameter is inactive by default. It requires an active [Amazon EventBridge Source integration](/ai/escalate/handover-reference/genesys-cloud-open-messaging-with-amazon-eventbridge). Genesys Cloud Open Messaging doesn't provide a queue position. Estimated wait time is the only queue parameter available. For more information, read [Real-Time Queue Notifications](/ai/escalate/handover-features/real-time-queue-notifications#configure-settings-in-genesys-cloud-open-messaging).
    - **Custom Attributes** — add custom attributes, which allows you to include additional information. When sending custom attributes from Cognigy to Genesys Cloud, you can enter them in two ways:
    ...
    Before sending to Genesys, Cognigy flattens nested objects, joins or indexes arrays, converts booleans to 1/0, and skips unsupported types.
...
<AccordionGroup>
    <Accordion title="Send Genesys Bot Messages to End Users">
      ...
      Cognigy is responsible for message routing logic. To configure this logic, follow the instructions:

      1. In your handover Flow, set a **Lookup** Node below the **Handover to Human Agent** Node. Set the **Lookup** Node as your Entrypoint.
      ...
      The main Flow on Cognigy should look like this:

      <Frame>
        <img src="/_assets/ai/escalate/handover-reference/genesys/genesys-flow-sample.png" alt="Cognigy Flow with Lookup and Go To Nodes for Genesys handover" style={{ width: 'auto' }} />
      </Frame>
    ...
    When using a [Webchat v3](/webchat/v3/configuration) or [WhatsApp](/ai/agents/deploy/endpoint-reference/whatsapp) Endpoint with the Genesys Cloud Open Messaging handover provider, end users can send and receive file attachments during a handover session.
  ...
  <Accordion title="Embedded">
    ...
    <AccordionGroup>
      <Accordion title="1. Create a Script">
        ...
        1. In Cognigy, go to Endpoint **Settings > Copilot**.
      ...
      <Accordion title="2. Add the Script to the Inbound Message Flow">
        ...
        2. Click or hover over the ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg) icon on the **Flows** tab and select **Inbound Message**.
