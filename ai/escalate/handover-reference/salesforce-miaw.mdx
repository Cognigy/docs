...
keywords: ['contact centers', 'handover provider', 'salesforce', 'salesforce miaw', 'salesforce messaging for in-app and web']
seo:
  metatags:
    description: >-
      The Salesforce MIAW handover provider bridges Cognigy and Salesforce
...
<Frame>
  <img
    src="/_assets/ai/escalate/handover-reference/salesforce.svg"
    alt="Salesforce MIAW handover provider logo"
  />
</Frame>

## Description

The Salesforce MIAW handover provider bridges Cognigy and [Salesforce Messaging for In-App and Web](https://help.salesforce.com/s/articleView?id=service.miaw_intro_landing.htm&type=5),
...
- Created Endpoint [compatible with Salesforce](/ai/escalate/handovers#endpoints-compatible-with-handover-providers).
- Give [Salesforce users access](https://help.salesforce.com/s/articleView?id=service.miaw_non_agent_permissions_1.htm&type=5).

## Restrictions

- The Salesforce MIAW handover provider supports attachments only when used with [Webchat](/webchat/overview).
...
<AccordionGroup>
  <Accordion title="1. Configure Omni-Channel Settings">
  ...
  <Accordion title="3. Create a Flow">
    ...
    9. Click **Activate**, then **Publish**. At this stage, you will route your flow via the queue. You can also configure routing for a specific agent, bot, or skill. For more details, refer to the Salesforce [documentation](https://help.salesforce.com/s/articleView?id=service.omnichannel_routing_targets.htm&type=5).
  </Accordion>
  <Accordion title="4. Create a Messaging Channel">
  ...
  <Accordion title="5. Create an Embedded Service">
    ...
    5. Go to **Code Snippet**. Copy the `OrganizationId`, `DeveloperName`, and `Url` values for later use in Cognigy.
    6. Click **Done**. Check whether your deployment is active and click **Publish**. The deployment will take about 10 minutes. Afterward, you can go to Cognigy to configure the handover settings.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="2. Add a Handover to Human Agent Node">
    In the [Handover to Human Agent](/ai/agents/develop/node-reference/service/handover-to-agent) Node, select **Salesforce MIAW** in the **Handover Provider** list, and configure the following fields:
    ...
    - **End Chat When Agent Leaves** — the setting allows the handover to end automatically when a human agent is removed from a Salesforce MIAW conversation. This setting is turned off by default. Instead of waiting for the conversation close event, which [Salesforce may not always send](https://help.salesforce.com/s/articleView?id=000397177&type=1), this setting relies on the participant change event to trigger the end of the handover.
    - **Agent Change Notification** — this setting sends a system message to the end user indicating when the conversation is reassigned to another human agent. This parameter is deactivated by default. When the conversation is reassigned, the system message is sent under the human avatar name set in the [Webchat v3 Endpoint settings](/webchat/v3/configuration#webchat-layout).
    - **Get Queue Position** — activate the setting to retrieve the end user's current queue position. This setting is turned off by default.
    - **Get Estimated Wait Time** — activate the setting to retrieve the estimated wait time for the end user. This setting is turned off by default.
    - **Alternative Update** — activate this setting to send an alternative message to the end user when certain conditions are met. This setting is turned off by default.
    - **Maximum Queue Position** — set the threshold for the maximum queue position. If this threshold is reached, the alternative text is sent instead of the actual queue position. This parameter appears if **Alternative Update** is active.
    - **Alternative Text** — specify the message to be sent when the maximum queue position is reached. This parameter appears if **Alternative Update** is active.
...
Within the Salesforce MIAW integration, you can use the [Agent Copilot workspace](/agent-copilot/overview) as an assistant to your human agents.
...
<AccordionGroup>
  ...
  <Accordion title="Embedded">
    ...
    <AccordionGroup>
      <Accordion title="1. Update Messaging Settings">
      ...
      <Accordion title="2. Update Embedded Service Deployments">
        1. In the **Quick Find** box, enter `Embedded Service Deployments` and click **Embedded Service Deployments**. Alternatively, navigate to **Feature Settings > Service > Messaging > Embedded Service Deployments** in the left-side menu.
      ...
      <Accordion title="3. Update the Flow">
      ...
      <Accordion title="4. Configure UI Components">
        To configure the Agent Copilot UI components, refer to the [Salesforce Integrations](https://github.com/Cognigy/salesforce-integrations) documentation on GitHub. Once the components are configured, you can test the embedded Agent Copilot workspace using [Demo Webchat](/webchat/demo).
...
Cognigy prevents Salesforce MIAW chats from closing automatically when human agents leave the conversation and continues polling for new events.

#### Cognigy 2025.16 and earlier
...
When a handover starts, Salesforce MIAW provides an access token with a configurable expiration time to Cognigy in the backend. To make sure that the session is synchronized, Cognigy polls Salesforce MIAW approximately every 2 minutes to check for new messages or actions, and verifies if the access token is still valid. Additionally, Cognigy has a configurable grace period, counted from the end of the access token expiration time to control the session termination. The default grace period is 30 minutes. When the grace period is reached, Cognigy terminates the session on both Cognigy and Salesforce MIAW sides. This approach prevents unnecessary API calls to Salesforce MIAW sessions with expired access tokens.

<AccordionGroup>
  <Accordion title="Set the Access Token Expiration Time">
    ...
    3. On the **Messaging Settings** page, click the **Show one more action** arrow next to the channel you want to configure, and select **Edit**.
  ...
  <Accordion title="Set the Grace Period">
    - For on-premises installations, set the grace period in seconds using the
    `SALESFORCE_MIAW_ACCESS_TOKEN_GRACE_PERIOD` variable in the `values.yaml`
    file.
    - For SaaS installations, contact [Cognigy Support](/help/get-help).
  </Accordion>
  <Accordion title="Example">
    If you have an access token expiration time of 6 hours (360 minutes) and a
    grace period of 30 minutes, Cognigy terminates the session on both
    platforms 5 hours and 30 minutes after the handover started.
  </Accordion>
</AccordionGroup>
