...
keywords: ['contact centers', 'handover provider', 'salesforce']
seo:
  metatags:
    description: >-
      Cognigy has seamlessly integrated with Salesforce, adding it as an
      additional handover provider. This integration enables users to
      effortlessly connect with human agents who use Salesforce Service Cloud as
...
<Frame>
  <img src="/_assets/ai/escalate/handover-reference/salesforce.svg" alt="Salesforce Service Cloud handover provider logo"/>
</Frame>
...
<Warning>
  ...
  The Salesforce handover provider bridges Cognigy and [Salesforce Service Cloud](https://salesforce.com/),
...
<Accordion title="Configure Salesforce Service Cloud">
  ...
  4. Navigate to **Feature Settings > Service > Embedded Service > Embedded Service Deployments**. In the row with the deployment you want to use, click ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg), then select **View**.
...
<AccordionGroup>
  ...
  <Accordion title="2. Add a Handover to Human Agent Node">
    In the [Handover to Human Agent](/ai/agents/develop/node-reference/service/handover-to-agent) Node, select **Salesforce** in the **Handover Provider** list, and configure the following fields:
  ...
  <Accordion title="2.1 Configure Pre-Chat Entities and Details">
    By default, Cognigy doesn't send a conversation transcript from the chat with the AI Agent to the chat with the human agent. If you activate the Send Transcript as a first message setting, Cognigy sends the conversation transcript to the chat with the human agent as the first message. The conversation transcript includes the entire chat between the user and the AI Agent.
    ...
    Moreover, Cognigy allows users to send Pre-chat Details and Pre-chat Entities arrays as a custom JSON structure. This feature allows users to include additional context or relevant information before initiating the chat with the human agent. This information can then be used for further processing or personalizing the conversation based on the provided details.

    **Pre-Chat Entities**

    Cognigy allows the creation of fields
    ...
    **Pre-Chat Details**
    ...
    Cognigy allows sending conversation information to Salesforce Live Agent using Pre-chat Details.
