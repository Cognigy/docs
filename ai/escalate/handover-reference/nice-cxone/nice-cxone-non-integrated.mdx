...
<Frame>
  ...
  <img src="/_assets/ai/deploy/endpoint-reference/nice-dark.svg" alt="NiCE CXone handover provider logo" />
</Frame>

## Description

The NiCE CXone (Non-Integrated) handover provider bridges Cognigy and [NiCE CXone](https://www.nice.com/), enabling end users to connect with human agents in a NICE CXone contact center.

NiCE CXone (Non-Integrated) is for Cognigy installations that are deployed independently from NiCE CXone. The two systems run on separate infrastructure, so you must configure a BYOC (Bring Your Own Channel) channel and provide NiCE CXone credentials manually.

## Prerequisites

- Access to [NiCE CXone](https://cxone.niceincontact.com/).
- Cognigy installation isn't integrated with [NiCE CXone environment](/ai/administer/access/management-ui#create-an-organization). If your Cognigy installation is already integrated with NiCE CXone at the platform level, use the standard [NiCE CXone (Integrated) handover provider](/ai/escalate/handover-reference/nice-cxone/nice-cxone-integrated).
- An Administrator role in NiCE CXone to access the required configuration settings and create API credentials.
- A Client ID and Client Secret obtained by submitting the [CXone Application Registration](https://forms.microsoft.com/Pages/ResponsePage.aspx?id=vdojcYcOqU2cubfsggEarfHlkRVlgSlMjqsp52ASGttUM1U2VTQ4N0pXNk5XSzROV0VNNkhBU1gzMC4u) form. NiCE sends the credentials to your registered technical contacts within 2–3 business days. You need these credentials later in Cognigy.

## Restrictions

<NiCERestrictions/>
- After importing a NICE CXone (Non-Integrated) handover provider from a Package or Snapshot, repeat [Create and Configure a Handover Provider](#1-create-and-configure-a-handover-provider) starting from step 4, then follow the steps in [Configuration on the NICE CXone side](#configuration-on-the-handover-provider-side). This configuration isn't included in the import.
...
<Accordion title="1. Create and Configure a Handover Provider">
    ...
    1. In Cognigy, go to **Deploy > Handover Providers**.
    ...
    <AccordionGroup>
      <Accordion title="Tenant ID">
        ...
        4. On the **General** tab, copy the value from the **Tenant ID** field and enter it in Cognigy.
      </Accordion>
      ...
      <Accordion title="Business Unit ID">
        ...
        3. On the **Business Unit details** page, copy the value from the **Unit Number** field and enter it in Cognigy.
      </Accordion>

      <Accordion title="NiCE CXone Authentication Outbound">
        Credentials for authenticating outbound API calls from Cognigy to NiCE CXone. Configure the following parameters:
      ...
      <Accordion title="Channel Integration ID">
        ...
        4. Configure the following fields:
            <Note>
              Replace `<cognigy-instance-domain>` in the URL with the domain of your Cognigy instance. For example, if your instance URL is `https://trial.cognigy.ai`, replace `<cognigy-instance-domain>` with `trial`.
            </Note>
            - **Name** — a name for the integration, for example, `Cognigy Integration`.
        ...
        5. Click **Save**. Copy the generated **Integration ID** and enter it in Cognigy.
      </Accordion>
...
7. Verify the created channel in NiCE CXone:
   ...
   2. Find the channel with the same name as your NiCE CXone handover provider in Cognigy. If you don't find the channel, the connection wasn't established. Check the handover provider configuration to ensure that all credentials are valid.
</Accordion>
...
<Accordion title="2. Add a Handover to Human Agent Node">
    ...
    4. _(Optional)_ Configure additional settings:
        ...
        - **Custom Attributes** — add custom attributes, which allow you to send additional data to NiCE CXone. When sending custom attributes from Cognigy to NiCE CXone, you can enter them in two ways:
        ...
        You can also [configure matching custom fields on the NiCE CXone side](#configure-matching-custom-fields-and-attributes) to:

            - Display custom attributes in the **View Details** section in the Agent Workspace.
            - Fetch custom attributes with the [CC Custom Fields card](https://help.nicecxone.com/content/studio/actions/ccard_fields/ccard_fields.htm).
