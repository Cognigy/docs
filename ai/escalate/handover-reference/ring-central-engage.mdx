---
title: RingCentral Engage Integration
sidebarTitle: RingCentral Engage
keywords: ['contact centers', 'handover providers', 'ringcentral engage']
seo:
  metatags:
    description: >-
      The RingCentral Engage handover provider bridges Cognigy and
      RingCentral Engage, enabling end users to connect with human agents
      working within the RingCentral Engage contact center.
---

<a href="/release-notes/2026.19"><Badge className="version-badge" color="blue">Updated in 2026.19</Badge></a>

<Frame>
    <img src="/_assets/ai/escalate/handover-reference/ring-central-engage.svg" alt="RingCentral Engage handover provider logo" />
</Frame>

## Description

The RingCentral Engage handover provider bridges Cognigy and [RingCentral Engage](https://engage.ringcentral.com/),
...
- Created Endpoint [compatible with RingCentral Engage](/ai/escalate/handovers#endpoints-compatible-with-handover-providers).
...
<AccordionGroup>
  <Accordion title="1. Create API Token">
  ...
  <Accordion title="2. Create the Categories">
    ...
    4. Double-click `Bot` or `Agent` to open their configuration pages. Extract the `CATEGORY_ID` from the URL, for example, `xxxxx.engagement.dimelo.com/admin/categories/CATEGORY_ID/edit`.
     These are needed for the **Bot Category Id** and **Agent Category Id** fields in Cognigy Handover Settings.
  </Accordion>

  <Accordion title="3. Create the Source">
    ...
    5. Copy the **API access token** field value from the source into the **Realtime Access Token** field in Cognigy Handover Settings.
    6. Copy the **Realtime Endpoint URL** from the **Implementation Details** section into the corresponding field in Cognigy Handover Settings.
  </Accordion>

  <Accordion title="4. Configure Agent Permissions">
  ...
  <Accordion title="5. Create the Webhook">
    ...
    3. In the **SETTINGS** section:
        ...
        4. Enter a randomly generated value in the **Secret** field. Copy this value for the **Webhook Secret** field in Cognigy.
    ...
    6. Update the **Verification Token** to match your Cognigy environment's token. By default, the value is `cognigy123`. Save changes.
  </Accordion>

  <Accordion title="6. Get the Base API URL">
    ...
    Save it for later use as the **Base API URL** in Cognigy.
  </Accordion>

  <Accordion title="7. Create Bot and Agent Identities">
  ...
  <Accordion title="8. Set Up Folders for Chatting">
...
<AccordionGroup>
  <Accordion title="1. Create a Handover Provider">
    ...
    3. _(Optional)_ Toggle the **Forward Only Handover Conversations** setting to choose whether to forward all conversations to the provider or only those that trigger a handover. If this setting is enabled, Cognigy will forward only conversations where the handover event is triggered.
  ...
  <Accordion title="2. Add a Handover to Human Agent Node">
    1. Create a Flow in Cognigy with a Handover to Human Agent Node.
    2. Link the Endpoint to this Flow and configure the following:
          - **Additional Category Ids** — sets additional `CATEGORY_ID` values to be passed to the RingCentral Engage handover provider, beyond the **Bot Category ID** and **Agent Category ID** configured in the handover provider.
...
<AccordionGroup>
  <Accordion title="Apple Business Chat">
    Ensure human agents with API token ownership have `reply` permissions on all non-custom SDK sources.
  </Accordion>

  <Accordion title="Send User Profile to RingCentral Engage">
    During a Handover event, the Contact Profile is sent to RingCentral Engage to provide helpful information to human agents.

    <Warning>
      The Cognigy Contact Profile will be sent to RingCentral Engage and matched up with the object keys existing on RingCentral Engage. The information that wasn't matched will be added to the `context_data` object.
      ...
      - [Cognigy Profile schema](/ai/agents/analyze/contact-profiles#profile-schema). Cognigy Profile Schema can be edited and additional fields with specific object keys added.
    </Warning>
  </Accordion>

  <Accordion title="Get Queue Updates">
    ...
    In addition to enabling the **Get Queue Update** option, you need to configure the Lookup Node so that the handover type is selected as **Queue**. For more details, refer to the [Handover Status](/ai/agents/develop/node-reference/logic/lookup) section.
