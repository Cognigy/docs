---
title: Slot Fillers
keywords: ['slot fillers', 'cognigy nlu', 'slots']
seo:
  metatags:
    description: >-
      Cognigy Slot Fillers let you monitor every user input for specific Slot
      matches and store them in context. This way, if an answer is already
      provided, the AI Agent skips the corresponding question.
---

_Slot Fillers_ let you monitor every user input for specific [Slot](/ai/platform-features/nlu/slots/overview) matches
and store them in the [Context](/ai/agents/develop/ai-agent-memory/context) object.
...
<iframe width="500" height="313" style={ {border: '1px solid #ccc', borderRadius: '12px'} } src="https://www.youtube.com/embed/bAP-r9iYeL0?si=-a5hMumF7aZ1O2eW" title="Cognigy Sessions: Cognigy Slot Fillers" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen class="centered-iframe"></iframe>
...
<Tabs>
  ...
  <Tab title="API">
         You can create, edit, and delete Slot Fillers by using [Cognigy API](https://api-trial.cognigy.ai/openapi#patch-/v2.0/flow/-flowId-/slotfillers).
...
<AccordionGroup>
  <Accordion title="Required Fields">
     ...
     | Type               | Defines the type of input expected. You can select from the following Slot types: <br/>- **Slot** – a [Lexicon Slot](/ai/platform-features/nlu/slots/user-defined/lexicon). Make sure a Lexicon is attached and that this Slot is included in it.<br/>- **Regular Expression** – a [Regex Slot](/ai/platform-features/nlu/slots/user-defined/regex).<br/>- **Intent** – extracts values based on recognized Intents. Useful when Slot filling is tied to Intent classification.<br/>- **System Slot** – [predefined Slot types](/ai/platform-features/nlu/slots/system-defined) that automatically extract common entity types such as dates, numbers, temperatures, and more. | -                                |
     ...
     | Regular Expression | This parameter appears if the **Regular Expression** type is selected. Enter the regular expression to extract specific patterns from user input. Regex expressions must start and end with `/`. Flags like `g` for global matching aren't supported. Example: `/^\d{3}-\d{3}-\d{4}$/`. |                                  |
  ...
  <Accordion title="Optional Fields">
     ...
     | Result Location                          | Determines where to extract the Slot value from. By default, the Slot value is extracted from the user input. You can set a CognigyScript expression in this parameter to override the detected Slot value with a value from the [Input, Context, or Profile objects](/ai/agents/develop/ai-agent-memory/overview). | `{{input.bookingReference}}` |
...
<AccordionGroup>
  <Accordion title="Step 1. Add Slots">
  ...
  <Accordion title="Step 2. Configure Question Nodes">
  ...
  <Accordion title="Step 3. Test Slot Fillers">
...
- [Overview](/ai/platform-features/nlu/slot-fillers)
