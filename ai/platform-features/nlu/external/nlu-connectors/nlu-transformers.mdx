...
keywords: ['nlu-transformers', 'nlu']
seo:
  metatags:
    description: >-
      NLU Transformers are used to customize the pipeline in NLU Connectors. By
      using NLU Transformers, you can change the incoming message from the
      Endpoint before it's sent to the NLU engine, and to change the response
      from the NLU engine before it's sent to the Flow execution.
---
...
<Accordion title="Example of the `transformerStack` object in the Interaction Panel">
  ```json
  {
    "transformerStack": {
      "preNluInput": {
...
<AccordionGroup>
  ...
  <Accordion title="getSessionStorage">
  ...
  <Accordion title="Logging">
    You can use the exposed modules `console.log`, `console.info`, and `console.error` in the transformers to display logs on the [Logs page](/ai/agents/test/logs).
  </Accordion>
</AccordionGroup>

## More Information

- [Overview](/ai/platform-features/nlu/external/nlu-connector-reference/all-nlu-connectors)
