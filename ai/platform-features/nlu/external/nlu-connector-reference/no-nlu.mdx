---
title: No NLU
keywords: ['no nlu', 'nlu connectors']
seo:
  metatags:
    description: >-
      The No NLU connector disables built-in Cognigy NLU for the selected
      Endpoint, meaning no Intents or Slots are detected.
---

The _No NLU_ connector disables built-in Cognigy NLU for the selected Endpoint, meaning no Intents or Slots are detected.
...
<AccordionGroup>
  ...
  <Accordion title="Execute Cognigy NLU Node">
    By default, Cognigy NLU runs before the Flow executes. However, in some cases, you may need to process parts of the Flow before triggering NLU analysis to better control when an Intent is recognized.
