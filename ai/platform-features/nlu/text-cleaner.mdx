---
title: Text Cleaner
keywords: ['text cleaner', 'text cleaning functions']
seo:
  metatags:
    description: >-
      Cognigy helps you clean up user input (text or voice) with built-in
      functions. For example, if a user says `alpha tango hotel double seven
      three nine minus L for London and C for Canada`, it can automatically be
      converted to `ath 7739 - lc`.
---

<a href="/release-notes/4.58"><Badge className="version-badge" color="blue">Updated in 4.58</Badge></a>


Cognigy helps you clean up user input (text or voice) with built-in functions. For example, if a user says `alpha tango hotel double seven three nine minus L for London and C for Canada`, it can automatically be converted to `ath 7739 - lc`.

These Text Cleaner functions support the English (`en`) and German (`de`) locales and are available in:

- [Clean Text Node](/ai/agents/develop/node-reference/ai/clean-text)
- [Question Nodes](/ai/agents/develop/node-reference/basic/question)
- [Code Node](/ai/for-developers/code/modules)
- [Transformers](/ai/for-developers/transformers/overview#text-cleaner-class)

To learn more about the benefits of using Text Cleaner functions and how they work, watch this video:

<iframe width="500" height="313" style={ {border: '1px solid #ccc', borderRadius: '12px'} } src="https://www.youtube.com/embed/37m4Yrs161E?si=b7MRNOEtBsOuksO3" title="Cognigy Sessions: Cognigy Slot Fillers" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen class="centered-iframe"></iframe>
...
| replaceSpecialWords _(Deprecated)_ | Replaces specifically set words with their replacements. For example, "lufthansa" > "lh". <br/><br/> As of Cognigy 4.58.0, this function is deprecated. Use the `replaceSpecialPhrases` function instead.  |
