...
keywords: ['cognigy nlu', 'yes/no intents', 'intents', 'confirmations', 'affirmative words', 'deny words']
seo:
  metatags:
    description: >-
      The *Yes/No Intents* feature enables machine learning to detect `yes` and
      `no` responses similar to Intent matching. With a small set of example
      sentences, it trains a model to recognize diverse `yes` and `no` phrases,
      creating a more fluent natural language experience.
---

<a href="/release-notes/4.98"><Badge className="version-badge" color="blue">Updated in 4.98</Badge></a>


The *Yes/No Intents* provide a machine learning-powered feature to detect `yes` and `no` responses similarly to traditional Intent matching, enabling a more natural and fluent conversational experience. Training a model with a small set of example sentences recognizes various `yes` and `no` phrases, enhancing an AI Agent's ability to understand user confirmations or denials.

The [Question](/ai/agents/develop/node-reference/basic/question), [Optional Question](/ai/agents/develop/node-reference/basic/optional-question) and [Code](/ai/for-developers/code/overview) Nodes support the Yes/No Intents feature.

## Key Benefits

- **Improved Natural Language Understanding**. Unlike the earlier [Confirmation Words](/ai/platform-features/nlu/intents/confirmation-words) feature, which required manual synonym definitions, Yes/No Intents use a preinstalled vocabulary that can be expanded. This approach makes it easier to detect diverse expressions.
...
<AccordionGroup>
  <Accordion title="Select a Locale">
    ...
    <Tabs>
      <Tab title="GUI">
        ...
        2. Select the locale and click ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg), then **Edit Yes/No Intents**.
        ...
        4. Additionally, for Yes and No Intents, you can add [rules](/ai/platform-features/nlu/intents/rules).
        5. Click **Save & Build** to build a model. The Yes/No model is always built when you edit and save settings.
      </Tab>
      <Tab title="API">
                To create or update sentences for a specific Locale, use the following Cognigy API requests:
  ...
  <Accordion title="Use Yes/No Intents for Specific or All Flows">
...
Check if the Yes/No Intent feature works as expected via the [Interaction Panel](/ai/agents/test/interaction-panel/overview):

1. In the existing Flow, open the Interaction Panel by clicking ![interaction-panel](/_assets/icons/blue/interaction-panel.svg) **Chat with your Agent** in the upper-right corner of the page.
2. Run a Flow.
...
<AccordionGroup>
  <Accordion title="Examples of possible results">

    <Tabs>
      <Tab title="Yes Intent">
      ...
      <Tab title="Fallback Reject Intent">
  ...
  <Accordion title="FAQ">
    ...
    **A2**: Yes, the Yes/No Intents are completely independent of standard intent scoring (`input.intentScore` or `input.nlu.intentMapperResults`). They specifically find the types `nAnswer` and `yAnswer` within the Flow. These types are used for the **Yes/No** type in [Question Nodes](/ai/agents/develop/node-reference/basic/question) and for [confirming Intents](/ai/platform-features/nlu/intents/overview), similar to [confirmation words](/ai/administer/access/project-settings#nlu-settings).
...
- [Confirmation Words](/ai/platform-features/nlu/intents/confirmation-words)
