...
seo:
  metatags:
    description: >-
      Cognigy.AI Intents are collections of identified potential user inputs
...
<AccordionGroup>
  ...
  <Accordion title="All Flows">
    1. In the left-side menu of the Project, go to **Build > Flows**.
    2. On the **Flows** page, check if the **Train all Flows** button exists. If the **Train all Flows** button isn't enabled for your environment, specify the `FEATURE_TRAIN_ALL_PROJECT_FLOWS: "true"` feature flag in the `values.yaml` file for on-premises installations or contact [Cognigy technical support](/help/get-help).
