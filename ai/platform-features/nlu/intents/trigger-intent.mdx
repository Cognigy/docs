...
keywords: ['cognigy nlu', 'trigger specific intent', 'trigger intent']
seo:
  metatags:
    description: >-
      In Cognigy.AI, the Trigger Intent feature lets you manually activate a
      specific Intent in your Flow using the syntax `cIntent:` followed by the
      Intent name, overriding regular Intent mapping.
---
...
2. In a Say, Question, or Optional Question Node, use the syntax `cIntent:Continue Booking` as a Postback value in [Output Types](/ai/agents/develop/node-reference/basic/say) that support Postbacks. 
