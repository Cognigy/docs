...
- [Knowledge Stores](/ai/agents/develop/knowledge-ai/knowledge-store)<sup><a href="#footnote2">2</a></sup>
...
- A Package can be up to 2 GB. If a Package exceeds this limit, you can't create or upload it. For on-premises installations, you can increase the maximum size by setting the `PACKAGE_MAX_FILE_SIZE` environment variable in `values.yaml`.
...
<span style={{ fontSize: "0.8rem" }}>
  ...
  Packages don't support the [NiCE CXone](/ai/escalate/handover-reference/nice-cxone/overview) handover providers because they aren't transferable between Projects. If you want to use the same handover provider in another Project or environment, create a new handover provider.
</span>

<span style={{ fontSize: "0.8rem" }}>
  <sup id="footnote2">2</sup>
