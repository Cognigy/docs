...
- The maximum Package size depends on how you create or upload it:
    - **Cognigy.AI UI**: You can create and upload Packages of up to 8 GB. For on-premises installations, you can increase this limit by setting the `RESUMABLE_UPLOAD_MAX_FILE_SIZE` environment variable in `values.yaml`. The same limit also applies to [Snapshots](/ai/agents/deploy/snapshots).
    - **Cognigy.AI API**: You can create and upload Packages of up to 2 GB. Packages that exceed this limit can't be created or uploaded. For on-premises installations, you can increase this limit by setting the `PACKAGE_MAX_FILE_SIZE` environment variable in `values.yaml`.
