...
Real-time translation between a user and a human agent works for text-based conversations after a user triggers a [Handover to Agent Node](/ai/agents/develop/node-reference/service/handover-to-agent) in the [Flow](/ai/agents/develop/projects-and-flows/overview). Conversations with real-time translation between the user and the human agent are recorded for compliance purposes.
