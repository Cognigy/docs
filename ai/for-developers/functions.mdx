---
title: Functions
description: "Execute long-running JavaScript functions in Cognigy.AI for async processes like HTTP API calls to third-party systems."
keywords: ['functions', 'javascript', 'api integration']
tag: Deprecated
---

<Warning>
  The Functions feature has been deprecated. Instead, use the [Code Node](/ai/for-developers/code/overview) or the [HTTP Request Node](/ai/agents/develop/node-reference/service/http-request). For long-running operations, use a middleware service, such as AWS Lambda or Azure Functions, with the [Inject API](/ai/agents/deploy/endpoints/inject-and-notify) to return results to the conversation.
</Warning>

_Functions_ are JavaScript functions that you can define and execute in Cognigy.AI. Use Functions to assist with asynchronous processes, such as interacting with third-party systems through an HTTP API.
...
- The maximum run-time of a Function instance is 15 minutes. For on-premises installations, you can change this limit via the `FUNCTION_EXECUTION_MAX_EXECUTION_TIME_IN_MINUTES` environment variable in the `values.yaml` file. For long-running operations that exceed this limit, set up a dedicated middleware service, such as AWS Lambda or Azure Functions, and call back using the [Inject API](/ai/agents/deploy/endpoints/inject-and-notify) once the process completes. This approach is more scalable than Cognigy Functions for time-intensive tasks.
