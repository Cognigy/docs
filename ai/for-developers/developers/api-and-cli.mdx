---
title: API and CLI
description: Build custom integrations, manage AI Agents, and automate workflows via the Cognigy REST API or CLI.
...
_Cognigy API and CLI_ are toolsets for developers to interact with Cognigy. The API allows programmatic interaction and integration with existing systems. The CLI provides direct command-line access to manage AI Agents. Using both tools streamlines workflows, enhances productivity, and manages Cognigy Projects efficiently.

## Limitations

- For Cognigy 4.100 and earlier, you can download [Snapshots](/ai/agents/deploy/snapshots) up to 100 MB using the Cognigy CLI. In later versions, there's no maximum download size for Snapshots via the CLI.
- The `push` and `restore` CLI commands can only update resources. If you want to create a resource, use [the Cognigy API](#cognigy-ai-api).
- CLI commands don't support adding or moving Nodes in the Flow structure. For this purpose, use [the Cognigy API](#cognigy-ai-api).

## Cognigy API

For communication with the Cognigy installation, the frontend uses an HTTP-based API.
This API can also be used by developers to build custom integrations with Cognigy. We also offer a library in the form of an NPM package called the "rest-api-client" which helps to lower the burden of consuming our APIs. This library is available on NPM and is written in modern JavaScript including Typescript definitions.
...
The API Base URL is the endpoint for accessing the Cognigy OpenAPI.
...
| On-premises              | Specify the API URL as the value for the environment variable `BACKEND_BASE_URL_WITH_PROTOCOL` in your Cognigy `values.yaml` file.                                                                                                                                                                                                                                                                                         |
...
An API key is used to authenticate to the Cognigy API as a user in the system without exposing the user's credentials.
...
For example, if your Cognigy installation runs at `https://api-<cognigy-domain>.ai` and your API key is `<api-key>`, you can open a browser tab and go to `https://api-<cognigy-domain>.ai/projects?api_key=<api-key>` to get a list of all the Projects assigned to you, including metadata about the resources within each Project.

This call is used by the frontend when you load the **Main** page.

### Super API-Key

Most of the APIs in Cognigy are agent-bound,
...
<AccordionGroup>
  <Accordion title="Feature availability">
    This feature is only available for on-premises customers with separate Cognigy installations.

    This feature is, by default, disabled in all Cognigy installations. Our on-premises customers can activate this feature by setting the following environment variable to "true":
...
The Cognigy CLI is a command-line interface enabling users to manage Projects directly from the command line. For example, with this CLI, you can create Snapshots, manage Locales, and deploy AI Agents efficiently.

To start using the Cognigy CLI, refer to the [Cognigy repository on GitHub](https://github.com/Cognigy/Cognigy-CLI).
...
<AccordionGroup>
  <Accordion title="Clone a Project">
    Clone a Project or its resources of one type from Cognigy to your local machine.
  ...
  <Accordion title="Restore a Project">
    Upload a Project or its resources from your local machine to Cognigy.
  ...
  <Accordion title="Run Playbooks">
    Schedule Playbook runs on Cognigy and poll for results. The output is written to `./playbookRunResults.json`.
