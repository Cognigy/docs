...
description: "NiCE Cognigy MCP Server lets you build, test, and optimize LLM-based AI Agents via the Cognigy REST API."
tag: deprecated
---

<Warning>
  The NiCE Cognigy MCP Server is deprecated and has been replaced by the [NiCE Cognigy plugin](/ai/for-developers/developers/cognigy-plugin). For new setups, install the plugin instead. Existing MCP Server installations continue to work. New features and improvements are delivered through the plugin.
</Warning>

The _NiCE Cognigy MCP Server_ is an [npm package](https://www.npmjs.com/package/@cognigy/mcp-server) that provides a [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server for connecting MCP clients like Claude, Cursor, or VS Code to the Cognigy REST API. With the NiCE Cognigy MCP server, you can create, test, and iteratively improve LLM-based AI Agents without leaving your development environment.

<div style={{textAlign: 'center'}}>
   <video width="500" height="300" controls autoPlay title="NiCE Cognigy MCP Server tools executed via Claude Code CLI in terminal" aria-label="NiCE Cognigy MCP Server tools executed via Claude Code CLI in terminal" style={{border: '1px solid #ccc', borderRadius: '12px', display: 'block', margin: '0 auto'}}>
...
- **Comprehensive API coverage.** Use workflow tools in the NiCE Cognigy MCP Server to cover the Cognigy API for end-to-end automation.
...
<Tabs>
  <Tab title="Claude Desktop (.mcpb)">
    ...
    3. Enter your Cognigy API base URL and API key.
  </Tab>

  <Tab title="CLI">
    Run the setup command for your MCP client in the CLI. The command prompts you to enter your Cognigy API base URL and API key, then automatically updates the MCP client configuration.
...
The NiCE Cognigy MCP Server sends requests only to the Cognigy API base URL. The server doesn't collect, store, or share any data. All data stays between your MCP client and your Cognigy instance. For more information, see the [Cognigy Privacy Policy](https://www.cognigy.com/privacy-policy).

## How to Use

Once the server is connected to your MCP client, you can interact with Cognigy using natural language prompts.
...
<AccordionGroup>
  <Accordion title="Create an AI Agent">
    Ask your MCP client to create an AI Agent in a Cognigy Project:
