...
You can send HTTP requests to external services from a transformer. All transformer functions can access the `httpRequest` module. The module is invoked as a function that takes a single configuration object as argument. You can use the following formats for the configuration object:

<Tabs>
  <Tab title="Axios">

  The configuration object follows the format described in the [Axios's Request Config documentation](https://axios-http.com/docs/req_config). For example:
