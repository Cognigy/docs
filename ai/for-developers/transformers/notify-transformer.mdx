...
keywords: ['notify transformer', 'webhook payload', 'notify api', 'data transformation', 'external service integration']
---
...
The notify transformer returns a valid user ID, session ID, text, and data. These return values need to match the format of the payload of the Endpoint you are using. The payload format is specific to the Endpoint type you use, for example, Facebook (Meta) Messenger. Read the documentation of the [specific Endpoint](/ai/agents/deploy/endpoint-reference/overview) to know how to format the payload.
