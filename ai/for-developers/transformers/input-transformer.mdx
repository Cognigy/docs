...
seo:
  metatags:
    description: >-
      The Input Transformer converts the user input after it's sent to an
      Endpoint. This allows you to manipulate the user input data before it is
      sent to the Flow, for example, to communicate with external systems, to
      implement integrations with a custom channel, and much more.
keywords: ['input transformer', 'user input', 'data transformation', 'endpoint integration', 'custom channels', 'external integrations']
---
...
The input transformer returns the user ID, session ID, text, and data, which are subsequently sent to the Flow. These values are extracted from the Endpoint payload. The payload format is specific to the Endpoint type you use, for example, Facebook (Meta) Messenger. Read the documentation of the [specific Endpoint](/ai/agents/deploy/endpoint-reference/overview) to know how to format the payload.
...
Conversations in Cognigy are counted if the input transformer returns any result but falsy.
