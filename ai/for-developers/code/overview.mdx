...
description: "Execute custom JavaScript or TypeScript code within Cognigy.AI Flows using Code Nodes with IntelliSense support."
keywords: ['code node', 'javascript', 'typescript', 'intellisense', 'flow scripting']
---

<a href="/release-notes/2025.24"><Badge className="version-badge" color="blue">Updated in 2025.24</Badge></a>

<Frame>
  <img class="image-center" src="/_assets/ai/develop/node-reference/code/code.png" alt="Code Node editor with JavaScript and TypeScript support" style={{ width: 'auto' }} />
</Frame>
...
The `api` object provides access to most internal Cognigy.AI functions within the Code Node. For more information, refer to [API Functions](/ai/for-developers/code/api-functions).

The `getTextCleaner` function allows you to get access to an instance of the [Text Cleaner](/ai/for-developers/code/modules).
...
- The maximum code execution time is 1 second. If the limit is exceeded, an error is returned, and the message can be accessed in the `input.codeNodeError.message` Input object. The maximum execution time isn't configurable. To run code that takes longer than one second to execute, we recommend using custom [Extensions](/ai/for-developers/extensions).
...
defined in the [Profile Schema](/ai/agents/analyze/contact-profiles#profile-schema).

## Adding Log Statements to Code Nodes

To view log statements on the [Logs page](/ai/agents/test/logs), insert `api.log()` statements into the Code Node.
...
### Encode the HTTP Authorization Header in Base64

To use Basic Authentication, add a Code Node above your HTTP Request Node in the Flow editor to encode credentials in Base64.

<Accordion title="Code Node">
  1. Create a Code Node that dynamically encodes credentials:
  ```js
  context.encodedCredentials = api.base64Encode(`${context.username}:${context.password}`);
  ```
  2. In the HTTP Request Node, set the Authorization Header to:
  ```js
  Basic {{context.encodedCredentials}}
  ```
</Accordion>

### Sending Facebook JSON

You can send Facebook JSON directly from within Code Nodes using the [output](/ai/for-developers/code/api-functions) action.

<Accordion title="JSON Sample">
  ```js
...
- [API Functions](/ai/for-developers/code/api-functions)
- [Input object](/ai/agents/develop/ai-agent-memory/input)
- [Context object](/ai/agents/develop/ai-agent-memory/context)
