...
description: Configure SAML 2.0 single sign-on (SSO) for Cognigy.AI to enable secure user authentication with your organization's identity provider.
...
Cognigy.AI integrates with popular single sign-on (SSO) [identity providers (IDPs)](/ai/administer/installation/identity-providers), such as Microsoft Entra, OneLogin, and Okta. This integration allows users within your organization to log in to Cognigy.AI without separate credentials.
...
For example, the api-url for the app environment is `api-app.cognigy.ai`. The `organization-id` is the `id` of your organization within Cognigy.AI. You can get your `organisationId` on the [My Profile](/ai/agents/develop/ai-agent-memory/profile) page by clicking ![vertical-ellipsis](/_assets/icons/black-and-white/vertical-ellipsis.svg) **> Copy Organization ID**.
