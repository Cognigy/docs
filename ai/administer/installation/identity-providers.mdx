...
description: Explore the identity providers to set up single sign-on in Cognigy.
---

import EncodeCertificate from "/snippets/installation/encode-certificate.mdx"

Cognigy integrates with popular identity providers (IdPs) to let users in your organization log in with [single sign-on (SSO)](/ai/administer/installation/single-sign-on-saml2) without the need for individual credentials.
...
- An account with the admin role in Cognigy.
- The organization ID of your Cognigy organization, referred to as `<organization-id>` in the examples. You can copy this ID from the [My Profile](/ai/administer/user-menu/my-profile) page by clicking ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg) **> Copy Organization ID**.
- The [API base URL](/ai/for-developers/developers/api-and-cli#api-base-url) of your Cognigy installation, referred to as `<api-base-url>` in the examples.
- Your Cognigy URL, referred to as `<cognigy-url>` in the examples. For example, for the trial environment, this URL is `https://trial.cognigy.ai/`.
- An [API key](/ai/administer/user-menu/my-profile) for sending configuration requests to the Cognigy API.
- Administrator access to the IdP tenant that you want to integrate with Cognigy.
...
- An organization can have only one SSO configuration. To replace an SSO configuration, delete it, then create another one. For more information, read [Change an SSO Configuration in Cognigy](/ai/administer/installation/single-sign-on-saml2#change-an-sso-configuration-in-cognigy).
- Only Microsoft Entra ID and OneLogin support single logout for Cognigy.
...
<Tabs>
  <Tab title="Auth0 - OpenID Connect">
    ...
    3. Enter a name, for example, `Cognigy`, select **Regular Web Applications** as the app type, and click **Create**.
    4. Go to the **Settings** tab and copy the values from the fields in the **Basic Information** section. You will use them later to [configure the IdP in Cognigy](#configure-sso-in-cognigy-ai):
  ...
  <Tab title="Auth0 - SAML 2.0">
    <Steps>
      <Step title="Set Up an App">
        ...
        1. Log in to the [Auth0 Dashboard](https://manage.auth0.com/) and select your tenant.
        2. In the left-side menu, go to **Applications > Applications** and click **+ Create Application**.
        3. Enter a name, for example, `Cognigy`, select **Single Page Web Applications** as the app type, and click **Create**.
        ...
        5. On the **Addons** tab, activate the **SAML2 Web App** add-on. Auth0 opens the add-on settings dialog. From the **Usage** tab, copy and save the values from the following fields for later use in the API request payload to [configure the IdP in Cognigy](#configure-sso-in-cognigy-ai):
      ...
      <Step title="Map Roles">
        For each user which you want to apply SSO to, set `user_metadata` to map the user's name and Cognigy role. To do so, follow these steps: 
        ...
        1. Go to **User Management > Users**, select the user, scroll to the **Metadata** section, and paste the following JSON into the **user_metadata** field:
            ...
            The `role` value must match a [role in Cognigy](/ai/administer/access/admin-center/access-control), for example, `admin` or `base_role`.
  ...
  <Tab title="Microsoft Entra ID">
    <Steps>
      <Step title="Set Up an App">
        ...
        4. Enter a name, for example, `Cognigy`, select **Integrate any other app you don't find in the gallery (Non-gallery)**, and click **Create**.
      ...
      <Step title="Map User Attributes">
        1. In the **User Attributes and Claims** section, click **Edit** and confirm that the following claims are mapped to the user's profile attributes:
            ...
            Cognigy uses the email claim to identify the user and to create the Cognigy account on the first login.
      
      </Step>
      <Step title="Get Authentication Data">

        1. In the **SAML Signing Certificate** section, locate **Certificate (Base64)** and click **Download**. Save the file locally. Use this certificate as `idpCertificate` in the API request to [configure the IdP in Cognigy](#configure-sso-in-cognigy-ai). You need to encode the certificate as base64 without newlines beforehand. To do so, use the following terminal command:

            <EncodeCertificate path="path-to-certificate" />

        2. In the **Set up `<application name>`** section, copy the following values to be used in the API request to [configure the IdP in Cognigy](#configure-sso-in-cognigy-ai):
        ...
        4. Paste the following JSON code in the `appRoles` array:
            <Accordion title="Roles Array">
              ```json
              {
                ...
                "isEnabled": true,
                "description": "The Admin role in Cognigy",
                "value": "admin"
              },
              {
                "allowedMemberTypes": [
                ...
                "isEnabled": true,
                "description": "The apiKeys role in Cognigy",
                "value": "apiKeys"
              },
              {
                "allowedMemberTypes": [
                ...
                "isEnabled": true,
                "description": "The base role in Cognigy",
                "value": "base_role"
              },
              {
                "allowedMemberTypes": [
                ...
                "isEnabled": true,
                "description": "Admin privileges, no user assignments in Cognigy",
              ...
              },
              {
                ...
                "isEnabled": true,
                "description": "The OData role in Cognigy",
                "value": "odata"
              },
              {
                "allowedMemberTypes": [
                ...
                "isEnabled": true,
                "description": "The Project Manager role in Cognigy",
              ...
              },
              {
                ...
                "isEnabled": true,
                "description": "The User Manager role in Cognigy",
                "value": "userManager"
              },
              {
                "allowedMemberTypes": [
              ...
              },
              {
                ...
                "isEnabled": true,
                "description": "The role to view user details in Cognigy",
              ...
              },
              {
                ...
                "isEnabled": true,
                "description": "Partial Admin, read-only, no assignments, no OData/API, no Knowledge AI in Cognigy",
              ...
              },
              {
                ...
                "isEnabled": true,
                "description": "Assigns Agents, read-only access, no global roles, limited features in Cognigy",
                "value": "projectAssigner"
              }
              ```
            </Accordion>

        5. In the left-side menu, go to **Users and groups** and assign the app to the users or groups that must have access to Cognigy through SSO.
      </Step>
    </Steps>
  </Tab>
  <Tab title="Google">
    <Steps>
      <Step title="Add a Custom Attribute">
        You need to add a custom attribute that Cognigy uses when the user logs in for the first time. To do so, follow these steps:
        ...
        4. Click the **Add Custom Attribute** button and configure the following:
            ...
            - **Category** — enter a unique name to identify the custom attribute, for example, `Cognigy SSO`.
            ...
            <Note>
              If a user doesn't have a role assigned, Cognigy assigns `base_role` to the user.
            </Note>
      ...
      <Step title="Set Up an App">
        ...
        4. Enter a name for the app, for example, `Cognigy`, and click **Next**.
        ...
        6. On the **Attribute Mapping** page, add the following attribute mappings so that Cognigy receives the user's first name, last name, and role from Google:

          | Service Provider Attribute | Custom Attribute | Field      |
          | -------------------------- | ---------------- | ---------- |
          | `firstName`                | Cognigy SSO   | First Name |
          | `lastName`                 | Cognigy SSO   | Last Name  |
          | `role`                     | Cognigy SSO   | Role       |

        7. Click **Finish**, then activate the app for the users or organizational units that must have access to Cognigy through Google SSO.
  ...
  <Tab title="Okta">
    <Steps>
      <Step title="Set Up an App">
        ...
        4. On the **General Settings** tab, enter an app name, for example, `Cognigy SSO`, optionally upload a logo, and click **Next**.
        ...
        6. _(Optional)_ To encrypt SAML requests, click **Show Advanced Settings**, set **Assertion Encryption** to **Encrypted**, and [upload an X.509 certificate](#prerequisites). You will need the private key of this certificate to register the IdP in Cognigy.
      </Step>
      <Step title="Map User Attributes">
        1. In the **Attribute Statements** section, add the following attributes so that Cognigy receives the user profile data it needs to create the account:
      ...
      <Step title="Assign Users">
        ...
        2. Click **Add Attribute** and configure the following:
            ...
            - _(Optional)_ **Description** — enter a relevant description, for example, `The role of the user in Cognigy`.
            - **Enum** — activate **Define enumerated list of values**.
            - **Attribute member** — enter a display name and the value of the role to which SSO applies, for example, `Base Role` and `base_role`. The value must match a [role in Cognigy](/ai/administer/access/admin-center/access-control).
            - **Attribute required** — select this option.  
            Click **Save**.

        3. Go to **Applications > Applications** and select the app you created.
        4. On the **Assignments** tab, click the Edit button next to the user's name and assign the role they should have in Cognigy. This user can log in via SSO.
      </Step>
    </Steps>
  </Tab>
  <Tab title="OneLogin">
    <Steps>
      <Step title="Set Up an App">
        ...
        4. In the **Display Name** field, enter a name that identifies the integration, for example, `Cognigy SSO`. Optionally upload icons. Click **Save**.
      ...
      <Step title="Get Authentication Data">
        ...
        3. Add the following Cognigy roles one by one, select the app you created, and click **Save**:
        ...
        4. In the left-side menu, go to the **Users** tab and select a user for SSO. On the user page, enter the role they have in Cognigy in the **role** field. The role must match the roles you configured in step 3.
...
## Configure SSO in Cognigy

After the SSO app is ready, use the [`POST /v2.0/identityprovider/configure`](/api-reference/identity-providers/set-up-an-identity-provider) method to register the SSO configuration in Cognigy.
...
<Tabs>
  ...
  <Tab title="Microsoft Entra ID">
    ...
    const response = await fetch("https://<api-base-url>/v2.0/identityprovider/configure", {
      ...
      body: JSON.stringify({
        idpType: "saml",
        idpIssuer: "https://<api-base-url>/auth/saml/login/<organization-id>",                                // Cognigy SSO URL
  ...
  <Tab title="Google">
    ...
    <Warning>
      Don't include the PEM header, the PEM footer, or any newline characters in the certificate you submit to Cognigy. A malformed certificate string causes the SSO configuration request to fail.
    </Warning>
...
A successful request returns a confirmation that the identity provider configuration has been saved. The Cognigy login page now displays a **Log in with SSO** button for users in your organization.

## Test the SSO Login

1. On the Cognigy login page, enter the email address of a user assigned to the IdP app and click **Log in with SSO**. Cognigy redirects you to the IdP and prompts you for your IdP credentials. After authentication, the IdP redirects back to Cognigy and logs you in.
