---
title: Weaviate Upgrade to v1.22.5 with Cognigy Release 4.66.0
sidebarTitle: Weaviate Upgrade to v1.22.5
description: Upgrade Weaviate from 1.19.0 to 1.22.5 with Cognigy 4.66.0 to improve Knowledge AI stability.
---

This guide is intended for Cognigy on-premises customers
who are using the [Knowledge AI features](/ai/agents/develop/knowledge-ai/overview).
...
Cognigy releases 4.65.0 and earlier use Weaviate 1.19.0 as the vector database for the [Knowledge AI features](/ai/agents/develop/knowledge-ai/overview).
The latest version of Weaviate, for example, 1.22.5, introduces several stability improvements necessary to ensure the smooth operation of Knowledge AI features in Cognigy.
...
- This guide uses the file name `cognigy-ai-values.yaml` to refer to the Helm values file containing your customizations for the Cognigy Helm chart. Replace this with the file name used in your environment.
- This guide assumes that Cognigy is deployed in the `cognigy-ai` namespace. If you have deployed Cognigy in any other namespace, adjust the namespace parameters in all the commands accordingly.

## Prerequisites

- Cognigy 4.66 is deployed.
    - The Cognigy upgrade from v4.65 to v4.66 will not affect the Weaviate version, and it must be completed before attempting to upgrade Weaviate.
...
- Kubernetes cluster meets general Cognigy [prerequisites](/ai/administer/installation/prerequisites#whitelisting-of-domains), including hardware resources.
...
3. Back up all Weaviate Persistent Volumes (PVs) using instructions from your cloud provider and also save the PV manifests. Save the PV manifests.
    ...
    ```
4. In your `cognigy-ai-values.yaml` file for Cognigy, set `weaviate.image.tag` to `1.22.5`.
5. Redeploy the Cognigy Helm chart. This action will scale up the Weaviate StatefulSet with Weaviate version 1.22.5.
    ```bash
...
5. Remove `weaviate.image.tag` from `cognigy-ai-values.yaml`. The Helm Chart upgrade will later pick up the 1.19.0 tag from the Cognigy values.yaml shipped with the Cognigy release.
6. Redeploy the Cognigy Helm chart. This action will scale up the Weaviate StatefulSet with Weaviate version 1.19.0.
    ```bash
    helm registry login cognigy.azurecr.io \
    --username <your-username> \
    --password <your-password>
