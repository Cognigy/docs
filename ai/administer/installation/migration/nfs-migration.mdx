---
sidebarTitle: NFS Migration
title: Migration Guide for Network File Systems
seo:
  metatags:
    description: >-
      This migration guide helps you migrate to the current NFS structure in
      Cognigy on-premises installations.
---

This migration guide helps you migrate to the current NFS structure in on-premises installations.

Several Cognigy features rely on file shares, Network File System (NFS), to store files that multiple services access.

A new NFS structure has been introduced to clearly and consistently reflect [Cognigy's architecture](/ai/administer/installation/architecture). As of Cognigy 4.90.0, the legacy NFS structure, for example, paths using `flow-modules` and `functions` as described in this guide, is deprecated. Migrate to the current NFS structure.
...
2. Deploy the Cognigy Helm chart:
...
- [Architecture](/ai/administer/installation/architecture)
- [Migrate Extensions Tenant Storage](/ai/administer/installation/migration/extensions-tenant-storage-migration): recommended next step after completing the NFS migration.
