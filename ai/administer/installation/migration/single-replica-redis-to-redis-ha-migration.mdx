...
description: Migrate Cognigy.AI from single-replica Redis to Redis High Availability (HA) mode, required since version 4.65.0.
---

As of Cognigy.AI 4.65.0, the single-replica Redis setup is deprecated and replaced with Redis in High Availability (HA) Mode.

## Prepare Redis and Redis persistent HA Configuration 

Before upgrading to Cognigy.AI 4.65, you must perform the following steps and modify the `values.yaml` of your Cognigy.AI Helm Release accordingly.
...
## Upgrade Cognigy.AI Helm Release to 4.65

1. Double check that all parameters in your `values.yaml` are adjusted as described above.
2. Perform the upgrade of the Cognigy.AI Helm Release to 4.65 as usual. During the upgrade:
...
After upgrading Cognigy.AI to 4.65 and verifying that the release works properly, you can clean up the remaining persistent 
