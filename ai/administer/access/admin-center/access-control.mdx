...
keywords: ['access control', 'global roles', 'user management', 'admin center']
seo:
  metatags:
    description: >-
...
<ExpandableTable
  ...
  rows={[
    ["<code>admin</code>", "Access to all organization resources, except the Cognigy OData endpoint. The system creates one admin user during installation. A platform owner should create a new admin user and delete the default one after the first login."],
    ...
    ["<code>apiKeys</code>", "Access to create organization-wide API keys for Cognigy RESTful API access. Permissions apply equally when using API keys."],
    ["<code>odata</code>", "Access to the Admin Center section and the OData interface for querying analytics information."],
    ["<code>liveAgentAdmin</code>", "Access to Live Agent as an admin. Same permissions as `base_role` in Cognigy."],
    ["<code>liveAgentSupervisor</code>", "Access to Live Agent as a supervisor."],
    ["<code>liveAgentAgent</code>", "Access to Cognigy Live Agent as a human agent. Same permissions as `base_role` in Cognigy."],
    ...
    ["voice-gatewayUser", "Access to the Voice Gateway Self-Service Portal with authorization via Cognigy user credentials. Assigned to users with the Account role."],
    ["<code>fullSupportUser</code>", "Access with admin-level privileges excluding the ability to assign Projects or global roles to others. Includes access to the Cognigy OData endpoint, Cognigy API, and Audit Events."],
    ...
    ["<code>opsCenterUser</code>", "Access to Cognigy AI Ops Center in addition to `base_role` in Cognigy."]
  ]}
  initialRowsToShow={5}
...
<AccordionGroup>
  <Accordion title="Create New Users">
    To create a new user in Cognigy for your organization, you must have the `admin` or `userManager` global role.
