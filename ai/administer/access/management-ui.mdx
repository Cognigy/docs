...
seo:
  metatags:
    description: "The Management UI is a web-based interface for managing system-wide and organization-specific settings in Cognigy."
---

<a href="/release-notes/2026.15"><Badge className="version-badge" color="blue">Updated in 2026.15</Badge></a>

The *Management UI* is a web-based interface for managing system-wide and organization-specific settings in Cognigy.
...
- **User and Organization Management**. Create, modify, remove users, and add them to specific organizations, which serve as isolated environments within Cognigy.
...
- **Multitenancy and Impersonation Features**. Cognigy supports multitenancy, enabling the management of multiple organizations under one installation, along with the ability to impersonate users for troubleshooting and assistance.

## Before you Begin

<AccordionGroup>
    <Accordion title="1. Check Availability">
        The Management UI is available for the following Cognigy installations:
    ...
    <Accordion title="2. Log in to the Management UI">
        ...
        | **Field**                   | **Description**                                                                                                              |
        | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
        | Cognigy Backend Base URL | The base URL must follow this format: `https://api-<your installation domain>`. For example: `https://api-trial.cognigy.ai`. |
...
Organizations define the environment for users, Projects, and data in Cognigy. Use the Management UI to create and configure organizations, adjust quotas and policies, and manage data privacy and expiration settings. Each organization has its own limits, users, and settings, allowing for secure and flexible multi-tenant environments.
...
<AccordionGroup>
    <Accordion title="Create an Organization">
        1. In the Management UI, select **Organisations** from the left-side menu, then click ![create organization](/_assets/icons/management-ui/plus.svg) in the bottom-right corner.
        2. In the **Create new Organisation** window, fill in the following fields:
            ...
            - **CXone Tenant ID** – this field is optional. If you want users to log in to Cognigy via their NiCE CXone accounts, enter the Tenant ID for your NiCE CXone instance. This way, users can log in via single sign-on (SSO), so they don't need a separate Cognigy username or password. Each Tenant ID is tied to a single Cognigy organization. When users log in via SSO, they can access only the organization associated with that Tenant ID.<br/>
              Specifying a Tenant ID allows NiCE CXone users to log in to Cognigy under the **Automation** category in the app selector of the NiCE CXone interface.
              This option is available only if the Cognigy application is activated in NiCE CXone.
              For more information, refer to the [NiCE CXone](https://help.nicecxone.com/content/cognigyai/enablecognigyaiandssoaccess.htm#Virtual) documentation.
    ...
    <Accordion title="View Organizations">
        ...
        | Tenant ID                 | The NiCE CXone Tenant ID that enables SSO login to a single Cognigy organization.                                                                                                                                                         |
        ...
        | **NiCE CXone Info**       | This section is optional and appears only if **Business Unit ID** or **Tenant ID** is configured for the organization.                                                                                                                       |
        | CXone Business Unit ID    | This field is optional. The ID used for NiCE Cognigy billing purposes. For more information, refer to the [Create an Organization](#create-an-organization) section.                                                                         |
        | CXone Tenant ID           | This field is optional. The ID used for SSO login via NiCE CXone. For more information, refer to the [Create an Organization](#create-an-organization) section.                                                                              |
        | **Conversation Counter**  |                                                                                                                                                                                                                                              |
        | Conversation Counter      | The total number of conversations for a specific period. Conversation counts are calculated overnight. For this reason, data for the current day isn't included. Historical data for conversation counts is available only from 2025 onward. |
    </Accordion>
    <Accordion title="Change Organization Information">
       To change the organization information, such as Billing Time Zone, Business Unit ID, or Tenant ID, select **Organisations** from the left-side menu. From the organization list, select the organization you want to change. On the **Organisation Details** page, click ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg) and select **Change Organization Information**.
    ...
    <Accordion title="Change Organization Quotas">
        To change organization quotas, select **Organisations** from the left-side menu. From the organization list, select the organization you want to change. On the **Organisation Details** page, click ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg) and select **Change Organisation Quotas**.
    ...
    <Accordion title="Change Password Policy">
        ...
        To change the password policy for an organization, select **Organisations** from the left-side menu. From the organization list, select the organization you want to change. On the **Organisation Details** page, click ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg) and select **Change Password Policy**.
    ...
    <Accordion title="Change Expiration Values">
        ...
        - In Cognigy, refer to [Data Expiration](/ai/agents/analyze/data-expiration) in the Cognigy documentation.
...
Create and manage users within your Cognigy installation.
...
<AccordionGroup>
    <Accordion title="Create a User">
        ...
        2. You can open a **Create new User** window by using one of the following options:
            - Select **Users** from the left-side menu, then click ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg) in the bottom-right corner.
    ...
    <Accordion title="View User Details">
        ...
        | Organisation              | The organization the user belongs to.                                                                                                                         | ACME                                                |
        ...
        | Organisation Enabled      | Whether the user's organization is currently enabled in the system.                                                                                           | Yes                                                 |
        | **CXone Info**            | This section is optional and appears only if the user is assigned to a Cognigy organization linked to a NiCE CXone instance.                               |                                                     |
        | CXone Email Address       | The user's email address (`cxoneEmail`) in the NiCE CXone instance. The value of this field is added automatically and can't be changed in the Management UI. |                                                     |
        | CXone ID                  | The user ID (`cxoneId`) in the NiCE CXone instance. The value of this field is added automatically and can't be changed in the Management UI.                 |                                                     |
    ...
    <Accordion title="Block User Impersonation">
        ...
        - For on-premises installations, use the `BLOCK_IMPERSONATE_USER_ORG_WHITELIST` feature flag. You can block user impersonation for:
    ...
    <Accordion title="Other Operations">
        You can also disable or delete a user by selecting them from the user list, clicking ![menu](/_assets/icons/black-and-white/vertical-ellipsis.svg), and selecting the respective option on the **User Details** page.
    </Accordion>
</AccordionGroup>
...
This feature is especially useful for notifying users in on-premises installations about planned downtimes or system-wide changes.

<Accordion title="Create a System Message">
To create a system message:

1. In the Management UI, select **System Message** in the left-side menu, then click ![create system message](/_assets/icons/management-ui/plus.svg) in the bottom-right corner.
