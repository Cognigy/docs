---
title: Set up a Cognigy Endpoint for Handover
description: Configure a Live Agent Endpoint for Live Agent in Cognigy to hand over conversations from AI Agents to humans.
keywords: ['handover endpoint', 'live agent setup', 'cognigy endpoint', 'api key', 'account id']
---
...
When you create a new Agent in Cognigy you are prompted to select an Endpoint. You can select, for example, **Cognigy Webchat** as a specific Endpoint to the Cognigy platform. 
...
<AccordionGroup>
  <Accordion title="Handover to pre-configured Live Agent">
    If the **Handover to pre-configured Live Agent account** option is activated, conversations will be transferred to the Live Agent account mapped to the current Cognigy organization.
...
When you have found the account ID, you can enter it in the **Handover Settings** of your Endpoint in Cognigy.
...
3. Copy the API Key and paste it into the **API Key** field in the **Live Agent Authentication** section of your handover provider settings in Cognigy.
