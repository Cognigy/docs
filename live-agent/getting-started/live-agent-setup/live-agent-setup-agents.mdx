---
title: Add Agents and Admins to Live Agent
description: Create users for human agents and admins in Live Agent, assign user roles, and connect them to inboxes.
keywords: ['add agents', 'live agent setup', 'user roles', 'inbox assignment', 'agent management']
...
Once you have [set up your Endpoint and Flow for Handover to Live Agent](/live-agent/getting-started/live-agent-setup/live-agent-setup-handover) and tested a conversation, you can then proceed to add new Agents and Admins and assign them to Inboxes in Live Agent.

## Create Users

To create users, follow these steps:

1. Open the Cognigy interface. If the Agent or Admin you want to add already exists as a User in Cognigy, proceed to [Adding Live Agent Roles to Users](#add-live-agent-roles-to-users).
2. On the **Main** page, in the upper-right corner, click **User Menu ![user menu](/_assets/icons/blue/user-menu.svg) > Admin Center**.
...
For more information, see [Live Agent Roles](/live-agent/roles).

To add one of these roles to users, follow these steps:

1. On the **Main** page of the Cognigy interface, click **User Menu ![user menu](/_assets/icons/blue/user-menu.svg) > Admin Center** in the upper-right corner.
...
When the Agents have been included in the Inbox, you can [add another Inbox for your Project](/live-agent/getting-started/live-agent-setup/live-agent-setup-additional-inbox).
