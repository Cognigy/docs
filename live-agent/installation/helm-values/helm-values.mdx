...
> Note: As of Cognigy.AI 4.38.0, setting Live Agent `SECRET_KEY_BASE` under the `configmap` section is deprecated. Set `SECRET_KEY_BASE` under the `cookiesIntegrity` section instead. For previous installations, it is recommended to create an existing secret, otherwise the value will change, and all the Live Agent user sessions will expire.
