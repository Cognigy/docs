---
title: Defining Ingress host
sidebarTitle: Helm Values
description: Configure Live Agent Helm values including Ingress host settings, annotations, and TLS options for deploying on Kubernetes clusters.
...
Cognigy uses an App Platform Token to perform operations,
 such as synchronizing and creating data (for example, Inboxes and accounts),
using the Live Agent API.
For this, Live Agent and Cognigy need to have secrets set to matching values.
...
2. Restart the following Cognigy pods in the Cognigy namespace:
   - service-handover
   - service-security
   - service-api
   - service-resources
...
contained in the Cognigy namespace with the name "overview-collector-api-key"
...
| Name                                              | Description                                           | Default Value                             |
|---------------------------------------------------|-------------------------------------------------------|-------------------------------------------|
| `configmap.COGNIGY_AI_UI_BASE_URL_WITH_PROTOCOL`  | URL used for accessing Cognigy UI from Live Agent. | `""https://installation.cognigy.ai""`     |
| `configmap.COGNIGY_AI_API_BASE_URL_WITH_PROTOCOL` | URL for performing requests to Cognigy API         | `""https://api-installation.cognigy.ai""` |
...
It will scan the file uploading for viruses and block the upload if a virus is found. It scans any file uploaded contained in a message as an attachment, including the ones from Live Agent UI and Cognigy Webchat.
...
| `configmap.FRONTEND_EXTERNAL_URL`    | Set a different Frontend URL for external systems to access Live Agent (for example, request file upload) | `""`          |
...
> Note: As of Cognigy 4.38.0, setting Live Agent `SECRET_KEY_BASE` under the `configmap` section is deprecated. Set `SECRET_KEY_BASE` under the `cookiesIntegrity` section instead. For previous installations, it is recommended to create an existing secret, otherwise the value will change, and all the Live Agent user sessions will expire.
...
Live Agent performs requests to Cognigy APIs. If you are running Cognigy services with a Self-Signed Certificate with Custom Certificate Authority (CA) to trust, or if you need SSL to be disabled, these settings are necessary.
...
Create a secret with a key named the certificate file name (for example, `cert.pem`). It will be mounted in the pod file system to be trusted by the Live Agent app. The value must contain the certificate file content without extra tabs/spaces. Then fill in the following values:
...
| `restClient.ssl.clientKeySecretKey`           | Client key secret key. Its value must be in RSA Key format                    |
