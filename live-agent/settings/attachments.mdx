---
title: Attachments
description: Manage file attachments in Live Agent conversations, including supported formats like PDF, DOCX, and images, and how human agents upload and access files.
keywords: ['attachments', 'file upload', 'live agent', 'conversation files', 'document sharing']

---

<a href="/release-notes/4.70"><Badge className="version-badge" color="blue">Updated in 4.70</Badge></a>
...
End users can send attachments to human agents by using the **Add attachments** icon in the chat if you set up a [Storage File](/ai/agents/deploy/endpoints/file-storage) provider in the Endpoint Settings.

## Request Attachments with the Plugin

If you are using a [Webchat](/webchat/overview) Endpoint with Live Agent, in addition to the standard Attach file feature, human agents can request an attachment from a user. To enable this functionality, you need to add the [File Upload plugin](https://github.com/Cognigy/WebchatPlugins/tree/master/plugins/file-upload) to the Webchat Endpoint to upload files.
...
- [Conversation Workflow](/live-agent/conversation/overview)
