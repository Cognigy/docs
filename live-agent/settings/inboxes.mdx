---
title: Inboxes
description: Create and manage Live Agent inboxes for routing conversations, configuring human agent assignments, message visibility, and integrating AI assistants.
keywords: ['inboxes', 'live agent', 'inbox management', 'conversation routing', 'agent assignment']
---
...
_Inboxes_ are workspaces where human agents handle real-time chat interactions with customers and manage related tasks. Each Inbox ID represents the connection from Cognigy to Live Agent. 
...
3. On the **Inbox setup** page, fill in the following fields:
    ...
    - **Webhook URL** — enter the Cognigy Endpoint Config URL domain name, extended by `/handover/liveagent`. For example, `https://endpoint-trial.cognigy.ai/handover/liveagent`. You can find the Endpoint Config URL in the **Configuration Information** section of your Endpoint.
...
5. Click the **Create Cognigy Inbox** button. Once you have successfully set up the Inbox, you are prompted to assign your agents to the new Inbox in the next step.
...
1. In the Live Agent interface, go to **Settings > Inboxes**.
2. On the **Inboxes** page, click ![Inbox settings](/_assets/icons/black-and-white/gear.svg) next to the Inbox you want to edit.
3. In the **Inboxes Settings** section, update the following fields:
    ...
    - **Cognigy Project ID** — this field is pre-populated for [Inboxes created through Cognigy](/live-agent/getting-started/live-agent-setup/live-agent-setup-additional-inbox). Enter a valid Project ID. The Project ID allows Live Agent to retrieve [Contact Profile](/ai/agents/analyze/contact-profiles) data from Cognigy and binds the Inbox to the respective Project. You can [copy the Project ID](/ai/agents/develop/projects-and-flows/projects) from the Cognigy interface. After you assign the Inbox to a Project, [copy the Inbox ID](#copy-inbox-id) and enter it in the respective Cognigy Nodes of the Project, for example, the [Handover Agent](/ai/agents/develop/node-reference/service/handover-to-agent) and [Check Agent Availability](/ai/agents/develop/node-reference/service/check-agent-availability) Nodes. If you delete the Project to which the Project ID refers, Live Agent can still communicate with Cognigy Nodes, but the system cannot retrieve Contact Profile data through this Inbox.
    - **Webhook URL** — enter the Cognigy Endpoint Config URL domain name, extended by `/handover/liveagent`. For example, `https://endpoint-trial.cognigy.ai/handover/liveagent`. You can find the Endpoint Config URL in the **Configuration Information** section of your Endpoint.
4. Click **Update**.

## Copy Inbox ID

Each Inbox ID represents the connection between Cognigy and Live Agent and is used in API and UI URL paths.

To connect an Inbox to Cognigy,
copy the ID of the Inbox
and specify it in the **Live Agent Settings** section in the [Handover to Human Agent](/ai/agents/develop/node-reference/service/handover-to-agent) Node on the Cognigy side.
...
Deleting an Inbox is irreversible and will cause a loss of synchronization with Cognigy.
...
To create such a bot, you need to have a separate Flow and Endpoint on the Cognigy side.
