...
description: Create and manage canned response templates in Live Agent to help human agents reply faster with pre-saved, HTML-formatted quick replies to common questions.
keywords: ['canned responses', 'reply templates', 'live agent', 'quick replies', 'message templates']
...
3. In the **Add Canned Response** window, fill in the following fields:
    ...
    - **Level** — select one of the following options from the list:
        ...
        - **Team** — only human agents from the selected [team](/live-agent/settings/teams) can use a canned response. You can select only one team.
4. Click **Submit**.
...
To start using canned responses in the conversations, go to the [Send a Canned Response](/live-agent/conversation/send-reply#advanced-quick-reply).
