...
description: Configure Live Agent account settings including general options, Agent Copilot, notifications, and conversation workflow. Only available to Administrators.
keywords: ['account settings', 'live agent', 'conversation limit', 'auto resolve', 'notifications']

---

import LiveAgentPushNotifications from '/snippets/live-agent/push-notifications.mdx'
...
You can select one of the available options in a dropdown list by clicking on the language field. The default language is English (en).
Setting a site language also translates system messages, push notifications, and email notifications.
...
When the agent performs an action, it will automatically be set back to the online state.
...
The TTL starts counting down after conversation activities take place, such as sending messages, changing the assignee, updating labels, or changing the conversation status.

Opening or reading a conversation doesn't count as an activity. As a result, a conversation can be deleted while an agent has it open.

Live Agent deletes conversations once every 24 hours. Therefore, conversations may remain available for up to 24 hours after their TTL has expired.

To deactivate the TTL for conversations, click **Disable TTL for conversations**.

<Accordion title="TTL Example">

  | **Timeline**   | **Scenario 1: Agent only reads**                                     | **Scenario 2: Agent sends a message**                                                      |
  | -------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
  | **January 1**  | Status changed to `Abandoned`. The TTL starts.                     | Status changed to `Abandoned`. The TTL starts.                                           |
  | **January 6**  | Agent opens and reads the conversation. The TTL is not affected. | Agent opens the conversation and sends a message. The TTL starts again from January 6. |
  | **January 8**  | TTL expires.                                                     | The TTL is still active.                                                                   |
  | **January 9**  | **Conversation is deleted during the next deletion cycle.**          | The TTL is still active.                                                                   |
  | **January 13** | —                                                                    | TTL expires.                                                                           |
  | **January 14** | —                                                                    | Conversation is deleted during the next deletion cycle.                                |

</Accordion>

### TTL for Attachments

The **TTL for Attachments** option lets you automatically delete attachments from messages across all conversations in the account after a set period.
...
When you activate this setting, Live Agent automatically deletes conversations marked as resolved.
...
Only the agent himself can see the clear name.
...
In the **Account Settings**, administrators can activate Agent Copilot if [it has been installed previously](/agent-copilot/getting-started/chat).

If an administrator has already preset the **Agent Copilot Workspace** setting in the **Account Settings**,
this setting will take precedence over the user profile settings.

Before you can enable **Agent Copilot Workspace** for all human agents,
activate the **Enable overriding Agent Copilot Workspace preferences** setting.
The configuration set here applies as a preset to all human agents of your organization account
and overrides the [Profile Settings](/live-agent/profile-settings#notifications).
...
You can configure email and push notifications as well as the notification sound of [audio notifications](/live-agent/notifications#configure-notifications-at-the-user-profile-level).

### Email and Push Notifications

<LiveAgentPushNotifications/>
...
To unlock the event options, activate the **Enable overriding email notification preferences** setting. The event options you set apply to all human agents of your Live Agent organization's account and override individual agents' [Profile settings](/live-agent/profile-settings#notifications).

### Notification Sound

You can add a notification sound to the push notifications you configured. To configure the notification sound, follow these steps:
...
For more information, read the [Notifications](/live-agent/notifications) documentation.
...
<Frame>
    <img src="/_assets/live-agent/getting-started/LA_Inbox_URL.png" alt="Live Agent browser URL showing the Account ID and Inbox ID" style={{ width: 'auto' }} />
</Frame>

For example, once you have found the account ID,
you can enter the Account ID in the [Handover Settings](/live-agent/getting-started/live-agent-setup/live-agent-setup-handover-endpoint#handover-to-manually-configured-live-agent) of your Endpoint in Cognigy,
to enable the handover of a customer conversation with your AI Agent to a human agent for support.

## Version

The indicated version is the current version of the Cognigy Live Agent interface. To check the features and fixes for each version, head over to [Release Notes](/release-notes/index).
...
- [Notifications](/live-agent/notifications)
