...
description: Learn how to assign agents, teams, priority, and labels to conversations in Live Agent. Transfer and manage conversation routing between human agents.
keywords: ['conversation assignment', 'agent assignment', 'team assignment', 'live agent', 'conversation routing']
---
...
<Frame>
<img src="/_assets/live-agent/conversation/conversation-actions.png" alt="Live Agent Conversation Actions" style={{ width: 'auto' }} />
</Frame>
...
You can select an agent with any role in the **Assigned Agent** list. However, if you want to specifically assign a supervisor, you can do so by escalating the conversation.

To escalate a conversation to a human agent with the `Supervisor` role, click the **↑ Escalate to Supervisor** button. The button appears in the **Conversation Actions** section if the Inbox settings [allow agents to escalate conversations to supervisors](/live-agent/settings/inboxes#escalate-conversations-to-supervisors).
The supervisor will be assigned to the conversation in the Inbox.

Additionally, supervisors can track the number of escalated conversations in the [Supervisor Overview Report](/live-agent/reports/supervisor-overview).
...
When a conversation is initiated, it is automatically assigned to the inbox defined in the Cognigy Flow. However, you can change the inbox for an existing conversation if needed.
...
1. In the left-side menu of Live Agent, click **Conversations**.
2. From the **Conversations** list, select the conversations where you want to change the inbox and click ![change inbox](/_assets/icons/black-and-white/live-agent/change-inbox.svg).
...
- [Manual Assignment](/live-agent/conversation/conversation-routing/manual-mode)
- [Automatic Assignment](/live-agent/conversation/conversation-routing/automatic-mode)
