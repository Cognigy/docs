...
description: Set up automatic conversation assignment and reassignment in Live Agent to evenly distribute incoming chats among human agents.
keywords: ['automatic routing', 'auto assignment', 'round-robin', 'live agent', 'conversation distribution']
...
- **Online Agents**. When a conversation is created in the Inbox, and there are agents online with access to it, the system distributes the conversations evenly among them.
  Unassigned conversations are automatically assigned to agents when the [limit threshold](/live-agent/settings/account-settings#conversation-limit-per-agent) is lower than the maximum value, and they take precedence over newly incoming conversations.
- **Offline Agents**. When a conversation is created in the Inbox, and there are no agents online with access to it, the system will leave all conversations unassigned.
...
<Frame>
    <img src="/_assets/live-agent/conversation/round-robin.png" alt="Round-robin assignment cycling conversations among three agents" style={{ width: 'auto' }} />
</Frame>
...
2. Select an Inbox from the list and click ![Inbox settings](/_assets/icons/black-and-white/gear.svg).
...
When one of these events is triggered, the conversations assigned to the human agent are automatically reassigned to other available human agents. If no human agents are currently online, the conversations are marked as unassigned.
...
those assigned conversations will remain in the [Conversation Queue](/live-agent/conversation/conversation-queue/overview).
However, new conversations will not be assigned to that agent until they're no longer in the `Busy` status.

You can modify this behavior by enabling the [Auto Assign Conversations to a Busy Agent](/live-agent/settings/account-settings#auto-assign-conversations-to-busy-agents) option in **Account Settings**. Note that this setting is activated at the Live Agent installation level, and you can't modify the behavior for a specific Inbox.
...
<AccordionGroup>
  <Accordion title="Behavior for Until Next Reply">
  ...
  <Accordion title="Behavior for Other Snooze Options">
...
<AccordionGroup>
  <Accordion title="Default Scenario">
    ...
    - No [conversation limit](/live-agent/settings/account-settings#conversation-limit-per-agent) per agent.
    ...
    Result: Agent 1 receives the first and fourth conversations, Agent 2 receives the second, Agent 3 gets the third.
  </Accordion>
  <Accordion title="Scenario 1">
   ...
   <Accordion title="Scenario 7">
    ...
    - The limit is 5 conversations per agent.
    - Each agent is assigned to one conversation.
    - Activated:
        - Automatic Conversation Assignment.
        - Automatic Conversation Reassignment.
        - Consider Conversation Count.
    ...
    **Example 1**
    ...
    - 1 new incoming conversation with the `Technical support`, `English` skills, and **Low** priority.
    ...
    **Example 2**
    ...
    - 1 new incoming conversation with the **Low** priority.
    - 1 new incoming conversation with the `Technical support`, `Billing`, `Spanish` skills, and **Urgent** priority.
    - Deactivated: Auto Assign Conversations to a Busy Agent.
...
- [Manual Conversation Mode](/live-agent/conversation/conversation-routing/manual-mode)
