...
description: "Track outbound call statuses in Cognigy Voice Gateway using webhook responses and call events. Monitor SIP response codes and identify call flow issues."
keywords: ['outbound call status', 'call tracking', 'webhook', 'sip response', 'voice gateway']
---

In Cognigy.AI, you can monitor [call events in your Flow](/voice-gateway/references/events/overview) to track the progress of both inbound and outbound calls.
...
To receive webhook responses for outbound calls, you need to add the [`notifyUrl`](/voice-gateway/creating-outbound-calls#create-outbound-calls-via-api-request) parameter with the appropriate webhook URL to the outbound call request.
...
- [Create Outbound Calls](/voice-gateway/creating-outbound-calls)
