---
title: Applications
description: "Configure Voice Gateway Applications to route Endpoints and set default speech vendor, TTS, and STT language and voice settings for call handling."
keywords: ['voice gateway applications', 'speech vendor', 'endpoint routing', 'call recording', 'tts stt configuration']
---
...
You can create more than one application, dependent on the number of Endpoints created in Cognigy.

After creating an application, you can edit or delete it.

<Frame>
  <img src="/_assets/voice-gateway/VG-webapp-applications-overview.png" alt="Voice Gateway applications overview page with Endpoint routing and speech vendor defaults" style={{ width: 'auto' }} />
</Frame>
...
| Calling Webhook                                         | A Cognigy Endpoint URL for the Web Application that will handle calls. Specify your Endpoint URL from the Voice Gateway Endpoint. Select the **POST** request type.                                      | Account, Service Provider, Admin |
| Call Status Webhook                                     | A Cognigy Endpoint URL for the Web Application that will receive the call status. Specify your Endpoint URL from the Voice Gateway Endpoint. Select the **POST** request type.                           | Account, Service Provider, Admin |
...
| Perform a fallback transfer if webhook connection fails | [Redirect a call](#call-forwarding) if the connection to Cognigy is unavailable for any reason.                                                                                                          | Account, Service Provider, Admin |
...
Note that this feature records the entire call, and you cannot manage it within a Cognigy Flow.
...
<Tabs>
  <Tab title="Forward an existing call">
             ...
             2. *(Optional)* In the **Reason** field, specify the reason for transferring a call, for example, `Cognigy connection wasn't established`. The value will be transmitted in the `"X-Reason": "<reason>"` format within a custom SIP header and will be present in the pcap file on the recipient's side for further analysis.<br/>
