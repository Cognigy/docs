...
description: "Integrate speech-to-text and text-to-speech vendors in Voice Gateway. Configure STT/TTS providers, voices, accents, and languages for AI voice agents."
...
<Warning>
  Elevenlabs deprecated and removed the following models:
  
  **Deprecated:** Turbo v2 and Turbo v2.5. If your speech service uses either of these models, ElevenLabs recommends migrating to Flash v2 and Flash v2.5.
  *Removed:** Monolingual v1 and Multilingual v1. If your speech service uses either of these models, migrate to the newer Multilingual v2.
</Warning>
...
If you have an Account scope, you can only edit speech services you have created, as well as speech services created by other users with the same scope. You can still use and view speech services shared by Service providers or Admins.

<Frame>
  <img src="/_assets/voice-gateway/VG-webapp-speech-services.png" alt="Voice Gateway speech services page listing configured TTS and STT vendors" style={{ width: 'auto' }} />
</Frame>
...
To configure the connection for a cloud-based speech service:
    ...
    <Tabs>
      <Tab title="Amazon Polly">
            ...
            1. Make sure you have set the following [IAM policies](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_elements_statement.html):
            
            <AccordionGroup>
            <Accordion title="Amazon Polly Minimal IAM Policy (TTS)">

              To use Amazon Polly as a TTS service, you need at least the following IAM policy:
              ...
              - `polly:SynthesizeSpeech` — allows the service to synthesize speech.
              - `polly:DescribeVoices` — allows the service to list the available voices.
              - `*` — allows the service to access all resources. AWS doesn't support resource-level restrictions.

              

            </Accordion>
            <Accordion title="Amazon Transcribe Minimal IAM Policy (STT)">

              Voice Gateway uses real-time streaming transcription for speech recognition. To use Amazon Polly's speech services, you need to configure Amazon Transcribe's STT policy to allow real-time streaming transcription. To do so, configure at least the following IAM policies:
              ...
              - `transcribe:StartStreamTranscription` — allows the service to start a real-time streaming transcription.
              - `transcribe:StartStreamTranscriptionWebSocket` — allows the service to start a real-time streaming transcription via WebSocket.
              - `*` — allows the service to access all resources. AWS doesn't support resource-level restrictions.
            </Accordion>
            </AccordionGroup>
            
            2. Enter the Access Key in the **Access key ID** field. For more information on AWS Access Keys, read the [Amazon AWS](https://repost.aws/knowledge-center/create-access-key) documentation.
      ...
      <Tab title="Deepgram">
        ...
        2. From the **Region** list, select one of the following regions to host the data processed by Deepgram:
            - **United States** — points to the endpoint for Deepgram services located in the U.S.
            - **Europe** — points to the endpoint for Deepgram services located in the EU.
        3. _(Optional)_ Activate the **Opt in to the Deepgram Model Improvement Program** option:
           - When the option is active, Deepgram may retain this account's call audio and transcripts and use them to improve its models. Participating in the Model Improvement Program may qualify you for a lower per-request cost. For more information, read [Deepgram's description of the program](https://developers.deepgram.com/docs/the-deepgram-model-improvement-partnership-program).
           - When the option is inactive, Deepgram doesn't retain or use your call data to train its models. Opting out keeps your audio and transcripts private but may increase your Deepgram cost.

            <Note>
              The option defaults to the `DEEPGRAM_IMPROVEMENT_PROGRAM_OPT_OUT` setting configured for your Voice Gateway installation. For on-premises installations, you set this under `cognigyEnv` in the `values.yaml` file. This setting applies only to new Deepgram configurations. Existing configurations keep their previously saved setting.
            </Note>
      </Tab>
      <Tab title="Deepgram Flux">
           ...
           2. From the **Region** list, select one of the following regions to host the data processed by Deepgram Flux:
                 - **United States** — points to the endpoint for Deepgram Flux services located in the U.S.
                 - **Europe** — points to the endpoint for Deepgram Flux services located in the EU.
           3. _(Optional)_ Configure the **Opt in to the Deepgram Model Improvement Program** toggle. This setting works the same as for Deepgram. For details, go to the [Deepgram](#deepgram) tab.
      </Tab>
      <Tab title="ElevenLabs">

            1. Select your language model from the **Model** list. The following TTS models are supported:

                - Multilingual v3
                - Multilingual v2
                - English v2
                - Turbo v2 (deprecated)
                - Turbo v2.5 (deprecated)
                - Flash v2
                - Flash v2.5

            2. Select **Use hosted ElevenLabs service** and configure the following:
                - **Region** — select the region to host the data processed by ElevenLabs:
                    ...
                    - **Europe**<sup>[1](#footnote1)</sup> — points to the endpoint for ElevenLabs services located in the European Union. Selecting data residency is an ElevenLabs Enterprise feature. For more information, read the ElevenLabs documentation on [Data Residency](https://elevenlabs.io/docs/product-guides/administration/data-residency). The EU environment doesn't support legacy voices. Legacy voices include a legacy tag in the voice selection in Cognigy. If you select the EU region, ensure you don't select legacy voices.
...
<VoiceGatewaySpeechServicesHowto/>
    ...
    <Tabs>
      <Tab title="Deepgram">
            ...
            2. Enter the container URI for STT and TTS in the **Container URI** field.
