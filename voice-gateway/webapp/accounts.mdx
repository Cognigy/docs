---
title: Accounts
description: "Manage Voice Gateway accounts and control user access, call recording, and API key settings."
keywords: ['accounts', 'account settings', 'call recording', 'api keys']
---

<a href="/release-notes/2026.11"><Badge className="version-badge" color="blue">Updated in 2026.11</Badge></a>

*Accounts* are containers for resources such as [applications](/voice-gateway/webapp/applications), [carriers](/voice-gateway/webapp/carriers), and [speech services](/voice-gateway/webapp/speech-services) that are used to create and manage voice applications.
...
<Frame>
  <img src="/_assets/voice-gateway/VG-webapp-accounts.png" alt="Voice Gateway accounts page listing resource containers for voice applications" style={{ width: 'auto' }} />
</Frame>
...
| Application for SIP device calls        | Select the application which is used to handle incoming calls from SIP users. | Account, Service Provider, Admin |
| Default carrier for WebRTC calls | Select the carrier used to place outbound calls from the Click to Call widget.                                               | Account, Service Provider, Admin |
...
| Account API keys                        | You can generate an API key by clicking **Add key**.                                                                            | Account, Service Provider, Admin |
...
Note that this feature records the entire call and cannot be managed within a Cognigy Flow.
...
All calls from all applications on this account will be recorded and appear in the [Recent calls](/voice-gateway/webapp/recent-calls#call-recordings) page.
