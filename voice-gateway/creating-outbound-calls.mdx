...
description: "Learn how to create outbound calls in Voice Gateway using the REST API, Transfer Node, or HTTP Request Node, with SIP trunk and phone number setup."
keywords: ['outbound calls', 'rest api', 'sip trunk', 'voice gateway', 'call initiation']
---
...
Before initiating an outbound call, ensure you have configured an [application](/voice-gateway/webapp/applications) and added the phone number you intend to call from to [Phone Numbers](/voice-gateway/webapp/phone-numbers).

To initiate an outgoing call, use one of the following methods:

- [Transfer Node](/ai/agents/develop/node-reference/voice/voice-gateway/transfer)
- REST API request:
...
When the call is answered, Voice Gateway invokes the specified webhook to manage the call.
You authorize the request with a Bearer token.
This token contains an [API key value](/voice-gateway/webapp/accounts#account-level-api-keys) generated in the Voice Gateway Self-Service Portal.
...
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Trial (trial.cognigy.ai) | Choose the environment you use:<ul><li>**Trial (EU)** - `https://api-vg-trial.cognigy.ai`</li><li>**Trial (US)** - `https://api-vg-trial-us.cognigy.ai`</li></ul> |
| SaaS                     | Choose the environment you use:<ul><li>**EU** - `https://voicegateway-app-vg.cognigy.ai`</li><li>**US** - `https://api-vg-us.cognigy.ai`</li><li>**UK** - `https://api-vg-uk.cognigy.ai`</li><li>**AU** - `https://api-vg-au.cognigy.ai`</li><li>**Other SaaS** - use the `https://api-<company-name>-vg.cognigy.cloud` pattern where `<company-name>` is your company name. For example, if your company name is `abc123`, the URL would be:<br />`https://api-abc123-vg.cognigy.cloud`</li></ul> |
| On-premises              | Specify the API URL based on your Voice Gateway deployment configuration.                                                                                                                                                                                                                                                                                         |
| NiCE CXone Environments  | Use the `cognigy-vg-api-<region>.nicecxone.com` pattern, where `<region>` matches your NiCE CXone cluster. For example, `cognigy-vg-api-na1.nicecxone.com`, `cognigy-vg-api-eu1.nicecxone.com`. For a full list of available regions, refer to [NiCE CXone Shared SaaS Clusters](/ai/administer/installation/ip-ranges-shared-environments#nice-cxone-shared-saas-clusters). |
...
The basic configuration provides details on initiating and managing calls, enabling the system to connect the caller and callee.
It also includes call duration, handling of unanswered calls, and the attachment of relevant metadata.
...
<Tabs>
  ...
  <Tab title="JSON">
    ...
    | to                        | [TargetType](/voice-gateway/references/verbs/dial#target-types) | The destination target, either a phone number or a SIP URI.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Yes      |
    ...
    | headers                   | Object                                              | An object containing custom SIP headers that can be applied to outbound call attempts. These headers are added alongside the system (default) headers in the SIP request. Custom SIP headers provide more control over the call setup process, including adjustments to authentication, routing, and other specific SIP parameters. For example, a custom header could be `P-Preferred-Identity: <sip:+123456@sip.cognigy.cloud>`, where the caller's preferred identity is the specified phone number or SIP address to present during the call. | No       |
...
we enable the [Answering Machine Detection](/voice-gateway/references/verbs/amd) feature
using the `amd` parameter
and use the `actionHook` parameter to send events to [Webhook](https://webhook.site/).
Via `timeout`, we define a maximum ringing time. If the call is not answered within this time, Voice Gateway stops calling (see the [NO_ANSWER](/voice-gateway/references/events/NO_ANSWER) event).
...
To implement this use case, use a [Transfer Node](/ai/agents/develop/node-reference/voice/voice-gateway/transfer) in your voice Flow.
...
offer a resolution to the user's issue, or follow up on their satisfaction with the support received.
...
To implement this use case, use an [HTTP Request Node](/ai/agents/develop/node-reference/service/http-request) in your voice Flow, or create an [API Request via Postman or CLI](#create-outbound-calls-via-api-request).

## More Information

- [Outbound Call Statuses](/voice-gateway/call-statuses)
- [Outbound Call Routing](/voice-gateway/webapp/outbound-call-routing)
- [Overview](/voice-gateway/overview)
