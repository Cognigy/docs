...
| listenDuringPrompt  | If this parameter is `false`, the system will not listen for user speech until the [`say`](/ai/agents/develop/node-reference/basic/say) or [`play`](/voice-gateway/references/verbs/play) verbs has completed. The default value is `true`. | No       |
...
| play                | A nested [`play`](/voice-gateway/references/verbs/play) verb that can be used to prompt the user.                                                                                                      | No       |
