...
description: This event triggers when the AI Agent ends a call due to the user not responding within the timeout period.
keywords: ['user input timeout', 'call event', 'no response', 'voice gateway', 'call timeout']
---
...
Input: Start a call via the Interaction Panel by clicking ![phone](/_assets/icons/blue/phone.svg) **> Start Call**. When the call starts, say "Hello". Wait several seconds until a payload with the `USER_INPUT_TIMEOUT` event appears.
