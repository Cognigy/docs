...
description: This event triggers when a SIP REFER call transfer fails due to an unavailable or rejecting destination.
keywords: ['transfer refer error', 'sip refer', 'call transfer failure', 'voice gateway', 'call event']
---
...
This event is triggered when `refer_status` is `202` and `final_referred_call_status` is any value other than `200`, including `410 Gone`. When this event fires, any configured failover or error hook runs automatically.
