...
description: This event triggers when an active call is established.
keywords: ['call in progress', 'call event', 'active call', 'voice gateway', 'sip status']
---
...
1. In your voice Flow, add a [Question](/ai/agents/develop/node-reference/basic/question),
   [Optional Question](/ai/agents/develop/node-reference/basic/optional-question),
   or [Wait For Input](/ai/agents/develop/node-reference/logic/wait-for-input) Node before the [Lookup](/ai/agents/develop/node-reference/logic/lookup) Node
   where the `CALL_IN_PROGRESS` event is selected.
2. In the [Voice Gateway](/ai/agents/deploy/endpoint-reference/voice-gateway) Endpoint, activate the **Enable Call Event: Call in Progress** setting.
