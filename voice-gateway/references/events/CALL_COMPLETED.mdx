...
description: This event triggers when a call is terminated by the AI Agent or user, with example payload details.
keywords: ['call completed', 'call event', 'call termination', 'voice gateway', 'payload']
---
...
Input: Start a call via the Interaction Panel by clicking ![phone](/_assets/icons/blue/phone.svg) **> Start Call**. The call will either automatically end when the AI Agent terminates the conversation due to inactivity, or you can end the call by clicking **End Call**.
