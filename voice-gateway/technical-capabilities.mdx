...
description: "Explore Voice Gateway technical capabilities including SIP protocols, audio codecs, TTS/STT providers, call recording, and call quality monitoring tools."
keywords: ['voice gateway capabilities', 'sip protocols', 'audio codecs', 'tts stt providers', 'call recording']
---
...
| DTMF                                                           | The use of RTP payloads to carry DTMF events, as specified in [RFC 2833](https://datatracker.ietf.org/doc/html/rfc2833).                                                                                        |
...
- [Answering machine detection](/voice-gateway/references/verbs/amd).
- Configurable music on hold.
- Configurable [Atmosphere Sounds](/ai/agents/develop/node-reference/voice/voice-gateway/parameter-details) playing in the background of the conversation.
- Configurable [Silence Overlay](/ai/agents/develop/node-reference/voice/voice-gateway/parameter-details) playing background sounds when there's a long pause in a call.
- [TTS streaming](/voice-gateway/streaming) to reduce latency.

### Call Management

- [Creating outbound calls via REST API](/voice-gateway/creating-outbound-calls).
- Performing operations on calls in progress via REST API.

### STT and TTS providers

- Many cloud-based and on-premises [speech providers](/voice-gateway/references/tts-and-stt-vendors) are available, along with the option to integrate [custom speech API](/voice-gateway/webapp/speech-services#add-custom-speech-vendors) tailored to your needs.
- [Fallback speech configuration](/voice-gateway/webapp/applications#add-additional-tts-and-stt-vendor) if the primary vendor fails.

### Call Recording and Storage

- [Call recording](/voice-gateway/webapp/recent-calls#call-recordings) via integration with a third-party SIPREC server.
- Pausing or resuming transcription and recording to avoid the collection of Personally Identifiable Information (PII).
- Call recording with no third-party components, [recordings streamed to the preferred cloud storage](/voice-gateway/webapp/recent-calls#call-recordings).

### Call Quality Monitoring and Optimization

- Observability features include [tracing call events with open telemetry](/voice-gateway/webapp/recent-calls).
- Visual view of [speech latency](/voice-gateway/webapp/recent-calls#call-recordings) (TTS and STT) on each call.
- [Caching of TTS audio](/voice-gateway/webapp/accounts#tts-prompt-caching) when possible to reduce costs.

## Technical Support

For any further or more specific questions about the Voice Gateway technical capabilities, contact [Cognigy technical support](/help/get-help).
