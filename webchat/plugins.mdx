...
description: "Extend Cognigy Webchat with plugins from the GitHub plugin collection or build custom plugins to add new UI components and behaviors to the chat widget."
keywords: ['webchat plugins', 'custom plugins', 'webchat customization', 'plugin collection', 'webchat extensions']
---

<a href="/release-notes/4.93"><Badge className="version-badge" color="blue">Updated in 4.93</Badge></a>
...
3. In Cognigy, select a Project from the Projects list.
4. In **Deploy > Endpoints**, select an Endpoint or create a new one:

<Tabs>
  <Tab title="Webchat v3">
...
You can set up plugins in your code in the embedding code of your [Webchat v3](/webchat/v3/embedding/hosted-script) or [Webchat v2](/webchat/v2/embedding). To add the plugins in your code, proceed as follows:

<Tabs>
  <Tab title="Plugin from Collection">
        1. Open the [Cognigy GitHub repository](https://github.com/Cognigy/WebchatPlugins/tree/master) and select the plugin you want to use.
        2. On the [Releases](https://github.com/Cognigy/WebchatPlugins/releases) page, copy the URL of the JavaScript file from the **Assets** section of the plugin you selected. For example, the URL for the Google Firebase Authentication Webchat Plugin is `https://github.com/Cognigy/WebchatPlugins/releases/download/google-firebase-authentication100/google-firebase-authentication.webchat-plugin.js`.
...
2. In the **Data** field, enter the following JSON to trigger the plugin. 
...
You can build an [Extension](/ai/for-developers/extensions) that sends a message triggering the Webchat plugin,
