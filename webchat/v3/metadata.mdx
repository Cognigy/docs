...
sidebarTitle: Collecting Metadata
description: "Configure Webchat v3 to collect user metadata such as browser language, geolocation, and device type to personalize AI Agent conversations and analytics."
keywords: ['webchat metadata', 'browser data collection', 'geolocation', 'user personalization', 'session data']
---

<a href="/release-notes/4.81"><Badge className="version-badge" color="blue">Updated in 4.81</Badge></a>
...
This data is stored in the [Input](/ai/agents/develop/ai-agent-memory/input) object, enabling you to create more tailored and effective interactions with the user.
After the session, the data is available through the [Insights Conversations explorer](/insights/explorers/conversations) and [Cognigy.AI OData endpoint](/ai/agents/analyze/odata).
...
- **Interface Optimization**. Optimize the user interface for different devices (mobile, tablet, desktop) and operating systems. Ensure compatibility and address issues specific to different browser versions.
...
| browser             | String | The name of the browser that the user has.                                                                                                                                                                                         | Chrome                           |
| browser_version     | String | The version of the browser that the user has.                                                                                                                                                                                      | 126.0.0.0                        |
| os                  | String | The operating system of the user's device.                                                                                                                                                                                         | Linux                            |
...
You can test the metadata object only in [Live Follow](/ai/agents/test/interaction-panel/live-follow) mode.
...
5. Get and copy the [user ID](/ai/agents/analyze/user-and-session-identification#user-id) from the chat interaction. Use developer tools of your browser. For example, in the Google Chrome browser, open **Developer Tools**, navigate to **Application > Storage**, and expand **Session Storage**. Click a domain to view its key-value pairs. Next to the `userId` key, copy the value.
6. In the [Interaction Panel](/ai/agents/test/interaction-panel/overview), paste the user ID to the **User ID** field on the Live Follow tab.
7. Send a message as a user via the [Demo Webchat](/webchat/demo) interface. Once the message is received, you will see the same message on the **Live Follow** tab.
8. View the metadata in the Interaction Panel:
    - To view the metadata in the payload, click ![expand](/_assets/icons/black-and-white/expand.svg).
...
- [Endpoint Configuration](/webchat/v3/configuration)
- [Interaction Panel](/ai/agents/test/interaction-panel/overview)
- [Input Object](/ai/agents/develop/ai-agent-memory/input)
