---
title: 'Webchat v3: Endpoint Configuration'
sidebarTitle: Endpoint Configuration
description: "Learn how to create and configure a Webchat v3 Endpoint in Cognigy, including deployment settings, appearance options, and AI Agent customization."
keywords: ['webchat v3 configuration', 'endpoint settings', 'webchat v3 setup', 'ai agent deployment', 'webchat endpoint']
---
...
1. In the left-side menu of the Cognigy interface, select a Project.
...
- [NLU Connectors](/ai/platform-features/nlu/external/nlu-connector-reference/all-nlu-connectors)
...
<Accordion title="Supports Inject and Notify">
  You can use the [Inject and
  Notify](/ai/agents/deploy/endpoints/inject-and-notify) feature with this
  Endpoint.
</Accordion>
...
>
  <video
    controls=""
    title="Classic style preset: smaller Webchat widget with bordered AI Agent messages"
    aria-label="Classic style preset: smaller Webchat widget with bordered AI Agent messages"
    style={{
  ...
  <p style={{ maxWidth: "200px", margin: "10px auto 0 auto", fontSize: "14px" }}>
    <strong>Classic:</strong> The classic, smaller Webchat widget with bordered
    AI Agent messages.
  </p>
</div>
...
>
  <video
    controls=""
    title="Modern style preset: larger, wider Webchat widget with borderless AI Agent messages"
    aria-label="Modern style preset: larger, wider Webchat widget with borderless AI Agent messages"
    style={{
      border: "1px solid #ccc",
      borderRadius: "12px",
      width: "100%",
  ...
  <p style={{ maxWidth: "300px", margin: "10px auto 0 auto", fontSize: "14px" }}>
    <strong>Modern:</strong> A larger, wider Webchat widget with borderless AI
    Agent messages.
  </p>
</div>
...
>
  <video
    controls=""
    title="Slick style preset: larger but narrower Webchat widget with bordered messages and grey background"
    aria-label="Slick style preset: larger but narrower Webchat widget with bordered messages and grey background"
    style={{
      border: "1px solid #ccc",
      borderRadius: "12px",
      width: "100%",
  ...
  <p style={{ maxWidth: "200px", margin: "10px auto 0 auto", fontSize: "14px" }}>
    <strong>Slick:</strong> A larger but narrower Webchat widget with bordered
    AI Agent messages with a grey background.
  </p>
</div>

</div>

Selecting a style preset automatically applies predefined values to the following settings: Webchat Layout (including Colors) and Webchat Behavior.
...
| Logo URL                                                       | Customizes the Webchat logo at the top of the chat interface. To do that, enter the URL of the image you want. Recommended formats: JPG, JPEG, SVG, PNG, GIF with a size of 28px x 28px.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Use different logo and name for Virtual & Human Agent Messages | By default, the specified name and logo are applied to messages written by AI and human agents. By activating this toggle, you can specify separate logos and names for AI and human agent messages. <br/><br/>If you use [Live Agent](/live-agent/index) as your handover provider, Webchat v3 will prioritize the Display Name and Profile Image from the [Profile Settings](/live-agent/profile-settings#user-profile-appearance) in Live Agent, regardless of whether the **Use different logo and name for Virtual & Human Agent Messages** parameter is enabled or disabled.<br/>In cases where neither the Display Name nor the Profile Image is provided, the following will apply:<br/>- When **Display Name** is not available, the system will use a value from the **Human Agent Avatar Name** parameter if provided. Otherwise, the Webchat name specified in the **Name** parameter will be displayed. <br/>- When **Profile Image** is not available, the system will use a value from the **Human Agent Avatar Logo URL** parameter if provided. Otherwise, a default human agent avatar will be displayed. |
...
| Human Agent Avatar Name                                        | Customizes the name displayed above each message sent by human agents. If no input is provided, the Webchat name specified in the **Name** parameter will be displayed.<br/><br/>If you use [Live Agent](/live-agent/index) as your handover provider, Webchat v3 will prioritize the Display Name from the [Profile Settings](/live-agent/profile-settings#user-profile-appearance) in Live Agent. When **Display Name** is not available, the system will use a value from the **Human Agent Avatar Name** parameter if provided. Otherwise, the Webchat name specified in the **Name** parameter will be displayed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Human Agent Avatar Logo URL                                    | Customizes the logo displayed above each message sent by human agents. If no input is provided, the logo specified in the **Logo URL** parameter will be displayed.<br/><br/> If you use [Live Agent](/live-agent/index) as your handover provider, Webchat v3 will prioritize the Profile Image from the [Profile Settings](/live-agent/profile-settings#user-profile-appearance) in Live Agent. When **Profile Image** is not available, the system will use a value from the **Human Agent Avatar Logo URL** parameter if provided. Otherwise, a default human agent icon will be displayed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
#### Webchat Icon

The Webchat Icon settings allow you to customize the icon displayed on your website to open the Webchat widget. Additionally, you can animate the icon and set the animation frequency and speed. After setting the icon animation, you can preview the animation in the icon field.

| Parameter          | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Icon               | Select or upload an icon to be displayed on your website to open the Webchat widget. You can select the following:<ul><li>![default message bubble](/_assets/icons/black-and-white/default-webchat-icon.svg)<br/>This is the default icon.</li><li>![shiny message bubble](/_assets/icons/black-and-white/shiny-message-bubble-icon.svg)</li><li>![round message bubble](/_assets/icons/black-and-white/round-message-bubble-icon.svg)</li><li>![upload custom](/_assets/icons/black-and-white/transparent-plus-square.svg)<br/>Upload your own icon.</li></ul> |
| Icon animation     | Select the animation for the icon. You can select one of the following options:<ul><li>**None** — the icon is displayed statically. This option is set by default.</li><li>**Bounce** — the icon bounces vertically.</li><li>**Swing** — the icon swings back and forth.</li><li>**Pulse** — the icon pulses.</li></ul>                                                                                                                                                                                                                                                                     |
| Animation interval | Set the interval between animations in seconds. The default value is 5 seconds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Animation speed    | Select the speed of the animation. You can select one of the following options:<ul><li>**Slow** — halves the animation speed.</li><li>**Normal** — this is the default option.</li><li>**Fast** — doubles the animation speed.</li><li>**Superfast** — multiplies the animation speed by 3.</li></ul>                                                                                                                                                                                                                                                                                    |

#### Colors

The Colors settings let you customize the Webchat v3 interface, including the primary and secondary colors.
...
| Text Link                 | Sets the color in which text links within messages will be displayed. |

#### Persistent Menu

The Persistent Menu settings ensure quick access to different conversation stages, guide users, offer features, share information, and enhance the overall user experience.
...
| Payload                | The payload text that will be sent to your AI Agent Flow when the user selects this menu item. This payload can be a simple word or phrase, or a more complex query, depending on your AI Agent's functionality. |

#### Advanced Layout Settings

The Advanced Layout settings provide parameters for managing the chat interface, focusing on input behavior, HTML styling, and JavaScript.

| Parameter                              | Description                                                                                                                                                                                                                                                                                                                                                                  |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Disable Bot Output Border              | Hides the chat bubble around AI Agent text output. Hiding the chat bubble makes AI Agent text outputs look cleaner and fit better into the chat screen. This parameter is useful for custom designs that include system messages or long messages. It also improves the layout of interactive elements, such as buttons and links, as well as static content, such as tables. |
...
| Maximum Number of Input Rows           | Determines the maximum limit of lines that can be entered into the Reply field. If the text exceeds this limit, a vertical scrollbar will automatically appear to make the input text easier to display.                                                                                                                                                                         |
| Enable Input Collation                 | Combines typed inputs from the Reply field into a single message after a one-second delay without new text input. Immediate feedback or rapid responses are sent right away, prompting the transmission of any grouped messages.                                                                                                                       |
...
| Enable generic HTML                    | Applies basic styling to HTML content within standard text messages. When this feature is activated, the system applies a standard style to HTML code, enhancing the appearance of your text messages.                                                                                                                                                                |
| Allow JavaScript in HTML Message       | Enables including JavaScript functionalities, such as `onclick` or `onload` attributes, in your HTML messages. By default, these elements are removed before the message is displayed to minimize the risk of harmful content. When this setting is activated, these elements will not be removed.                                                                       |
| Allow JavaScript in Button/Action URLs | Enables JavaScript functionalities within button or action URLs in your interface. By default, JavaScript URLs starting with 'javascript:' are blocked for security reasons. However, activating this setting preserves such URLs, allowing the embedded JavaScript code to execute when users interact with the button or element.                                           |
...
| Collate Streamed Outputs           | Allows multiple streamed text messages to appear in the same chat bubble as they are received, rather than showing each one separately. The configuration applies to streamed outputs from the [AI Agent Node](/ai/agents/develop/node-reference/ai/ai-agent) or [LLM Prompt Node](/ai/agents/develop/node-reference/service/llm-prompt). To achieve the best results with this feature, activate the **Progressive Message Rendering** and **Render Markdown** parameters as well.                                                                                                                                                                                                          |
...
| Render Markdown                    | Enables Markdown rendering for AI Agent text outputs. It supports basic formatting options, including italics, bold text, links, and headers, as well as advanced features such as tables, footnotes, and nested lists. Formulas are not supported.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Collect Metadata                   | Lets you collect additional data from your end users' browsers, such as language, location, device, and browser type. By using this data, you can personalize the AI Agent's behavior. For example, you can offer the user the option to switch the conversation language to their browser language, or provide offers relevant to their current location. For more information, read [Collect Metadata](/webchat/v3/metadata).                                                                                                                                                                                                                                          |
| Enable Typing Indicators           | Shows when the AI Agent is typing a response, helping users understand when to expect replies. When the parameter is turned on, a typing symbol appears, signaling that the AI Agent is processing your request and will respond soon. <br/><br/>If you integrate Webchat v3 with a contact center, only [Cognigy Live Agent](/live-agent/index), [Genesys Cloud Open Messaging](/ai/escalate/handover-reference/genesys-cloud-open-messaging), [Salesforce MIAW](/ai/escalate/handover-reference/salesforce-miaw), and [NiCE CXone](/ai/escalate/handover-reference/nice-cxone/nice-cxone-integrated) support the typing indicator.                                                                                                                                         |
...
| Enable Connection Status Indicator | Displays a warning message if the user's internet connection is lost during a conversation with the AI Agent. This ensures users are aware of potential interruptions or delays in the chat. The warning automatically disappears once the internet connection is re-established, indicating that the chat can proceed normally.                                                                                                                                                                                                                                                                                                                                            |
...
| Parameter                        | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Enable Business Hours            | Restricts the availability of the Webchat widget to specified business hours, ensuring live chat support during those times. Outside of business hours, options include hiding, disabling, or displaying a message informing users. This feature helps companies effectively manage customer support and align with operational timings.                                                                                                                                                                                                                                                                  |
...
| Parameter                      | Description                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Enable Home Screen             | Ensures the home screen is displayed whenever the Webchat widget is launched, creating a welcoming initial interface for users to begin their interaction. If the parameter is not activated, users will be shown the privacy notice page (if activated) or the standard chat interface.                                                                                            |
| Welcome Text                   | This parameter appears if **Enable Home Screen** is activated. This parameter lets you customize the greeting message displayed when the Webchat widget launches, providing users with an inviting entry point into the conversation. By default, the message is `Welcome! How can we help?`                                                                                                  |
...
You can customize messages, choose an interaction type (Postback Value or URL), and add up to 5 starters.
...
You can customize the title of the Chat Options screen to personalize the user experience.
...
Note that after deletion, the conversation will be removed from the Webchat widget on the end user's side but will still be available in the Cognigy OData endpoint, Insights, and Contact Profile.
...
| Title                       | Lets you personalize the text of the quick reply. You can tailor the quick responses to address frequently asked questions or to provide additional information.                                                                                                                               |
...
| URL                         | Defines the specific URL that users will be directed to when selecting a Quick Reply. It can guide users to relevant web pages or resources, enhancing their experience by providing immediate access to the information they need.                                                                              |
| Add                         | Enables you to include more Quick Replies in your list, with a maximum limit of 5 Quick Replies.                                                                                                                                                                                                          |

#### Text-to-Speech

The Text-to-Speech settings allow you to convert text to speech, with options to customize the toggle label and set the toggle to be enabled by default.

| Parameter                      | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Show text-to-speech toggle     | Lets you activate the text-to-speech feature that converts text messages into spoken voice output. This feature is especially beneficial for users who prefer or require auditory feedback. <br/><br/> By default, this parameter is inactive. Enabling this parameter gives users the option to activate text-to-speech. Note that the text-to-speech feature uses the Browser API, so the available voices depend on the OS and browser vendor. |
...
The Conversation Rating settings include a chat rating widget that lets users rate their experience and provide feedback.
You can customize text prompts and confirmation messages to ensure a user-friendly feedback process.

| Parameter                          | Description                                                                                                                                                                                                                                                    |
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Enable Rating Functionality        | Activates the conversation rating mechanism in the chat options page, allowing users to rate and provide feedback on their chat experience. Enabling this feature lets you gather direct user feedback to continually improve the AI Agent's effectiveness.    |
...
| Rating Submitted Event Banner Text | Enables you to customize the text that appears in a banner after a user successfully submits their rating and feedback. The default message is `Your feedback was submitted`.                                                                             |
...
| Add           | Lets you include a new Title and URL pair to the footer. You can add up to 2 pairs for convenience, ensuring the footer remains uncluttered.                             |
...
Accepting the privacy notice in the chat doesn't count as a user input in terms of [billable conversations](/ai/administer/billing#billable-conversations).
...
| Parameter      | Description                                                                                                                                                                                                                                                                                |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Watermark      | Lets you select the type of watermark that appears at the bottom of the chat interface, acting as a subtle brand or platform identifier. You have the ability to choose between **Default**, **Custom**, and **None**. If Default is selected, the Cognigy Watermark will be displayed. |
| Watermark Text | This parameter appears when the Custom option is selected for the watermark. This parameter enables you to specify your own text. The default text is `Powered by Cognigy`.                                                                                                             |
| Watermark URL  | This parameter appears when the Custom option is selected for the watermark. This parameter lets you specify your URL as a hyperlink in the format `https://www.example.com/`. The default URL is `https://www.cognigy.com/`.                                             |
...
| Maintenance Information Text  | This parameter applies to the following modes: **Inform users about maintenance mode** and **Disable the Webchat Widget**.<br/> It determines the text displayed to users during Maintenance mode, allowing for a customizable message to inform users about the maintenance activity or any other relevant information.                                                                                                                                                                                                                                                                                                                                                                              |
...
- Change and translate UI text and [aria labels](/webchat/v3/accessibility#aria-labels). You can use dynamic strings, such as `{name}` or `{time}`, as described in the [embedding](https://github.com/Cognigy/Webchat/blob/main/docs/embedding.md) documentation.
- Add values from embedding files not present in the GUI.
- Specify parameters for your custom Webchat version.
- Use features from a later Cognigy version while using an earlier one.

<AccordionGroup>
  <Accordion title="JSON Example: Change Webchat Colors">
  ...
  <Accordion title="JSON Example: Translate Webchat Texts">
...
For more information on creating and triggering plugins, refer to [Webchat Plugins](/webchat/plugins).

### Demo Webchat Settings

Demo Webchat lets you test integrations with different providers and preview Webchat's appearance before deployment.
It's a valuable tool for ensuring your Webchat functions as expected before launch.
For more information on how to use this feature, refer to [Demo Webchat](/webchat/demo).

## More Information

- [Webchat v3: Overview](/webchat/v3/overview)
- [Webchat v3: Features](/webchat/v3/features)
