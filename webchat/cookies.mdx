...
description: "Understand how Cognigy Webchat uses cookies for session management and privacy configuration."
...
Webchat uses [WebSockets](https://websockets.spec.whatwg.org/) to create a persistent connection between the user's browser and the Cognigy backend.
...
| ------------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------- | ----------------------------------------------------------- |
| `_752f8` (Mandatory Session Cookie)   | Always required for routing Webchat messages to the Cognigy backend instance.                       | Expires at the end of the browsing session. |  Preserves visitor sessions for AI‑driven customer support. |
...
sequenceDiagram
    ...
    participant B as Cognigy Backend
...
2. In the [Webchat embedding configuration](/webchat/v3/embedding/hosted-script#customize-the-webchat-settings), use the `disableWebsockets` and `forceWebsockets` options to control WebSocket behavior.
