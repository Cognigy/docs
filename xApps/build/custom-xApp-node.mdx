...
description: "Advanced guide for building a custom xApp Node using the Cognigy Extension SDK, giving full control over xApp display and dialog Flow logic."
keywords: ['custom xapp node', 'xapp extension', 'cognigy extension', 'xapp sdk', 'custom node development']
---
...
To create a custom xApp extension, you will need to build an [Extension](/ai/for-developers/extensions) from scratch using a [template](https://github.com/Cognigy/Extensions/tree/master/docs/example) provided in the Cognigy repository on GitHub. Below is the code snippet that you can use within the Extension.
...
Use the [setAppState API](/xApps/api) with an `app template URL` set to `generic-html` and `app template data` that contain the HTML code generated.
